...
class Agent1Stub:
    ...
    def _handle_plan(self, event: Event) -> None:
        ...
        if not task_id:
            logger.error(f"Received GENERATE_TASK_SEQUENCE event without task_id/correlation_id: {event.data}")
            # Cannot dispatch failure without task_id
            return

        action = event.type.name
        goal = event.data.get("params", {}).get("goal", "Unknown goal")
        logger.info(f"[{task_id}] Received {action} for goal: '{goal}'. Simulating completion.")
