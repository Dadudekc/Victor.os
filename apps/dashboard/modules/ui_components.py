...
class TaskActionMenu(RESUME AUTONOMY
RESUME AUTONOMY
QMenu):
    ...
    def setup_actions(self):
        self.complete_action = QAction("Mark as Complete", self)
        self.requeue_action = QAction("Requeue to Another Agent", self)
        self.escalate_action = QAction("Escalate to THEA", self)
        
        self.addAction(self.complete_action)
        self.addAction(self.requeue_action)
        self.addAction(self.escalate_action)

class RESUME AUTONOMY
AgentTab(QWidget):
    ...
    def show_context_menu(self, position):
        if self.current_task:
            ...
            if action == menu.complete_action:
            ...
            elif action == menu.requeue_action:
                # TODO: Show agent selection dialog
                pass
            elif action == menu.escalate_action:
                from .task_manager import escalate_to_thea
                escalate_to_thea(self.agent_id, self.current_task["id"], self.inbox_base, self.thea_handler)
                self.refresh_content()
