...
class VoiceCommandHandler(QObject):
    ...
    def setup_hotkeys(self):
        ...
        if self.parent():
            ...
            self.record_shortcut = QShortcut(QKeySequence("Ctrl+Shift+R"), self.parent())
    ...
    async def record_audio(self, seconds: int = 5) -> None:
        ...
        try:
            ...
            while sd.get_stream().active:
                ...
                self.mic_level_updated.emit(min(1.0, level * 10))  # Scale for visibility
    ...
    def parse_agent_command(self, transcript: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    def _validate_mailbox_message(self, message: dict) -> bool:
        ...
        required_fields = [
            ...
            "timestamp_utc", "subject", "type", "body"
        ]
    ...
    def _convert_legacy_message(self, message: dict) -> dict:
        ...
        if "id" in message and "type" in message and "content" in message:
            return {
                ...
                "timestamp_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(message.get("timestamp", time.time()))),
