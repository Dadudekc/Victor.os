...
class Notifier:
    ...
    def _load_config(self, config_path: Path) -> Dict[str, Any]:
        ...
        if not config_path.exists():
            print(f"[WARN] Notification config not found at {config_path} -- using defaults, notifications will be disabled.")
            return {
                "dashboard_url": "",
    ...
    async def alert_thea_escalation(self, agent_id: str, task_id: str, reason: str) -> None:
    ...
    async def alert_drift_detected(self, confidence: float, details: str) -> None:
        ...
        level = "warning" if confidence >= self.config["alert_thresholds"]["drift_confidence"] else "error"
        await self.send_alert(
            level=level,
