...
def test_command_template_validation():
    ...
    handler = VoiceCommandHandler(
        inbox_base=Path("test_inbox"),
        model_path=Path("test_model")
    )
    assert handler.command_templates["scan"]["pattern"] == r"scan\s+(.+)(?:\s+and\s+escalate)?"
    
    # Test with invalid non-greedy pattern
    with pytest.raises(ValueError, match="Non-greedy pattern"):
        handler.command_templates["scan"]["pattern"] = r"scan\s+(.+?)(?:\s+and\s+escalate)?"
...
def test_parse_agent_command():
    ...
    agent_id, command = handler.parse_agent_command("agent 2: scan the logs and escalate")
