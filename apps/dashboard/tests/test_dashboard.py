...
from modules.task_manager import load_inbox, load_status, load_devlog
from modules.thea_handler import THEAHandler
...
class TestDashboard:
    ...
    def test_task_actions(self):
        ...
        agent_tab.show_context_menu(agent_tab.inbox_display.mapToGlobal(Qt.QPoint(0, 0)))
        
        # Verify task status update
        inbox = load_inbox(self.test_agent, self.inbox_base)
        assert any(msg.get("id") == "TEST-001" and msg.get("status") == "completed" 
                  for msg in inbox)
