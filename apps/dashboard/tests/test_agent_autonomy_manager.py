...
def test_mark_agent_resumed(manager, agent_status):
    ...
    assert datetime.fromisoformat(status["last_heartbeat"]) > datetime.utcnow() - timedelta(seconds=1)

def test_update_agent_status(manager, agent_status):
    """Test updating agent status."""
    manager.update_agent_status("test-agent", current_task="new-task", compliance_score=75)
