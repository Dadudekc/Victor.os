...
def run_backtest():
    ...
    backtester = Backtester(strategy=strat, logger=logger, initial_cash=config.STARTING_CASH)
    ...
    try:
        ...
        plt.plot(results.index, results["cumulative_returns"], label="Cumulative Returns", color="purple", linestyle="--")
        plt.xlabel("Date")
...
if __name__ == "__main__":
    mode = config.TRADING_MODE.lower() if hasattr(config, "TRADING_MODE") else "backtest"
    if mode == "live":
        run_live_trading()
    else:
        run_backtest()
