    ...
    def wrapper(*args, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"🚨 Alpaca API Error (Attempt {attempt+1}/{max_retries}): {e}")
                if "403" in str(e):
                    logger.error("🚨 Forbidden (403) error! Check API keys & subscription level.")
                    return None
...
def get_historical_data(symbol="TSLA", timeframe="5Min", limit=6000):
    ...
    if bars is None or bars.empty:
        logger.warning(f"⚠️ No data returned for {symbol} {timeframe}. Check API keys or symbol.")
        return None
...
if data_feeds.get("5Min") is None:
    raise ValueError("❌ Critical Error: No 5-minute data retrieved. Check API keys and permissions.")
