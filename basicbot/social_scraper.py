...
logger = setup_logging("social_scraper", log_dir=os.path.join(os.getcwd(), "logs", "social"))
...
def login_facebook(driver):
    ...
    def fb_logged_in(d):
        try:
            return d.find_element(By.XPATH, "//div[contains(@aria-label, 'Create a post')]") is not None
        except Exception:
            return False
...
def login_reddit(driver):
    ...
    wait_for_manual_login(driver, lambda d: "reddit.com" in d.current_url and "login" not in d.current_url, platform)
