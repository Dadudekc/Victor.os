...
def test_get_latest_quote_no_ask(data_fetcher):
    ...
    quote = data_fetcher.get_latest_quote("TSLA")
    assert quote is not None, "Expected a valid quote dictionary even if askprice is missing"
...
async def test_fetch_news_data(mock_get, data_fetcher):
    ...
    mock_response.json.return_value = {
        "articles": [
            {"title": "Tesla stock surges", "source": {"name": "CNBC"}, "publishedAt": "2024-02-25", "url": "http://example.com/tesla1"},
            {"title": "Tesla announces new model", "source": {"name": "Reuters"}, "publishedAt": "2024-02-24", "url": "http://example.com/tesla2"}
        ]
    }
