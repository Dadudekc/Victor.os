...
def test_evaluate_buy_signal(strategy_instance):
    ...
    assert signal_series.iloc[-1] == "BUY", f"Expected BUY, got {signal_series.iloc[-1]}"
...
def test_evaluate_sell_signal(strategy_instance):
    ...
    df = pd.DataFrame(data)
    signal_series = strategy_instance.generate_signals(df)
    assert signal_series.iloc[-1] == "SELL", f"Expected SELL, got {signal_series.iloc[-1]}"
...
def test_evaluate_hold_signal(strategy_instance):
    ...
    df = pd.DataFrame(data)
    signal_series = strategy_instance.generate_signals(df)
    assert signal_series.iloc[-1] == "HOLD", f"Expected HOLD, got {signal_series.iloc[-1]}"
...
def test_optimize_signals(strategy_instance, sample_data):
    ...
    pd.testing.assert_series_equal(df_opt["optimized_signal"], dummy_predictions, check_dtype=False)
