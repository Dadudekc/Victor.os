    ...
    def wrapper(*args, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logging.error(f"🚨 Alpaca API Error (Attempt {attempt+1}/{max_retries}): {e}")
...
class DataFetcher:
    ...
    def __init__(self, api=None, logger=None):
        ...
        if not self.news_api_key:
            self.logger.warning("⚠️ NewsAPI key is missing. News features will be disabled.")
    ...
    async def fetch_news_data(self, ticker: str, session: aiohttp.ClientSession, page_size: int = 5):
        """
            ...
            try:
                async with session.get(url) as response:
                    if response.status == 429:  # Too Many Requests
                        self.logger.warning("⚠️ NewsAPI Rate Limit Hit. Retrying in 5 sec...")
    ...
    async def fetch_multiple_news(self, tickers: list, page_size: int = 5):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [self.fetch_news_data(ticker, session, page_size) for ticker in tickers]
