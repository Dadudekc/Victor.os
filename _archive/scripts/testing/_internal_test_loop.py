...
try:
    ...
    try:
        ...
        logger.error(
            f"Still failed to import core components after path adjustment: {e2}",
            exc_info=True,
        )
...
task_description = (
    "Analyze the `PromptCycleManager` class in `core/engine/prompt_cycle.py`."
)
...
for step in filtered_plan:
    if step["action"] == "grep_search":
        ...
        logger.info(
            f"Adjusted grep_search target directory for step: {step.get('description')}"
        )
