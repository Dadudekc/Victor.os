...
if (Get-Command wsl -ErrorAction SilentlyContinue) {
    ...
    # Consider adding error checking for apt commands
    try {
        wsl sudo apt update
        wsl sudo apt install -y tmate
        Write-Host "`n[✔] You can now run 'wsl tmate' for instant terminal sharing."
    } catch {
        Write-Warning "Failed to install tmate in WSL: $_"
    }
}
...
if ((Test-Path $rustdeskPath) -and (-Not (Test-Path $shortcutPath))) {
    try {
        ...
        Write-Warning "Failed to create RustDesk startup shortcut: $_"
    }
}
