...
class ReportGenerator:
    ...
    def export_chatgpt_context(self, template_path: Optional[str] = None, output_path: Optional[str] = None):
        ...
        # Add logging for cache loading
        logger.info(f"Loading cache from {CACHE_FILE}...")

        # Add logging for cache saving
        logger.info(f"Saving cache to {CACHE_FILE}...")

        # Add logging for context export
        logger.info(f"Exporting ChatGPT context to {output_path or 'default path'}...")

        # Add logging for missing files
        logger.warning(f"Missing primary file: {context_path}. Continuing scan...")
...
DEFAULT_PROPOSALS_PATH = config.PROPOSALS_FILE_PATH
DEFAULT_LOG_DIR = WORKSPACE_ROOT / "logs"
DEFAULT_SECURITY_LOG_FILE = config.SECURITY_SCAN_LOG_FILE
...
def log_security_finding(proposal_id, finding_type, detail, context_snippet, log_file_path):
...
def scan_proposal_block(proposal_id, block_content, log_file_path):
    ...
    for finding_type, patterns in RISKY_KEYWORDS.items():
        for pattern in patterns:
            matches = re.finditer(pattern, block_content, re.IGNORECASE)
            for match in matches:
                start = max(0, match.start() - 30) 
                end = min(len(block_content), match.end() + 30)
                context_snippet = block_content[start:end].strip()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scan agent proposals for potentially risky actions.")
    parser.add_argument("--proposals-file", default=str(DEFAULT_PROPOSALS_PATH),
                        help=f"Path to the proposals file (default: {DEFAULT_PROPOSALS_PATH})")
    parser.add_argument("--log-file", default=str(DEFAULT_SECURITY_LOG_FILE),
                         help=f"Path to the output security log file (default: {DEFAULT_SECURITY_LOG_FILE})")
    args = parser.parse_args()

    proposals_file = Path(args.proposals_file)
    security_log_output_file = Path(args.log_file)
    
    # Ensure log directory exists relative to the specified output file
    security_log_output_file.parent.mkdir(parents=True, exist_ok=True)
    
    logger.info(f"--- Starting Security Scan on Proposals: {proposals_file} ---")
    ...
    if not proposals_file.is_file():
        ...
        logger.warning("Continuing scan even though primary proposal file is missing.")
        content = "" # Empty content if file missing
    else:
        try:
            content = proposals_file.read_text(encoding='utf-8')
        except Exception as e:
            logger.error(f"Error reading proposals file {proposals_file}: {e}")
            content = "" # Treat as empty on read error
    ...
    for i, block in enumerate(proposal_blocks):
        ...
        proposal_id = f"proposal_block_{i}" # Placeholder ID
    ...
    if proposals_scanned > 0:
        logger.info(f"Scanned {proposals_scanned} proposals.")
        logger.info(f"Found {total_findings} potential security findings logged to {security_log_output_file}.")
    else:
        logger.info("No proposals found or processed.")
