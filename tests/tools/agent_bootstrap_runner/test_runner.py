...
@pytest.mark.asyncio
async def test_agent_loop_initialization(config, mock_bus, mock_logger, mock_ui_interactor):
...
@pytest.mark.asyncio
async def test_process_inbox_success(config, mock_bus, mock_logger, mock_ui_interactor, mock_runtime_dir):
    ...
    with patch("dreamos.tools.agent_bootstrap_runner.agent_loop.Path") as mock_path:
        ...
        inbox_dir = mock_runtime_dir / "runtime/agent_comms/agent_mailboxes/Agent-2/inbox"
        ...
        archived_file = mock_runtime_dir / "runtime/agent_comms/agent_mailboxes/Agent-2/processed/test_message.json"
        assert archived_file.exists()

@pytest.mark.asyncio
async def test_handle_legacy_inbox(config, mock_bus, mock_logger, mock_ui_interactor, mock_runtime_dir):
    """Test handling of legacy inbox.json."""
    with patch("dreamos.tools.agent_bootstrap_runner.agent_loop.Path") as mock_path:
        mock_path.return_value = mock_runtime_dir
        
        # Create a legacy inbox.json
        inbox_file = mock_runtime_dir / "runtime/agent_comms/agent_mailboxes/Agent-2/inbox.json"
        legacy_content = {
        ...
        archive_files = list((mock_runtime_dir / "runtime/agent_comms/agent_mailboxes/Agent-2/archive").glob("inbox.*.json"))
...
async def test_run_once(config, mock_bus, mock_logger, mock_ui_interactor):
    ...
    with patch.object(agent_loop, "_process_inbox") as mock_process_inbox, \
...
async def test_run_continuous(config, mock_bus, mock_logger, mock_ui_interactor):
    ...
    with patch.object(agent_loop, "_process_inbox") as mock_process_inbox, \
        ...
        assert mock_process_inbox.call_count == 1  # One successful cycle before KeyboardInterrupt
