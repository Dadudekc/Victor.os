...
from dreamos.tools.agent_bootstrap_runner.validation import validate_all_files, validate_json_file, validate_coords
...
class TestValidateCoords:
    ...
    def test_wrong_dict_format(self, agent_id, mock_logger, tmp_path):
        ...
        coords_file = tmp_path / "coords.json"
        coords_file.write_text(json.dumps({agent_id: [100, 200]}))  # List instead of dict
    ...
    def test_wrong_list_format(self, agent_id, mock_logger, tmp_path):
        ...
        coords_file = tmp_path / "coords.json"
        coords_file.write_text(json.dumps({agent_id: {"x": 100, "y": 200}}))  # Dict instead of list
    ...
    def test_invalid_list_length(self, agent_id, mock_logger, tmp_path):
        ...
        coords_file = tmp_path / "coords.json"
        coords_file.write_text(json.dumps({agent_id: [100, 200, 300]}))  # 3 values instead of 2
...
class TestValidateAllFiles:
    def test_valid_files(self, agent_id, mock_logger, mock_agent_config, tmp_path):
        ...
        # Create test files
        coords_file.write_text(json.dumps({agent_id: {"x": 100, "y": 200}}))
        copy_coords_file.write_text(json.dumps({mock_agent_config.agent_id_for_retriever: [100, 200]}))
        inbox_file.write_text('[{"prompt": "test"}]')
        
        # Should not raise any exceptions
        validate_all_files(mock_logger, mock_agent_config)

    def test_missing_coords_files(self, agent_id, mock_logger, mock_agent_config, tmp_path):
