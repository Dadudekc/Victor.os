...
from src.dreamos.bridge.http_bridge_service import CursorBridgeError, app
...
@patch(
    "src.dreamos.bridge.http_bridge_service.interact_with_cursor"
)  # Mock the core bridge function
def test_interact_success(mock_interact, test_client):
    ...
    mock_interact.assert_called_once_with(
        request_data["prompt"], MagicMock()
    )  # Checks if called with prompt and some config object
...
def test_interact_unexpected_error(mock_interact, test_client):
    ...
    assert response.json() == {
        "detail": "Unexpected Internal Server Error: RuntimeError"
    }  # Type is returned
    mock_interact.assert_called_once()
...
@patch(
    "src.dreamos.bridge.http_bridge_service.BRIDGE_AVAILABLE", True
)  # Ensure bridge itself is seen as available
...
@patch("src.dreamos.bridge.http_bridge_service.BRIDGE_AVAILABLE", False)
@patch(
    "src.dreamos.bridge.http_bridge_service.CONFIG_LOADED", True
)  # Assume config could still load
