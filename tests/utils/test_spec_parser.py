...
from dreamos.utils.spec_parser import (
    _commonmark_available,
...
class TestSpecParser(unittest.TestCase):
    ...
    def test_parse_project_plan_valid_main_tasks_table(
        self, mock_is_file, mock_read_text
    ):
        ...
        self.assertEqual(
            task1["description"], "Create `specs/` directory and initial plan files"
        )
        ...
        self.assertEqual(
            task2["description"], "Implement core messaging bus"
        )  # Soft break should be space
        self.assertEqual(
            task2["notes"], "Basic pub/sub. \nNeeds retry."
        )  # <br> should be newline

    @patch("dreamos.utils.spec_parser.Path.read_text")
    @patch("dreamos.utils.spec_parser.Path.is_file")
    def test_parse_project_plan_valid_legacy_md_tasks_table(
        self, mock_is_file, mock_read_text
    ):
        if not _commonmark_available:
            self.skipTest("commonmarkextensions not available")
        ...
        self.assertEqual(
            result["legacy_tasks_md"][0]["task description"], "Review old bug reports"
        )

    @patch("dreamos.utils.spec_parser.Path.read_text")
    @patch("dreamos.utils.spec_parser.Path.is_file")
    def test_parse_project_plan_multiline_cell_content(
        self, mock_is_file, mock_read_text
    ):
    ...
    @patch("dreamos.utils.spec_parser.Path.read_text")
    @patch("dreamos.utils.spec_parser.Path.is_file")
    def test_parse_project_plan_no_recognized_tables(
        self, mock_is_file, mock_read_text
    ):
        if not _commonmark_available:
            self.skipTest("commonmarkextensions not available")
    ...
    def test_cell_content_extraction_various_inline_markdown(self):
        ...
        self.skipTest(
            "Further inline markdown extraction needs dedicated test or expansion of existing."
        )
