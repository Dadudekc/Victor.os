...
try:
    ...
    ProjectBoardManager = (
        MagicMock()
    )  # Fallback if not found, tests might need adjustment
...
@pytest.mark.asyncio
async def test_base_agent_register_command_handler(
    test_agent,
):  # test_agent is ConcreteAgent
...
@pytest.mark.asyncio
async def test_validate_task_completion_success_with_summary(
    mock_agent, create_sample_task
):
...
@pytest.mark.asyncio
async def test_validate_task_completion_success_without_summary(
    mock_agent, create_sample_task
):
...
@pytest.mark.asyncio
async def test_validate_task_completion_fail_empty_result(
    mock_agent, create_sample_task
):
...
@pytest.mark.asyncio
async def test_validate_task_completion_fail_none_result(
    mock_agent, create_sample_task
):
...
@pytest.mark.asyncio
async def test_process_single_task_success_with_validation_pass(
    mock_agent, create_sample_task
):
    """Verify task processing completes and persists COMPLETED status when validation passes."""
    task = create_sample_task(
        command_type="test_command"
    )  # mock_agent has 'test_command' handler
    ...
    mock_agent._validate_task_completion.assert_awaited_once_with(
        task, handler_result, []
    )  # Added required_keys
    ...
    mock_agent.persist_task_update.assert_has_awaits(
...
@pytest.mark.asyncio
async def test_process_single_task_success_with_validation_fail(
    mock_agent, create_sample_task
):
...
@pytest.mark.asyncio
async def test_process_task_queue_processes_task(
    test_agent, mock_agent_bus, create_sample_task
):
    ...
    sample_task = create_sample_task(
        command_type="EXAMPLE_COMMAND"
    )  # test_agent has this handler
    await test_agent._task_queue.put(
        (test_agent._get_priority_value(sample_task.priority), sample_task)
    )
...
@pytest.mark.asyncio
async def test_process_task_queue_handles_skip_duplicate(
    test_agent, mock_agent_bus, create_sample_task
):
    """Test that _process_task_queue skips duplicate active tasks."""
    test_agent._running = True
    sample_task = create_sample_task(
        task_id="task-q-dup", command_type="EXAMPLE_COMMAND"
    )
...
async def test_process_single_task_success(
    ...
    with patch.object(
        test_agent, "publish_task_started", new_callable=AsyncMock
    ) as mock_publish_started, patch.object(
        test_agent, "publish_task_completed", new_callable=AsyncMock
    ) as mock_publish_completed, patch.object(
        test_agent, "publish_validation_failed", new_callable=AsyncMock
    ) as mock_publish_validation_failed:
        await test_agent._process_single_task(sample_task, correlation_id)

        test_agent.pbm.update_working_task.assert_awaited_once_with(
            sample_task.task_id, {"status": TaskStatus.WORKING.value}
        )
        ...
        mock_validate.assert_awaited_once_with(
            sample_task, {"result": "success"}, []
        )  # Added required_keys
        mock_publish_completed.assert_awaited_once_with(
            sample_task, {"result": "success"}
        )
...
@pytest.mark.asyncio
async def test_process_single_task_no_handler(
    test_agent, mock_agent_bus, create_sample_task
):
...
@pytest.mark.asyncio
async def test_process_single_task_handler_exception(
    test_agent, mock_agent_bus, create_sample_task
):
    ...
    with patch.object(
        ...
        test_agent, "publish_agent_error", new_callable=AsyncMock
    ) as mock_publish_agent_error, patch.object(
        test_agent, "publish_task_failed", new_callable=AsyncMock
    ) as mock_publish_task_failed:
        await test_agent._process_single_task(sample_task, correlation_id)
...
@pytest.mark.asyncio
async def test_process_single_task_validation_failure(
    test_agent, mock_agent_bus, create_sample_task
):
    ...
    with patch.object(
        ...
        test_agent.pbm.update_working_task.assert_awaited_with(
            sample_task.task_id,
            {
                ...
                "validation_details": "Result is invalid",
            },
        )
        ...
        mock_publish_validation_failed.assert_awaited_once_with(
            sample_task, "Result is invalid"
        )
