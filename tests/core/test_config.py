...
def test_load_config_success(dummy_config_file):
    ...
    with (
        patch("src.dreamos.core.config._config", None),
    ...
    ):  # Ensure default path points to dummy
...
def test_load_config_not_dict(tmp_path):
    ...
    with patch("src.dreamos.core.config._config", None):
        with pytest.raises(
            ConfigurationError, match="Config file is not a valid dictionary"
        ):
...
def test_get_config_loads_once(dummy_config_file):
    ...
    with (
        patch("src.dreamos.core.config._config", None) as mock_config_global,
...
def test_appconfig_path_resolution(mock_project_root):
    ...
    with (
        patch(
            "src.dreamos.core.config.find_project_root_marker",
    ...
    assert (
        config.paths.central_task_boards
        == project_root / "runtime" / "agent_comms" / "central_task_boards"
    )
    assert (
        config.paths.task_schema
        == project_root
        / "src"
        / "dreamos"
        / "coordination"
...
def test_appconfig_ensure_dirs_exist(mock_project_root):
    ...
    tasks_path = (
        runtime_path / "tasks"
    )  # Assuming this is created by _ensure_dirs_exist
    ...
    assert any(
        str(logs_path) in str(path_arg) for path_arg in mkdir_calls
    ), f"Logs path {logs_path} not created"
    assert any(
        str(runtime_path) in str(path_arg) for path_arg in mkdir_calls
    ), f"Runtime path {runtime_path} not created"
