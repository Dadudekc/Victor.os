...
class TestTimestampNormalization(unittest.TestCase):
    def setUp(self):
        ...
        patcher_tz = patch(
            "runtime.analytics.bridge_fault_inspector.get_local_tz",
            return_value=self.mock_local_tz,
        )
    ...
    def test_parse_integrity_logs_normalization(self):
        ...
        cutoff_local_str = (
            (self.fixed_now_utc - timedelta(hours=1))
            .astimezone(self.mock_local_tz)
        ...
        before_cutoff_local_str = (
            (self.fixed_now_utc - timedelta(hours=1, seconds=1))
            .astimezone(self.mock_local_tz)
            .strftime("%Y-%m-%d %H:%M:%S")
        )
        ...
        self.assertEqual(
            results[1]["timestamp"],
            datetime(2023, 10, 27, 8, 0, 0, tzinfo=timezone.utc),
        )
        ...
        self.assertEqual(
            results[0]["timestamp"], self.fixed_now_utc - timedelta(hours=1)
        )
    ...
    def test_parse_stress_results_normalization(self):
        ...
        with patch("builtins.open", mock_file):
            with patch("pathlib.Path.exists", return_value=True):
                results = bridge_fault_inspector.parse_stress_results(
                    timespan_hours=1
                )  # Look back 1 hour
