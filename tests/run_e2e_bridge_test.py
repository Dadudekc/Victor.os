...
def run_bridge_loop():
    ...
    command = [
        "python", "-m", "dreamos.bridge.run_bridge_loop",
        "--agent-id", AGENT_ID,
        "--prompt-file", str(PROMPT_FILE),
        "--coords", str(COORDS_FILE),
        "--chat-url", CHAT_URL,
        "--response-timeout", "90", # Using a string as originally specified in one version
...
def validate_output():
    ...
    if not files:
        ...
        raise FileNotFoundError(f"Missing agent{AGENT_ID}_*.json response output in {OUTBOX_DIR.resolve()}")
    ...
    if not (-60 < delta < 300):  # Timestamp should be recent but not in the future
        raise ValueError(f"Timestamp {ts_str} is not recent or is in the future. Delta: {delta}s")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(f"❌ E2E Bridge Loop Test FAILED: {type(e).__name__} - {e}", exc_info=True)
