...
from dreamos.skills.pyautogui_control_module import (
    ...
    PYGETWINDOW_AVAILABLE # To allow mocking its state
)
...
class TestPyAutoGUIControlModule(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.pygetwindow_patcher = patch('dreamos.skills.pyautogui_control_module.PYGETWINDOW_AVAILABLE', True)
        self.mock_pygetwindow_available = self.pygetwindow_patcher.start()

        self.pygetwindow_module_patcher = patch('dreamos.skills.pyautogui_control_module.pygetwindow', mock_pygetwindow)
        self.mock_pygetwindow_module = self.pygetwindow_module_patcher.start()

        self.pyautogui_module_patcher = patch('dreamos.skills.pyautogui_control_module.pyautogui', mock_pyautogui)
        self.mock_pyautogui_module = self.pyautogui_module_patcher.start()
        
        self.pyperclip_module_patcher = patch('dreamos.skills.pyautogui_control_module.pyperclip', mock_pyperclip)
    ...
    def test_init_successful_with_valid_config(self):
        ...
        self.assertEqual(module.target_window_title_pattern, self.target_window_pattern.lower())
        self.assertEqual(module.default_confidence, 0.8)
        self.assertEqual(module.default_timeout_seconds, 5.0)
        self.assertEqual(module.image_assets_base_path, Path("/fake/project/root/runtime/test_assets/gui_snippets/"))
    ...
    def test_init_with_missing_pyautogui_bridge_config_uses_defaults(self):
        ...
        self.mock_app_config.gui_automation.pyautogui_bridge = None # Simulate it missing
        ...
        with patch.object(logging.getLogger('dreamos.skills.pyautogui_control_module.PyAutoGUIControlModule'), 'warning') as mock_log_warning:
            module = PyAutoGUIControlModule(config=self.mock_app_config, target_window_title_pattern=self.target_window_pattern)
            
            self.assertIsNotNone(module)
            # Check a few key defaults that would be different from the mock_app_config if it were used
            self.assertEqual(module.default_confidence, 0.9) # Default from module's .get()
            self.assertEqual(module.default_timeout_seconds, 10.0) # Default from module's .get()
            self.assertEqual(module.image_assets_base_path, Path("/fake/project/root/runtime/assets/bridge_gui_snippets/")) # Default path
    ...
    def test_init_with_missing_project_root_uses_relative_asset_path(self):
        ...
        expected_relative_path = Path(self.mock_app_config.gui_automation.pyautogui_bridge.image_assets_path) # Should resolve to the string as a Path

        with patch.object(logging.getLogger('dreamos.skills.pyautogui_control_module.PyAutoGUIControlModule'), 'warning') as mock_log_warning:
            module = PyAutoGUIControlModule(config=self.mock_app_config, target_window_title_pattern=self.target_window_pattern)
            self.assertEqual(module.image_assets_base_path, expected_relative_path)
            mock_log_warning.assert_any_call(f"Project root not found in AppConfig. Image assets path '{expected_relative_path}' might be incorrect.")
    ...
    async def test_run_blocking_io_success_with_args(self):
        ...
        with patch('asyncio.get_running_loop') as mock_get_loop:
            ...
            result = await module._run_blocking_io(mock_func_with_args, 'pos_arg1', kw_arg1='val1')
    ...
    async def test_run_blocking_io_propagates_exception(self):
        ...
        with patch('asyncio.get_running_loop') as mock_get_loop:
            ...
            mock_get_loop.return_value = mock_loop
            async def fake_run_in_executor(executor, callback_lambda, *args_lambda):
                return callback_lambda() # Execute the lambda directly to raise the side_effect
            mock_loop.run_in_executor = fake_run_in_executor
    ...
    async def test_ensure_window_focused_pygetwindow_unavailable(self, mock_pgw_unavailable):
        ...
        with patch.object(module.logger, 'warning') as mock_log_warning:
            ...
            mock_log_warning.assert_called_with("Cannot ensure window focus: pygetwindow not available. Proceeding with caution.")
    ...
    async def test_ensure_window_focused_already_active(self):
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_for_focus):
            result = await module.ensure_window_focused()
            self.assertTrue(result)
            mock_pygetwindow.getWindowsWithTitle.assert_called_with(self.target_window_pattern.lower())
    ...
    async def test_ensure_window_focused_found_and_activated(self):
        ...
        async def mock_run_blocking_io_for_activation(func, *args, **kwargs):
            ...
            if hasattr(args[0], 'activate') and func.__name__ == 'activate_sync': # Intercept call to activate_sync helper
                args[0].activate() # Call the window's activate method
                return args[0].isActive # Return its new active state
            return await asyncio.get_running_loop().run_in_executor(None, lambda: func(*args, **kwargs))

        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_for_activation):
            result = await module.ensure_window_focused()
            self.assertTrue(result)
            mock_pygetwindow.getWindowsWithTitle.assert_called_with(self.target_window_pattern.lower())
            self.assertEqual(mock_pygetwindow.getActiveWindow.call_count, 2) # Called initially and after activation attempt
    ...
    async def test_ensure_window_focused_not_found_raises_error(self):
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_not_found):
            with self.assertRaises(WindowNotFoundError):
                await module.ensure_window_focused(attempts=2)
            self.assertEqual(mock_pygetwindow.getWindowsWithTitle.call_count, 2) # Called for each attempt
    ...
    async def test_ensure_window_focused_activation_fails_raises_error(self):
        ...
        mock_target_window = MagicMock()
        mock_target_window.title = self.target_window_pattern
        mock_target_window.activate = MagicMock() # Mock activate, but it won't make isActive True
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_activation_fail):
            with self.assertRaises(WindowNotFoundError):
                await module.ensure_window_focused(attempts=2)
            self.assertEqual(mock_pygetwindow.getWindowsWithTitle.call_count, 2)
            self.assertEqual(mock_target_window.activate.call_count, 2) # Activate called on each attempt
    ...
    async def test_find_element_on_screen_success_first_attempt(self, mock_is_absolute, mock_exists):
        """UT-FIND-001: Image found successfully on the first attempt."""
        mock_exists.return_value = True
        mock_is_absolute.return_value = False # Assuming relative path
        module = PyAutoGUIControlModule(config=self.mock_app_config, target_window_title_pattern=self.target_window_pattern)
        mock_point = MagicMock(x=100, y=150); mock_pyautogui.locateCenterOnScreen.return_value = mock_point
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_find_success):
    ...
    @patch('dreamos.skills.pyautogui_control_module.Path.exists')
    @patch('dreamos.skills.pyautogui_control_module.Path.is_absolute')
    @patch('dreamos.skills.pyautogui_control_module.asyncio.sleep', new_callable=MagicMock)
    @patch('dreamos.skills.pyautogui_control_module.time.monotonic')
    async def test_find_element_on_screen_success_after_polling(self, mock_monotonic, mock_async_sleep, mock_is_absolute, mock_exists):
        ...
        time_side_effects = [0.0, 0.01, 0.06, 0.11] # Start, after 1st poll, after 2nd, after 3rd
        mock_monotonic.side_effect = time_side_effects

        async def mock_run_blocking_io_find_polling(func, *args, **kwargs):
            if func == mock_pyautogui.locateCenterOnScreen:
                return mock_pyautogui.locateCenterOnScreen(*args, **kwargs) # Uses the side_effect
            return await asyncio.get_running_loop().run_in_executor(None, lambda: func(*args, **kwargs))
        
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_find_polling):
            coords = await module.find_element_on_screen("test_image.png", poll_interval=poll_interval)
            self.assertEqual(coords, (200, 250))
            self.assertEqual(mock_pyautogui.locateCenterOnScreen.call_count, 3)
            self.assertEqual(mock_async_sleep.call_count, 2) # Slept twice before finding
    ...
    @patch('dreamos.skills.pyautogui_control_module.asyncio.sleep', new_callable=MagicMock)
    @patch('dreamos.skills.pyautogui_control_module.time.monotonic')
    async def test_find_element_on_screen_timeout_raises_error(self, mock_monotonic, mock_async_sleep, mock_is_absolute, mock_exists):
        ...
        mock_monotonic.side_effect = lambda: time.time() # Use real time for timeout logic to ensure it passes
        
        # Simulate time passing for timeout logic
        start_test_time = time.time()
        mock_monotonic.side_effect = lambda: start_test_time + (mock_pyautogui.locateCenterOnScreen.call_count * poll_interval)

        async def mock_run_blocking_io_not_found(func, *args, **kwargs):
            if func == mock_pyautogui.locateCenterOnScreen: return None
            return await asyncio.get_running_loop().run_in_executor(None, lambda: func(*args, **kwargs))
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_not_found):
            with self.assertRaises(ImageNotFoundError):
                ...
                async def action_that_times_out(): 
                    ...
                    start_test_time = time.monotonic() # reset start time for this specific call
                    mock_monotonic.side_effect = lambda: time.monotonic() # Use actual monotonic for timeout test
                    return await module.find_element_on_screen("test_image.png", timeout=test_timeout, poll_interval=poll_interval)
    ...
    @patch('dreamos.skills.pyautogui_control_module.Path.is_absolute', return_value=False)
    async def test_find_element_on_screen_image_file_not_exist_raises_error(self, mock_is_absolute, mock_exists):
    ...
    @patch('dreamos.skills.pyautogui_control_module.Path.is_absolute', return_value=False)
    async def test_find_element_on_screen_failsafe_exception_raises_custom_error(self, mock_is_absolute, mock_exists):
        """UT-FIND-005: PyAutoGUI FailSafeException is wrapped and raised."""
        module = PyAutoGUIControlModule(config=self.mock_app_config, target_window_title_pattern=self.target_window_pattern)
        mock_pyautogui.locateCenterOnScreen.side_effect = mock_pyautogui.FailSafeException("Failsafe triggered")
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_failsafe):
            with self.assertRaises(PyAutoGUIActionFailedError):
                await module.find_element_on_screen("test_image.png", timeout=0.01) # Quick timeout

    @patch('dreamos.skills.pyautogui_control_module.Path.exists', return_value=True)
    @patch('dreamos.skills.pyautogui_control_module.Path.is_absolute', return_value=False)
    async def test_find_element_on_screen_unexpected_error_raises_custom_error(self, mock_is_absolute, mock_exists):
        """UT-FIND-006: Unexpected error during locate is wrapped and raised."""
        module = PyAutoGUIControlModule(config=self.mock_app_config, target_window_title_pattern=self.target_window_pattern)
        mock_pyautogui.locateCenterOnScreen.side_effect = RuntimeError("Unexpected locate error")
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_unexpected):
            with self.assertRaises(PyAutoGUIControlError) as cm:
    ...
    @patch('dreamos.skills.pyautogui_control_module.Path.exists', return_value=True)
    @patch('dreamos.skills.pyautogui_control_module.Path.is_absolute')
    async def test_find_element_on_screen_uses_custom_params(self, mock_is_absolute, mock_exists):
        """UT-FIND-007: Verify correct usage of provided confidence, region, grayscale."""
        mock_is_absolute.return_value = False
        module = PyAutoGUIControlModule(config=self.mock_app_config, target_window_title_pattern=self.target_window_pattern)
        mock_point = MagicMock(x=1, y=1); mock_pyautogui.locateCenterOnScreen.return_value = mock_point
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_custom):
            await module.find_element_on_screen("test_image.png", confidence=custom_confidence, region=custom_region, grayscale=custom_grayscale)
            expected_path = module.image_assets_base_path / "test_image.png"
            mock_pyautogui.locateCenterOnScreen.assert_called_once_with(
                image=str(expected_path),
    ...
    @patch('dreamos.skills.pyautogui_control_module.Path.exists', return_value=True)
    @patch('dreamos.skills.pyautogui_control_module.Path.is_absolute', return_value=True) # Absolute path
    async def test_find_element_on_screen_absolute_path_used_directly(self, mock_is_absolute, mock_exists):
        ...
        with patch.object(module, '_run_blocking_io', side_effect=mock_run_blocking_io_abs):
