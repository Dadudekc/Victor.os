...
print("DEBUG: chronicle_conversations.py ATTEMPTING ResponseHandler IMPORT", file=sys.stderr)
try:
    ...
    print("DEBUG: chronicle_conversations.py ResponseHandler IMPORT SUCCESSFUL", file=sys.stderr)
except ImportError as e_import:
    # Fallback or error if import fails - might need path manipulation
    print(f"DEBUG: chronicle_conversations.py ResponseHandler IMPORT FAILED: {e_import}", file=sys.stderr)
    logging.error("Failed to import ResponseHandler. Ensure src directory is in PYTHONPATH or adjust import path.")
    ...
    print(f"DEBUG: chronicle_conversations.py ResponseHandler IMPORT FAILED (OTHER EXCEPTION): {e_other_import}", file=sys.stderr)
    raise
...
CURRENT_MEMORY_STATE_PLACEHOLDER = "{ \"message\": \"Memory state currently unavailable in this script.\" }"
...
def load_latest_state(conn: sqlite3.Connection) -> dict:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT state_json FROM cumulative_state ORDER BY id DESC LIMIT 1")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                state = json.loads(row[0])
                logger.info(f"Loaded latest state from database (ID: {cursor.lastrowid or 'N/A'})")
                return state
            except json.JSONDecodeError as e:
                logger.error(f"Error decoding JSON from database: {e}. Starting with empty state.")
                return {}
        else:
            logger.info("No previous state found in database. Starting with empty state.")
            return {}
    except sqlite3.Error as e:
...
def parse_response(raw_response: str) -> tuple[str | None, dict | None]:
    ...
    if match:
        ...
        try:
            ...
            logger.error(f"JSON decode error in MEMORY_UPDATE block: {e}. JSON string: {json_str}")
            narrative = raw_response.strip() # Fallback
    else:
        logger.warning("No ```json MEMORY_UPDATE``` block found in response. Treating whole response as narrative.")
...
def update_cumulative_state(current_state: dict, changes: dict | None) -> dict:
    ...
    for key, change_value in changes.items():
        ...
        elif key in ['newly_stabilized_domains', 'newly_unlocked_protocols', 'quest_completions'] and isinstance(change_value, list):
...
def run_chronicle():
    ...
    previous_narrative_summary = "No previous narrative summary available." # Initialize
    # EDIT END
    ...
    try:
        ...
        logger.info("Processing conversations in reverse chronological order (oldest first).")
        ...
        for i, convo in enumerate(conversations):
            ...
            logger.info(f"--- Processing Conversation {i+1}/{len(conversations)}: '{convo_title}' (ID: {convo_id}) --- (Oldest to Newest)")
            ...
            try:
                ...
                scraper.ensure_chat_page(target_chat_url) # Handles navigation and waits
                ...
                if current_convo_text == "<SCRAPE_ERROR>" or not current_convo_text:
                    logger.error(f"Failed to scrape content for conversation {convo_id}. Skipping prompt.")
                ...
                if len(current_convo_text) > MAX_CONVO_TEXT_CHARS:
                    ...
                    logger.warning(f"current_convo_text is too long ({len(current_convo_text)} chars). Truncating by removing first {chars_to_remove} chars.")
                    current_convo_text = "... [earlier parts of conversation truncated] ...\n\n" + current_convo_text[-MAX_CONVO_TEXT_CHARS:]
                # EDIT END

                # Prepare the prompt with the current state AND previous narrative AND current text
                state_json_for_prompt = json.dumps(current_cumulative_state, indent=2, ensure_ascii=False)
                # EDIT START: Inject all 3 context pieces into the prompt
                prompt_text = PROMPT_TEMPLATE.replace("{{ PREVIOUS_NARRATIVE_SUMMARY }}", previous_narrative_summary)
                prompt_text = prompt_text.replace("{{ CUMULATIVE_JSON_STATE }}", state_json_for_prompt)
                prompt_text = prompt_text.replace("{{ CURRENT_CONVERSATION_TEXT }}", current_convo_text)
                # EDIT END
                prompt_preview = prompt_text[:150].replace('\n', ' ') # Log a bit more for context
                logger.info(f"Sending prompt (using state, previous narrative, current text; first 150 chars): {prompt_preview}...")
                ...
                if not scraper.send_prompt(prompt_text):
                    ...
                    parsed_narrative, parsed_memory_update = None, None # Ensure variables exist
                else:
                    logger.info("Prompt sent. Waiting for stable response...")
                    response_text = scraper.wait_for_stable_response()
                    if not response_text:
                         logger.warning(f"Received empty response for conversation {convo_id}.")
                         response_text = "<EMPTY_RESPONSE>"
                         parsed_narrative, parsed_memory_update = response_text, None
                    else:
                         logger.info(f"Received response (length: {len(response_text)}). Parsing...")
                         parsed_narrative, parsed_memory_update = parse_response(response_text)
                         if parsed_narrative:
                              narrative_preview = parsed_narrative[:100].replace('\n', ' ') # Clean newline for logging
                              logger.info(f"Parsed narrative (first 100 chars): {narrative_preview}...")
                         if parsed_memory_update:
                              ...
                              current_cumulative_state = update_cumulative_state(current_cumulative_state, parsed_memory_update)
                              # Save the *new* state to DB after *each* update
                              save_cumulative_state(conn, current_cumulative_state)
                         else:
                             logger.warning("No MEMORY_UPDATE block parsed. Cumulative state not updated for this conversation.")
                ...
                output_filename = OUTPUT_DIR / f"summary_{convo_id}_{timestamp_str}.json"
                ...
                try:
                    ...
                    logger.error(f"Failed to save per-conversation summary file {output_filename}: {e_save}")

            except Exception as e_convo:
                logger.error(f"Error processing conversation {convo_id} ('{convo_title}'): {e_convo}", exc_info=True)
        ...
        logger.critical(f"An critical error occurred in the main process: {e_main}", exc_info=True)
    finally:
        # Close DB connection
        if conn:
