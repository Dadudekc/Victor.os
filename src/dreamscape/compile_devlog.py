...
logging.basicConfig(
    ...
    handlers=[
        logging.StreamHandler(sys.stdout) # Log to stdout for this script
    ]
)
...
def compile_devlog():
    ...
    if not summary_files:
        logger.warning(f"No summary_*.json files found in {SUMMARY_DIR}. Nothing to compile.")
        return
    ...
    for summary_file_path in summary_files:
        ...
        try:
            ...
            if not narrative: # Check if None or empty
                raw_response = data.get("raw_response_received", "No content available for this entry.")
                if raw_response == "<SEND_FAILED>" or raw_response == "<EMPTY_RESPONSE>":
                    logger.warning(f"Narrative missing or marked as failed for '{title}' (Status: {raw_response}). Using placeholder.")
                ...
                elif not raw_response: # Still none or empty after checking raw_response
                    ...
                    logger.info(f"Using raw_response_received as fallback narrative for '{title}'.")
            ...
            logger.error(f"Error decoding JSON from {summary_file_path.name}. Skipping.")
        except Exception as e:
            logger.error(f"Error processing file {summary_file_path.name}: {e}. Skipping.")
