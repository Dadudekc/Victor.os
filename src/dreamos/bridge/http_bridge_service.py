...
try:
    ...
    from dreamos.tools.cursor_bridge.cursor_bridge import (
        interact_with_cursor,
        CursorBridgeError,
        # Import other specific exceptions if needed (e.g., CursorInjectError)
    )
    BRIDGE_AVAILABLE = True
    logging.info("Successfully imported bridge components.")
except ImportError as e:
    logging.critical(f"Failed to import critical bridge components: {e}. HTTP Bridge Service endpoints will be unavailable.", exc_info=True)
...
if AppConfig: # Only try to load if AppConfig class was successfully imported
    try:
        ...
        logger.error(f"Failed to load AppConfig on startup: {e}. Bridge interactions will fail.", exc_info=True)
...
@app.post("/interact", 
            ...
            responses={500: {"model": ErrorResponse}, 503: {"model": ErrorResponse}, 400: {"model": ErrorResponse}})
async def handle_interaction(request: InteractRequest):
    ...
    """
    logger.debug(f"Received /interact request for prompt: {request.prompt[:50]}...") # Use debug for potentially verbose logging
    
    if not BRIDGE_AVAILABLE:
        logger.error("Cannot process /interact: Bridge components not available due to import errors.")
        raise HTTPException(status_code=503, detail="Bridge service dependencies are unavailable.") # 503 Service Unavailable

    if not CONFIG_LOADED or config is None:
        logger.error("Cannot process /interact: Application configuration is not loaded.")
        raise HTTPException(status_code=503, detail="Bridge service configuration is unavailable.") # 503 Service Unavailable

    try:
        # Call the core bridge interaction function
        result_text = interact_with_cursor(request.prompt, config)
        logger.info(f"Interaction successful. Returning response (length: {len(result_text)}). Preview: {result_text[:100]}...")
        ...
        raise HTTPException(status_code=400, detail=f"Invalid Request Data: {e}") # 400 for bad input
    except Exception as e:
        # Catch any other unexpected errors
        logger.exception(f"Unexpected error during /interact endpoint processing: {e}")
        raise HTTPException(status_code=500, detail=f"Unexpected Internal Server Error: {type(e).__name__}") # Avoid leaking detailed error message

# --- Optional: Add health check endpoint ---
@app.get("/health")
async def health_check():
    # Reflect actual state based on imports and config load
    return {"status": "ok" if BRIDGE_AVAILABLE and CONFIG_LOADED else "error", 
