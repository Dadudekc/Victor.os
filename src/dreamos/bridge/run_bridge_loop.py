...
print(
    "DEBUG: run_bridge_loop.py - About to import pyautogui", file=sys.stderr, flush=True
)
print(
    "DEBUG: run_bridge_loop.py - Successfully imported pyautogui",
    file=sys.stderr,
    flush=True,
)
...
print(
    "DEBUG: run_bridge_loop.py - About to import CursorInjector",
    file=sys.stderr,
    flush=True,
)
...
print(
    "DEBUG: run_bridge_loop.py - Successfully imported CursorInjector",
    file=sys.stderr,
    flush=True,
)
...
sys.stderr.write(
    f"DEBUG: run_bridge_loop.py PYTHONPATH ENV: {os.environ.get('PYTHONPATH')}\n"
)
sys.stderr.flush()
...
class BridgeLoop:
    def __init__(
        ...
        self.coords = {
            "chat_input_field": [
                agent_coords["input_box"]["x"],
                agent_coords["input_box"]["y"],
            ],
            "copy_button": [
                agent_coords["copy_button"]["x"],
                agent_coords["copy_button"]["y"],
            ],
        }
        ...
        self.scraper = ResponseHandler(
            timeout=response_timeout, poll_interval=poll_interval
        )
    ...
    def _default_queue_fn(self, prompt_text: str):
        ...
        logger.error(
            f"QUEUE_FN (Default): All models failed. Prompt dropped: {prompt_text[:100]}..."
        )
    ...
    def run(self) -> None:
        ...
        try:
            if not self.scraper or not self.scraper.driver:
                logger.error(
                    "Scraper or driver not initialized, cannot run bridge loop."
                )
                return

            logger.info("Ensuring login session (on chatgpt.com)...")
            if (
                not self.scraper.ensure_login_session()
            ):  # Uses updated chatgpt.com logic
                logger.error(
                    "Login session failed or was not established. Exiting bridge loop."
                )
                return
            ...
            try:
                ...
                logger.info(
                    f"Confirmed browser is on target chat page: {self.chat_url}"
                )
            except Exception as page_e:
                logger.error(
                    f"Failed to navigate/confirm target page {self.chat_url}. Exiting. Error: {page_e}",
                    exc_info=True,
                )
                return
            ...
            for prompt_text in prompts:
                ...
                logger.info(
                    f"Sending prompt directly (model selected via URL: {self.chat_url})..."
                )
                ...
                if (
                    not response or response == "<SEND_FAILED>"
                ):  # Assuming prompt_with_fallback might return <SEND_FAILED> or empty on total failure
                    logger.warning(
                        "No response received or send failed for this prompt."
                    )
            ...
            logger.error(
                f"Exception during BridgeLoop.run execution: {e}", exc_info=True
            )
    ...
    def _persist_response(self, prompt: str, resp: Optional[str]) -> None:
        ...
        outfile.write_text(
            json.dumps({"prompt": prompt, "response": resp, "timestamp": ts}, indent=2),
            encoding="utf-8",
        )
...
def cli() -> None:
    ...
    try:
        ...
        parser.add_argument(
            "--agent-id", type=int, required=True, help="Agent numeric ID"
        )
        parser.add_argument(
            "--prompt-file",
            type=Path,
            required=True,
            help="Path to a .txt prompt file OR directory of .txt prompts",
        )
        parser.add_argument(
            "--coords",
            type=Path,
            default=Path("runtime/config/cursor_agent_coords.json"),
            help="JSON file with screen‑coords for Cursor UI elements",
        )
        parser.add_argument(
            "--window-title",
            default=GuiAutomationConfig().target_window_title,
            help="Substring of Cursor window title to activate",
        )
        parser.add_argument(
            "--outbox",
            type=Path,
            default=Path("runtime/bridge_outbox"),
            help="Folder to write response JSONs",
        )
        parser.add_argument(
            "--response-timeout",
            type=int,
            default=180,
            help="Seconds to wait for stable response",
        )
        parser.add_argument(
            "--chat-url",
            type=str,
            default="https://chat.openai.com/chat",
            help="Base URL for chat interaction",
        )
        ...
        logger.critical(
            f"FATAL ERROR in run_bridge_loop.py cli(): {type(e_cli).__name__} - {e_cli}",
            exc_info=True,
        )
        sys.exit(1)
