...
from dreamos.core.config import AppConfig, load_config, CoreConfigurationError
from dreamos.core.tasks.nexus.task_nexus import TaskNexus # Assuming TaskNexus can be imported
...
# --- Scraper Log Import ---
MODULE_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'runtime/modules/chatgpt_scraper'))
...
try:
    ...
    print(f"Error: Could not import log_interaction from {MODULE_PATH}/scraper.py - {e}")
...
if log_interaction is None:
    ...
    def log_interaction(prompt, response, tags=None):
        print("CRITICAL: Fallback logger active. Cannot log interaction to scraper log due to import error.")
        return False, None
...
def call_gpt_api(prompt: str) -> str:
    ...
    status_logger.info(f"Attempting to call ChatGPTWebAgent for prompt: '{prompt[:50]}...'")

    if not chatgpt_agent_instance:
        status_logger.error("CRITICAL: chatgpt_agent_instance is not initialized. Bridge cannot function.")
        raise ConnectionError("Critical: chatgpt_agent_instance is not available. Check main_loop initialization.")
    ...
    try:
        ...
        response = loop.run_until_complete(chatgpt_agent_instance.process_external_prompt(prompt))

        if response is None:
            status_logger.error("ChatGPTWebAgent returned None, indicating an error or critical failure during processing.")
            raise ConnectionError("Failed to get valid response from ChatGPTWebAgent (returned None).")
        ...
        status_logger.error(f"Error calling ChatGPTWebAgent's process_external_prompt: {e}", exc_info=True)
        if chatgpt_agent_instance and chatgpt_agent_instance.driver:
            status_logger.info("Attempting to close ChatGPTWebAgent browser due to error...")
            try:
                chatgpt_agent_instance.close()
                status_logger.info("ChatGPTWebAgent browser closed.")
            except Exception as close_e:
                status_logger.error(f"Error closing ChatGPTWebAgent browser: {close_e}", exc_info=True)
        raise
...
def write_bridge_output(prompt_id, log_id, gpt_response):
    ...
    try:
        ...
        with open(BRIDGE_OUTPUT_FILE, 'a', encoding='utf-8') as f:
            ...
            f.write('\n')
        status_logger.info(f"Successfully wrote response for prompt {prompt_id} to {BRIDGE_OUTPUT_FILE}")
        return True
    except Exception as e:
        status_logger.error(f"Error writing bridge output to {BRIDGE_OUTPUT_FILE}: {e}", exc_info=True)
        return False
...
def log_failure_trace(prompt_id, prompt_text, error_details):
    ...
    try:
        ...
        with open(FAILURE_LOG_FILE, 'a', encoding='utf-8') as f:
            ...
            f.write('\n')
        status_logger.warning(f"Logged failure trace for prompt {prompt_id} to {FAILURE_LOG_FILE}")
    except Exception as e:
        status_logger.error(f"CRITICAL: Failed to log failure trace: {e}", exc_info=True)
...
def relay_prompt_to_gpt(prompt: str, prompt_id: str = "unknown") -> bool:
    ...
    try:
        ...
        logged_ok, log_id = log_interaction(prompt, gpt_response, tags=["bridge_loop", f"prompt_{prompt_id}"])
        if not logged_ok:
            status_logger.warning(f"Failed to log interaction {prompt_id} to scraper log.")
        ...
        if not write_ok:
            raise IOError(f"Failed to write response for {prompt_id} to {BRIDGE_OUTPUT_FILE}")
            
        return True # Full success
        
    except Exception as e:
        status_logger.error(f"Error during relay for prompt {prompt_id}: {e}", exc_info=True)
...
def main_loop():
    ...
    try:
        status_logger.info(f"Attempting to load bridge configuration from: {BRIDGE_CONFIG_FILE}")
        app_cfg_for_agent = load_config(config_path=BRIDGE_CONFIG_FILE) # Use the load_config function
        status_logger.info(f"Successfully loaded AppConfig via load_config for bridge.")
        ...
        # TaskNexus setup
        nexus_task_file_str = app_cfg_for_agent.paths.task_list_file_for_bridge # Assuming this path is set in AppConfig via bridge_config.yaml
        if not nexus_task_file_str:
            raise CoreConfigurationError("Path 'task_list_file_for_bridge' not found in loaded AppConfig paths.")
        ...
        status_logger.info(f"TaskNexus initialized for bridge agent using: {nexus_task_file}")
        ...
        try:
            with open(BRIDGE_CONFIG_FILE, 'r') as f_direct_cfg:
                 ...
                 agent_settings = direct_bridge_cfg_data.get('chatgpt_web_agent_settings')
                 if not agent_settings:
                     raise CoreConfigurationError("'chatgpt_web_agent_settings' section missing in bridge_config.yaml")
        except Exception as e_direct_load:
             status_logger.error(f"Failed to directly load chatgpt_web_agent_settings from {BRIDGE_CONFIG_FILE}: {e_direct_load}. Using defaults or potentially failing.")
             # Set defaults or raise - raising is safer
             raise CoreConfigurationError(f"Could not extract chatgpt_web_agent_settings from config: {e_direct_load}")
        ...
        if not conversation_url:
             raise CoreConfigurationError("'conversation_url' missing in chatgpt_web_agent_settings within bridge_config.yaml")
        ...
        try:
            delay_val = getattr(app_cfg_for_agent.agents.chatgpt_web, 'external_prompt_delay')
            status_logger.info(f"Agent external_prompt_delay configured via AppConfig: {delay_val}")
        except AttributeError:
             status_logger.warning("Could not verify external_prompt_delay in AppConfig structure. Agent might use default.")
        ...
        status_logger.info(f"ChatGPTWebAgent instance '{agent_id}' initialized successfully for bridge.")

    except CoreConfigurationError as e_core_cfg:
         status_logger.critical(f"Core configuration error during bridge initialization: {e_core_cfg}. Bridge cannot function.", exc_info=True)
         return # Exit main_loop
    except Exception as e:
        status_logger.critical(f"Unexpected error during bridge initialization: {e}. Bridge cannot function.", exc_info=True)
        return # Exit main_loop
    ...
    while True: # Run indefinitely until stopped externally
        ...
        if is_suspended:
            if os.path.exists(RESUME_FLAG):
                status_logger.warning(f"Resume flag found ({RESUME_FLAG}). Resuming operation.")
                ...
                if current_mtime > last_prompt_mtime:
                     status_logger.warning("Input file changed while suspended. Resuming operation.")
                     last_prompt_mtime = current_mtime
                     is_suspended = False
                     consecutive_failures = 0
                     cycles_since_last_change = 0 # Reset stall counter
                else:
                    status_logger.info("Suspended. Checking for resume flag or input file change next cycle.")
        ...
        if current_mtime == last_prompt_mtime:
            ...
            status_logger.info(f"Input file unchanged. Cycles since last change: {cycles_since_last_change}")
            if cycles_since_last_change >= STALL_THRESHOLD_CYCLES:
                status_logger.warning(f"Input file ({CURSOR_PROMPT_FILE}) stalled for {cycles_since_last_change} cycles. Creating alert flag.")
                # Create or update the alert flag file
                try:
                    with open(STALL_ALERT_FLAG, 'w') as f:
                        f.write(f"Stalled since: {datetime.datetime.now(datetime.timezone.utc).isoformat()}\n")
                except Exception as e:
            ...
            if os.path.exists(STALL_ALERT_FLAG):
                ...
                except Exception as e: status_logger.error(f"Failed to remove stall alert flag: {e}")
        ...
        try:
                ...
                with open(CURSOR_PROMPT_FILE, 'r', encoding='utf-8') as f:
                    for line in f:
                        try:
                            ...
                            prompt_id = data.get('prompt_id', f'missing_id_{uuid.uuid4()}') # Ensure unique ID
                            ...
                            if prompt_id not in processed_ids and prompt_text:
                                ...
                                if success:
                                    ...
                                    status_logger.warning(f"Relay failed for prompt {prompt_id}. Consecutive failures: {consecutive_failures}")
                                    if consecutive_failures >= CONSECUTIVE_FAILURE_THRESHOLD:
                                        status_logger.error(f"Reached consecutive failure threshold ({CONSECUTIVE_FAILURE_THRESHOLD}). Suspending loop.")
                                        is_suspended = True
                                        # Log final failure context if needed, maybe the last error from relay?
                                        break # Stop processing further prompts this cycle
                                        
                            elif not prompt_text:
                                status_logger.warning(f"Skipping entry with missing prompt text: {line.strip()}") 
                                
                        except json.JSONDecodeError as e:
                            status_logger.error(f"Skipping invalid JSON line: {line.strip()} - Error: {e}")
                        except Exception as e:
                            status_logger.error(f"Error processing line: {line.strip()} - Error: {e}", exc_info=True)
            ...
            status_logger.error(f"Error reading or processing {CURSOR_PROMPT_FILE}: {e}", exc_info=True)
        ...
        status_logger.info(f"--- Poll Cycle End. Duration: {cycle_duration:.2f}s. Sleeping for {sleep_time:.2f}s ---")
