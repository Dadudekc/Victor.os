...
class CursorState:
    ...
    def add_to_history(self, entry: Dict[str, Any]) -> None:
        ...
        entry["timestamp"] = get_utc_iso_timestamp()
    ...
    def record_command(self, command: str) -> None:
        ...
        self.add_to_history({"type": "command", "command": command})
    ...
    def get_file_context(self, file_path: str) -> Dict[str, Any]:
        ...
        return {
            "path": file_path,
            "history": [
                entry for entry in self.history if entry.get("file") == file_path
            ],
            "context": {
                k: v for k, v in self.context.items() if k.startswith(file_path)
            },
        }
    ...
    def _save_state(self) -> None:
        ...
        state = {
            ...
            "updated_at": get_utc_iso_timestamp(),
        }
...
class ChatContext:
    ...
    def add_message(
        ...
        self.cursor_state.add_to_history({"type": "chat", "message": message})
        log_event("chat", f"Added {role} message", {"content": content[:100]})

    def get_context(
        self, num_messages: Optional[int] = None, include_system: bool = True
    ) -> List[Dict[str, Any]]:
