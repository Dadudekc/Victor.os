...
from ..schemas.thea_response_schema import ResponseStatus, ResponseType, TheaResponse
...
class BridgeSimulator:
    ...
    def _generate_response(self, prompt: str, task_id: str) -> TheaResponse:
        ...
        # Generate next steps
        next_steps = (
            ["Simulated next step 1", "Simulated next step 2"]
            if response_type == ResponseType.TASK_COMPLETE
            else None
        )
    ...
    async def simulate_prompt_processing(self, prompt_data: Dict):
        ...
        response = self._generate_response(
            prompt_data["prompt"], prompt_data["task_id"]
        )

        # Process response
        self.response_handler.process_response(
            response.to_dict(), prompt_data["task_id"]
        )
    ...
    async def run_simulation(self, num_prompts: int = 10):
        ...
        # Process prompts
        tasks = [self.simulate_prompt_processing(prompt) for prompt in prompts]
        await asyncio.gather(*tasks)

        # Start response relay
        relay_task = asyncio.create_task(asyncio.to_thread(self.response_relay.run))
