...
class CursorShadowController:
    ...
    def monitor_output(self, prompt_id: str, timeout=60, poll_interval=1) -> Dict:
                ...
                try:
                    ...
                    with open(result_file, "r", encoding="utf-8") as f:
                        ...
                        if not content.strip():
                            logger.warning(
                                f"Result file {result_file} is empty. Retrying read."
                            )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to remove result file {result_file}: {del_e}"
                        )
                    logger.info(
                        f"Successfully processed result for prompt {prompt_id}."
                    )
                    ...
                    logger.error(
                        f"Failed to read or process result file {result_file}: {e}",
                        exc_info=True,
                    )
                    return {
                        "success": False,
                        "error": f"Failed to read/process result: {e}",
                    }
        ...
        return {
            "success": False,
            "error": f"Timeout waiting for Cursor response for prompt {prompt_id}",
        }

    def run_prompt_cycle(
        self, prompt: str, context: Optional[Dict] = None, timeout=60
    ) -> Dict:
        """
        ...
        if isinstance(result, dict):
            ...
            result = {
                "success": False,
                "error": "Invalid result format from monitor",
                "prompt_id": prompt_id,
            }
...
if __name__ == "__main__":
    ...
    controller = CursorShadowController(
        prompt_dir=str(test_inbox), output_dir=str(test_outbox)
    )
    ...
    try:
        ...
        assert (
            not simulated_result_file.exists()
        ), "Test failed: Result file was not cleaned up."
