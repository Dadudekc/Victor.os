import asyncio
# import hashlib # No longer needed directly
import logging
...
# MODIFIED IMPORT: Import FileLock from the new location
from dreamos.utils.file_locking import (
    FileLock,
    LockAcquisitionError,
    LockDirectoryError,
)

# Import file_io for hashing
from dreamos.utils import file_io

# Type checking imports
if TYPE_CHECKING:
    # MODIFIED IMPORT PATH (though only used for type checking)
    from dreamos.core.config import AppConfig  # Keep core for AppConfig itself
...
async def affirm_onboarding_contract(
    ...
    try:
        # Uses file_io.calculate_file_sha256 now
        protocol_hash = await asyncio.to_thread(file_io.calculate_file_sha256, onboarding_protocols_path)
        
        if not protocol_hash:
            logger.error(
                f\"Error: Could not calculate SHA256 hash for {onboarding_protocols_path} using file_io.calculate_file_sha256.\"
            )
            return False
        ...
        logger.error(
            f\"Error: Onboarding protocols file not found at {onboarding_protocols_path} (when attempting hash)\"
        )
        return False
    except IOError as e:
        logger.error(
            f\"Error reading onboarding protocols file {onboarding_protocols_path} (when attempting hash): {e}\"
        )
        return False
    except Exception as e:
        logger.error(
            f\"Unexpected error calculating hash for {onboarding_protocols_path}: {e}\", exc_info=True
        )
        return False
