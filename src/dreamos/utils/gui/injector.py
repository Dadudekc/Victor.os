...
class CursorInjector:
    ...
    def __init__(
        ...
        self.window_title = window_title if window_title else f"{DEFAULT_WINDOW_TITLE}" # Simplified for now
        ...
        if not self.all_coords:
            self.log.error(f"Failed to load coordinates for any agent from {self.coords_file}")
        ...
        if not self.input_coords:
             self.log.warning(f"Could not find/load coordinates for '{self.agent_id}' input_box_initial.")
        ...
        try:
            if self.coords_file.exists():
                with open(self.coords_file, 'r') as f:
                    data = json.load(f)
                    self.log.info(f"Successfully loaded coordinates from {self.coords_file}")
                    return data
            else:
    ...
    def _get_specific_agent_coords(self, element_key_suffix: str) -> Optional[Tuple[int, int]]:
        ...
        elif isinstance(coords, dict) and 'x' in coords and 'y' in coords: # Support for dict format e.g. {"x": 1, "y": 2}
    ...
    def _pause(self, duration: Optional[float] = None) -> None:
        time.sleep(duration if duration is not None else random.uniform(self.min_pause, self.max_pause))

    def focus_window(self) -> bool:
        """Brings the target agent's window to the foreground."""
        if not self.focus_check_available:
            self.log.debug("Focus check skipped: pygetwindow not available.")
            return True # Assume success or rely on user to ensure focus

        self.log.debug(f"Attempting to focus window for {self.agent_id} (Title: '{self.window_title}')")
        try:
            # Attempt to find a window that starts with the agent_id (e.g. "Agent-1")
            # This is more robust if multiple "Cursor" windows are open.
            # The calibration script implies distinct windows per agent.
            agent_specific_title_prefix = self.agent_id 
            
            candidate_windows = []
            # First, try a very specific title if one was configured (e.g. "Agent-1 - Cursor GUI")
            if self.window_title and self.window_title != DEFAULT_WINDOW_TITLE:
                 candidate_windows = pygetwindow.getWindowsWithTitle(self.window_title)

            if not candidate_windows: # Fallback to more general search
                all_windows = pygetwindow.getAllWindows()
                for win in all_windows:
                    # Check if the window title contains the agent_id AND "Cursor"
                    # This is a heuristic, might need refinement based on actual window titles
                    if agent_specific_title_prefix in win.title and "Cursor" in win.title:
                         candidate_windows.append(win)
                    elif self.window_title and self.window_title in win.title: # Check configured title
                         candidate_windows.append(win)


            if not candidate_windows:
                 # Last resort: try the default "Cursor" title if no agent-specific one found
                if self.window_title == DEFAULT_WINDOW_TITLE or not self.window_title:
                    candidate_windows = pygetwindow.getWindowsWithTitle(DEFAULT_WINDOW_TITLE)
            
            if not candidate_windows:
                self.log.warning(f"No window found for agent '{self.agent_id}' with title cues: specific='{self.window_title}', prefix='{agent_specific_title_prefix}', default='{DEFAULT_WINDOW_TITLE}'.")
                return False

            # Assuming the first match is the correct one.
            # If multiple windows match, this might select the wrong one.
            # TODO: Add more sophisticated window selection if needed.
            win = candidate_windows[0]
            self.log.debug(f"Found candidate window: '{win.title}' for agent '{self.agent_id}'.")

            if win.isMinimized:
                win.restore()
            
            # On some systems, activate might not be enough or might need a delay
            # For robustness, try to bring it to front and then activate
            if hasattr(win, 'activate'):
                win.activate()
            elif hasattr(win, 'set_foreground'): # Some libraries/OS might use this
                 win.set_foreground()
            else: # Basic bring to front
                win.minimize()
                win.maximize() # Or win.restore()

            self._pause(0.2) # Allow time for focus to shift

            # Verify focus (optional, as pygetwindow.getActiveWindow() can be unreliable)
            if self.focus_verify_enabled:
                active_window = pygetwindow.getActiveWindow()
                if active_window and ( (self.window_title and self.window_title in active_window.title) or (agent_specific_title_prefix in active_window.title) ):
                    self.log.info(f"Successfully focused window: '{active_window.title}' for {self.agent_id}.")
                    return True
                else:
                    self.log.warning(
                        f"Focus verification failed for {self.agent_id}. Expected something like '{self.window_title}' or containing '{agent_specific_title_prefix}', "
                        f"but active window is '{(active_window.title if active_window else 'None')}'. Proceeding anyway."
                    )
                    return True # Proceeding as focus is hard to verify reliably across platforms
            return True # If verification disabled or passed
        except Exception as e:
            self.log.error(f"Error focusing window for {self.agent_id} ('{self.window_title}'): {e}", exc_info=True)
            return False
    ...
    def inject(self, prompt: str) -> bool:
        ...
        self.log.info(f"Attempting to inject prompt for {self.agent_id}: '{prompt[:50]}...'")

        if not self.input_coords:
            self.log.error(f"Cannot inject prompt: Input coordinates not loaded/found for {self.agent_id}.")
            return False

        if not self.focus_window():
            self.log.warning(f"Failed to focus window for {self.agent_id}. Injection may fail or go to wrong window.")
        ...
        try:
            ...
            self.log.error(f"Error during prompt injection for {self.agent_id}: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    if injector.input_coords:
        log.info(f"Injector created for {test_agent_id} with input_coords: {injector.input_coords}")
        ...
        log.info("Please ensure a GUI window for Agent-1 (or a window with 'Cursor' in title) is open and visible.")
        ...
        log.error(f"Could not initialize injector properly for {test_agent_id} due to missing coordinates. Test aborted.")
        log.info(f"Ensure '{test_agent_id}.input_box_initial' key exists in {coords_file_path}")

    log.info(f"Script finished. To test other agents, modify 'test_agent_id'.") 