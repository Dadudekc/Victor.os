import logging  # EDIT: Add logging import
import os
...
# AZURE_STORAGE_CONNECTION_STRING = os.getenv("AZURE_STORAGE_CONNECTION_STRING") # REMOVED
...
def get_blob_channel(config: AppConfig):
    ...
    if use_local or not azure_config:
        ...
        if not azure_config and not use_local:
            logger.warning(
                "Warning: Azure Blob config not found, falling back to LocalBlobChannel."
            )  # Use logging?
        else:
        ...
        try:
            ...
            # TODO: Add support for other credential types if needed (e.g., DefaultAzureCredential)
            elif account_url:  # Maybe using DefaultAzureCredential?
                logger.warning(
                    "account_url provided without explicit credential (SAS/connection string). Assuming DefaultAzureCredential is configured."
                )
                ...
                raise ValueError(
                    "Azure Blob config requires connection_string OR account_url (with optional sas_token)."
                )

            logger.info(f"Info: Using AzureBlobChannel (Container: {container_name})")
            return AzureBlobChannel(
                # Pass client directly if constructor supports it, else pass config values
                # Assuming constructor is updated as below:
                container_name=container_name,
                connection_string=conn_str,  # Pass original config values
                sas_token=sas_token,
                account_url=account_url,
                # blob_service_client=blob_service_client, # Alternative if constructor takes client
            )
            # EDIT END
        except Exception as e:
            # Missing credentials or other errors -> fallback to local
            logger.warning(
                f"Warning: Failed to initialize AzureBlobChannel ({e}), falling back to LocalBlobChannel."
            )
            local_path = getattr(
                config.paths, "memory_blobs_local_root", "./local_blob_storage"
            )
