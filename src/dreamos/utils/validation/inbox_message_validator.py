...
DEFAULT_SCHEMA_PATH = Path(__file__).resolve().parents[3] / "runtime" / "governance" / "protocols" / "inbox_message_schema_v1.json"
...
class MessageValidator:
    ...
    def _load_schema(self) -> Dict[str, Any]:
        ...
        if not self.schema_path.exists():
            ...
            if alt_path.exists():
                ...
                print(f"ERROR: Schema file not found at {self.schema_path} or {alt_path}")
                return None
        ...
        try:
            ...
            if "$schema" not in schema_data or "properties" not in schema_data:
                print(f"ERROR: {self.schema_path} does not appear to be a valid JSON Schema.")
                return None
            return schema_data
        except json.JSONDecodeError as e:
            print(f"ERROR: Could not decode JSON from schema file {self.schema_path}: {e}")
            return None
        except Exception as e:
            print(f"ERROR: An unexpected error occurred while loading schema {self.schema_path}: {e}")
            return None
    ...
    def validate_message(self, message_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if not validate or not ValidationError or not SchemaError:
            return False, ["jsonschema library is not installed. Validation cannot be performed."]
        
        if not self.schema:
            return False, [f"Schema not loaded from {self.schema_path}. Cannot validate."]
        ...
        try:
            ...
            return False, [f"Schema Error: The schema file {self.schema_path} is invalid. {e.message}"]
        except Exception as e:
