...
try:
    ...
    logger.info(
        "spec_parser.py: Successfully imported commonmark and commonmark_extensions.tables."
    )
except ImportError as e:
    _commonmark_available = False
    logger.warning(
        f"spec_parser.py: Failed to import commonmark or commonmark_extensions.tables. Parser functionality will be disabled. Error: {e}"
    )
...
class ProjectPlanTableVisitor(
    commonmark.NodeVisitor if _commonmark_available else NodeVisitor
):
    ...
    def __init__(self):
        if not _commonmark_available:
            logger.error(
                "commonmarkextensions not installed. Cannot parse project plan tables."
            )
        ...
        self._current_expected_headers = (
            None  # Store expected headers for current table
        )
    ...
    def visit_Table(self, node):
        ...
        logger.debug(
            f"Table headers (raw): {headers}, (normalized): {normalized_headers}"
        )
        ...
        # Add elif blocks for other expected legacy tables here
            ...
            logger.debug(
                f"Skipping table with unrecognized headers: {normalized_headers}"
            )
        ...
        table_body = next(
            (
                ...
                if isinstance(child, commonmark.nodes.TableBody)
            ),
            None,
        )
        if table_body:
            ...
            for child_row in table_body.children:
                if isinstance(child_row, commonmark.nodes.TableRow):
                    self.visit_TableRow(
                        child_row
                    )  # Explicitly call on the TableRow in TableBody
    ...
    def visit_TableRow(self, node):
        ...
        if (
            not _commonmark_available
            or self._current_table_type is None
            or self._current_expected_headers is None
        ):
        ...
        logger.debug(
            f"Processing row with {len(row_cells_content)} cells for table type {self._current_table_type}"
        )

        task_dict = self._map_row_to_headers(
            row_cells_content, self._current_expected_headers
        )
        if task_dict:
            ...
            if self._current_table_type in self.extracted_data:
                ...
                logger.debug(
                    f"Appended task: {task_dict} to {self._current_table_type}"
                )
            else:
                logger.warning(
                    f"Table type '{self._current_table_type}' not initialized in extracted_data."
                )
    ...
    def _extract_headers(self, table_node):
        ...
        head = next(
            (
                ...
                if isinstance(child, commonmark.nodes.TableHead)
            ),
            None,
        )
        if head:
            header_row = next(
                (
                    ...
                    if isinstance(child, commonmark.nodes.TableRow)
                ),
                None,
            )
            if header_row:
    ...
    def _extract_row_cells(self, row_node):
        ...
        cells_content = []
        if not _commonmark_available or not isinstance(
            row_node, commonmark.nodes.TableRow
        ):
        ...
        for cell_node in row_node.children:
            if isinstance(cell_node, commonmark.nodes.TableCell):
                cells_content.append(
                    self._extract_cell_content(cell_node)
                )  # Keep raw content for now, strip in mapping
        return cells_content
    ...
    def _extract_cell_content(self, cell_node):
        ...
        for sub_node in cell_node.walker():  # Use walker to get all descendants
            ...
            if isinstance(current_node, commonmark.nodes.Text):
                ...
                isinstance(current_node, commonmark.nodes.HtmlInline)
                and current_node.literal.lower().strip() == "<br>"
            ):
                text_parts.append("\n")  # Treat <br> as newline
    ...
    def _map_row_to_headers(self, row_cell_contents, headers_list_normalized):
        ...
        if num_cells_in_row > num_headers:
            logger.warning(
                f"Row has more cells ({num_cells_in_row}) than headers ({num_headers}). Ignoring extra cells."
            )
...
def parse_project_plan_tasks(file_path: str = "specs/PROJECT_PLAN.md") -> dict | None:
    ...
    if not _commonmark_available:
        logger.error(
            "Cannot parse project plan: commonmarkextensions library not found."
        )
        return None
    ...
    try:
        ...
        logger.info(
            f"Parsed project plan. Found {len(visitor.extracted_data['main_tasks'])} main tasks."
        )
        ...
        logger.exception(
            f"Error parsing project plan {file_path}: {e}"
        )  # Log traceback
        return None
