...
class BusCorrelationValidator:
        ...
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    ...
                    logger.warning(
                        "BusCorrelationValidator already initialized by another thread."
                    )
        else:
    ...
    def get_instance(cls) -> "BusCorrelationValidator":
        ...
        if cls._instance is None:
            ...
            raise RuntimeError(
                "BusCorrelationValidator must be configured using configure() before accessing the instance."
            )
        return cls._instance
    ...
    def validate_event_sequence(
        ...
        expected_event_order: Optional[
            List[Any]
        ] = None,  # New parameter for event order
    ) -> bool:
        ...
        if not events:
            logger.debug(
                "validate_event_sequence called with empty list, returning True."
            )
        ...
        if sequence_correlation_id is not None:
            ...
            if first_event_id is None:
                self.log_issue(
                    ...
                    event_details={
                        "first_event_id": getattr(events[0], "event_id", "N/A")
                    },
                )
                return False
            target_id = first_event_id
            logger.debug(
                f"Validating sequence against inferred target ID: {target_id} from event {getattr(events[0], 'event_id', 'N/A')}"
            )
        ...
        for i, event in enumerate(events):
            ...
            if not self.validate_event(event, context_correlation_id=target_id):
                ...
                logger.warning(
                    f"Event index {i} (ID: {getattr(event, 'event_id', 'N/A')}) failed validation within sequence {target_id}."
                )
                all_valid = False
                ...
                if not all(ot in found_origin_types for ot in expected_origin_types):
                    self.log_issue(
                        ...
                        event_details={
                            ...
                            "expected_types": expected_origin_types,
                            "found_types": list(found_origin_types),
                        },
                        level=logging.WARNING,
                    )
                    all_valid = False
            elif (
                not found_origin_types
            ):  # require_all_origin_types is False, so any one is enough
                self.log_issue(
        ...
        if expected_terminal_types:
            ...
            if require_all_terminal_types:
                if not all(
                    tt in found_terminal_types for tt in expected_terminal_types
                ):
                    self.log_issue(
                        ...
                        event_details={
                            "correlation_id": target_id,
                            "expected_types": expected_terminal_types,
                            "found_types": list(found_terminal_types),
                        },
                        level=logging.WARNING,
                    )
                    all_valid = False
            elif (
                not found_terminal_types
            ):  # require_all_terminal_types is False, so any one is enough
                self.log_issue(
        ...
        if expected_event_order:
            if len(events) < len(expected_event_order):
                ...
                if (
                    all_valid
                    and len(events) > len(expected_event_order)
                    and expected_event_order
                ):  # only if order check passed so far and order was specified
        ...
        if not all_valid:
            logger.warning(
                f"Sequence validation failed for correlation ID: {target_id}"
            )
    ...
    def log_issue(
        ...
        corr_id = (
            event_details.get(
                "correlation_id", event_details.get("event_correlation_id", "N/A")
            )
            if event_details
            else "N/A"
        )
        ...
        logger.log(
            level,
            f"{log_entry} [EventID: {ev_id}, CorrID: {corr_id}, Source: {source_id}] Details: {details_summary}",
        )
    ...
    def reset_issues(self):
        ...
        with self._lock:  # <<< Restored lock
            ...
            self.issues_log = []
            logger.info(
                f"BusCorrelationValidator issue log reset. Cleared {count} issues."
            )  # <<< Restored access within lock
