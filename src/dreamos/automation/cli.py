...
def log_violation(message: str) -> None:
    ...
    if len(ENV_FAILURES) >= MAX_FAILURES:
        logger.critical("⚠️ Multiple environment failures detected! Escalating to THEA...")
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description='DreamOS Episode Execution CLI'
    )
    parser.add_argument(
        'episode_path',
        type=str,
        help='Path to the episode YAML file'
    )
    parser.add_argument(
        '--debug',
        action='store_true',
