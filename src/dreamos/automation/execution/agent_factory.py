...
from dreamos.core.config import AppConfig, AgentActivationConfig
...
class AgentFactory:
    ...
    def create_active_agents(self) -> Dict[str, BaseAgent]:
        ...
        if not hasattr(self.config, 'swarm') or not hasattr(self.config.swarm, 'active_agents'):
            logger.warning("AppConfig has no swarm.active_agents configured. No agents loaded by factory.")
            return created_agents

        active_agent_configs: list[AgentActivationConfig] = self.config.swarm.active_agents

        if not active_agent_configs:
            logger.warning("AppConfig.swarm.active_agents list is empty. No agents loaded by factory.")
            return created_agents

        logger.info(f"Attempting to load {len(active_agent_configs)} configured active agents...")

        for agent_config in active_agent_configs:
            agent_id = None
            try:
                logger.debug(f"Processing agent config: Module={agent_config.agent_module}, Class={agent_config.agent_class}, Pattern={agent_config.worker_id_pattern}, Override={agent_config.agent_id_override}")
                ...
                if agent_config.agent_id_override:
                    ...
                    logger.warning(f"No agent_id_override for {agent_config.agent_class}, using class name '{agent_id}' as ID. Ensure uniqueness if multiple instances are intended.")

                if agent_id in created_agents:
                    logger.error(f"Duplicate agent ID detected: '{agent_id}'. Skipping configuration for {agent_config.agent_module}.{agent_config.agent_class}. Check agent_id_override in config.")
                    continue
                ...
                try:
                    ...
                    raise AgentFactoryError(f"Failed to import agent module '{module_path}': {e}") from e

                # Get the class from the module
                try:
                    AgentClass = getattr(agent_module, agent_config.agent_class)
                    logger.debug(f"Successfully found class '{agent_config.agent_class}' in module {module_path}")
                except AttributeError as e:
                    raise AgentFactoryError(f"Class '{agent_config.agent_class}' not found in module '{module_path}': {e}") from e

                # Ensure it's a subclass of BaseAgent (optional but good practice)
                if not issubclass(AgentClass, BaseAgent):
                    raise AgentFactoryError(f"Class '{AgentClass.__name__}' is not a subclass of BaseAgent.")
                ...
                logger.info(f"Instantiating agent: ID='{agent_id}', Class='{AgentClass.__name__}'")
                ...
                logger.error(f"Failed to create agent from config {agent_config}: {afe}")
                # Optionally continue to try loading other agents
            except Exception as e:
                logger.error(f"Unexpected error creating agent (ID: {agent_id or 'unknown'}) from config {agent_config}: {e}", exc_info=True)
                # Optionally continue

        logger.info(f"AgentFactory finished loading. Created {len(created_agents)} agents.")
        return created_agents 