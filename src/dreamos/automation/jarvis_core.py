...
class JarvisCore:
    ...
    def __init__(self, config_path: Optional[str] = None, config_dict: Optional[Dict[str, Any]] = None):
        ...
        """
        try:
            self.config = AppConfig.load(config_path) if config_path else AppConfig.load()
    ...
    def _get_memory_path(self) -> Path:
        ...
        try:
            if isinstance(self.config, dict) and "jarvis" in self.config:
                ...
                elif isinstance(self.config.jarvis, dict) and "memory_path" in self.config.jarvis:
                    memory_path = Path(self.config.jarvis["memory_path"])
                    logger.info(f"Using config dict memory path: {memory_path}")
        except Exception as e:
        ...
        try:
            with self.lock:
                ...
                if len(self.context_window["recent_interactions"]) > 10:
                    self.context_window["recent_interactions"] = \
                        self.context_window["recent_interactions"][-10:]
    ...
    def execute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            with self.lock:
                ...
                # Log task execution
                logger.info(f"Executing task: {task.get('id', 'unknown')} - {task.get('description', 'No description')}")
                
                # Execute task based on type
                task_type = task.get("type", "unknown")
                result = {"task_id": task.get("id"), "status": "error", "message": "Unknown task type"}
                ...
                for i, active_task in enumerate(self.context_window["active_tasks"]):
                    if active_task.get("id") == task.get("id"):
                        self.context_window["active_tasks"][i]["status"] = result.get("status")
                        self.context_window["active_tasks"][i]["result"] = result
                        break
                
                # Clean up completed tasks
                self.context_window["active_tasks"] = [
                    t for t in self.context_window["active_tasks"] 
    ...
    def _generate_response(self, intent: str, entities: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if intent == "request_help":
            response["content"] = "I'm JARVIS, your AI assistant. I can help with tasks, answer questions, and coordinate with other agents."
        elif intent == "status_request":
            system_state = self._get_system_state()
            response["content"] = f"I'm operational. System status: {system_state['status']}. {len(self.context_window.get('active_tasks', []))} active tasks."
    ...
    def _execute_system_command(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"status": "failed", "message": "System commands not implemented for security reasons"}
    ...
    def _coordinate_with_agent(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            inbox_path = Path(f"runtime/agent_comms/agent_mailboxes/{agent_id}/inbox.json")
            ...
            return {"status": "failed", "message": f"Agent coordination error: {str(e)}"}
