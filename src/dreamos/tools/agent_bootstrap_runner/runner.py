...
class AgentBootstrapRunner:
        ...
        for message_path in self.config.inbox_dir.glob("*.json"):
            try:
                ...
                if message.recipient_agent_id != self.config.agent_id:
                    log.warning(f"Found message for {message.recipient_agent_id} in {self.config.agent_id}'s inbox")
                    continue
    ...
    async def send_response(self, original_message: MessageFormat, response_body: str) -> bool:
        """
