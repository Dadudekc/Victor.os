...
class CursorAgentMessenger:
    ...
    async def send_message(self, message: str) -> bool:
        ...
        """
        try:
            self.logger.info(f"Sending message to {self.config.agent_id}: {message[:50]}...")
    ...
    async def get_response(self, timeout_sec: float = 30.0) -> Optional[str]:
        ...
        try:
            ...
            while (datetime.now() - start_time).total_seconds() < timeout_sec:
                ...
                if response:
                    self.logger.info(f"Got response from {self.config.agent_id}: {response[:50]}...")
                    return response
                await asyncio.sleep(2)
                
            self.logger.warning(f"No response from {self.config.agent_id} after {timeout_sec}s")
            return None
    ...
    async def send_and_get_response(self, message: str, timeout_sec: float = 30.0) -> Optional[str]:
        """
