...
async def onboard_agent(agent_id: str, delay_sec: int = 5) -> bool:
    ...
    try:
        ...
        config = AgentConfig(
            agent_id=agent_id,
            startup_delay_sec=0  # No startup delay for onboarding
        )
                ...
                if directory.exists():
                    if directory.is_file():
                        ...
                        if directory.stat().st_size > 0:
                            ...
                            backup_file = backup_dir / f"{directory.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        # Load agent charter
        charter_file = Path("runtime/governance/onboarding") / f"AGENT_CHARTER_{agent_id.split('-')[1]}.md"
        if charter_file.exists():
            charter = charter_file.read_text()
        else:
            charter = f"# {agent_id} Charter\n\nRole: {AGENT_CHARTERS[agent_id]}"
