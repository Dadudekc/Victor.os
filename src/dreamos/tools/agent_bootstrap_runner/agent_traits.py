...
from .config import AGENT_TRAITS, AGENT_CHARTERS
    ...
    def _load_custom_traits(self, traits_file: Path) -> None:
        ...
        try:
            ...
            if not isinstance(config, dict):
                raise TraitsConfigError("Invalid traits file format - must be a JSON object")
            ...
            if "validation_policy" in agent_config:
                if agent_config["validation_policy"] not in ["strict", "lenient"]:
                    raise TraitsConfigError("Validation policy must be 'strict' or 'lenient'")
