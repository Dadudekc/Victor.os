...
def validate_directory_structure(logger: logging.Logger, config: 'AgentConfig') -> ValidationResult:
    """
...
def validate_message_format(message_data: Dict[str, Any]) -> ValidationResult:
    ...
    for field in ["sender_agent_id", "recipient_agent_id"]:
        ...
        if not agent_id.startswith("Agent-") or not agent_id[-1].isdigit():
            return ValidationResult(
                passed=False,
                error=f"Invalid agent ID format in {field}: {agent_id}"
            )
...
def validate_message_file(message_path: Path) -> ValidationResult:
    ...
    try:
        ...
        if message_path.suffix != ".json":
            return ValidationResult(
                passed=False,
                error=f"Invalid message file extension: {message_path}"
            )
        ...
        return ValidationResult(
            passed=False,
            error=f"Invalid JSON format in message file: {message_path}"
        )
    except Exception as e:
        return ValidationResult(
            passed=False,
            error=f"Error validating message file {message_path}: {e}"
        )

def validate_all_files(logger: logging.Logger, config: 'AgentConfig', is_onboarding: bool = False) -> ValidationResult:
    """
    ...
    if not config.coords_file.exists():
        return ValidationResult(
            passed=False,
            error=f"Missing coordinates file: {config.coords_file}"
        )
    ...
    try:
        with config.coords_file.open("r", encoding="utf-8") as f:
            ...
            if config.agent_id not in coords:
                return ValidationResult(
                    passed=False,
                    error=f"Missing coordinates for {config.agent_id}"
                )
    except Exception as e:
        return ValidationResult(
            passed=False,
            error=f"Error validating coordinates file: {e}"
        )
    ...
    try:
        ...
        return ValidationResult(
            passed=False,
            error=f"Error validating copy coordinates file: {e}"
        )
