...
)  # src/dreamos/tools/agent2_bootstrap_runner -> Dream.os
...
MAINTENANCE_BACKUP_RETENTION_DAYS = int(
    os.getenv("AGENT_MAINTENANCE_BACKUP_RETENTION_DAYS", "30")
)
...
class AgentConfig:
    ...
    def __init__(
        ...
        self.startup_delay_sec = (
            startup_delay_sec if startup_delay_sec is not None else STARTUP_DELAY_SEC
        )
        ...
        self.base_runtime = (
            base_runtime
            or PROJECT_ROOT / "runtime/agent_comms/agent_mailboxes" / agent_id
        )
        ...
        self.traits = AGENT_TRAITS.get(
            agent_id, "Versatile, Adaptive, Reliable, Focused"
        )
