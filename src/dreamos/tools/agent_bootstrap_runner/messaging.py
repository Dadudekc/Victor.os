...
async def publish_event(bus: AgentBus, logger: logging.Logger, agent_id: str, event_type: str, data: Dict[str, Any]):
...
def archive_inbox(logger: logging.Logger, config: AgentConfig) -> bool:
    ...
    if not config.inbox_file.exists():
        return False
    try:
        dest = config.archive_dir / f"inbox.{int(datetime.now(timezone.utc).timestamp())}.json"
    ...
    try:
        with config.inbox_file.open(encoding="utf-8") as fh:
            ...
            if isinstance(data, dict):
                logger.info(f"Loaded inbox message {data.get('prompt_id') or data.get('id')}")
                return [data]
...
def create_seed_inbox(logger: logging.Logger, config: 'AgentConfig', custom_prompt: Optional[str] = None) -> bool:
    """
    ...
    try:
        message_path = config.inbox_dir / f"seed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def update_inbox_with_prompt(logger: logging.Logger, config: AgentConfig, custom_prompt: str) -> bool:
    """
    ...
    if config.inbox_file.exists():
        try:
            ...
            logger.info(f"Updated existing inbox with custom prompt @ {config.inbox_file}")
            success = True
        except Exception as e:
...
def archive_inbox_file(logger: logging.Logger, config: AgentConfig, file_path: Path) -> bool:
    """
...
def update_agent_state(logger: logging.Logger, config: AgentConfig, 
                      cycle_count: int, current_task: str = None) -> bool:
    """
