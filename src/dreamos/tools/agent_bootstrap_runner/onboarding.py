...
class AgentOnboardingManager:
    ...
    async def onboard(self) -> bool:
        ...
        try:
            ...
            validation = validate_all_files(self.logger, self.config, is_onboarding=True)
            ...
            try:
                ...
                if response:
                    ...
                    self.logger.error(f"No response received from {self.config.agent_id}")
                    return False
                    
            except Exception as e:
                self.logger.error(f"Failed to interact with {self.config.agent_id}: {e}")
                return False
