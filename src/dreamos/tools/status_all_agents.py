...
class AgentStatus:
    ...
    def __init__(self, agent_id: str, state_data: Optional[Dict] = None):
        ...
        self.start_time = (
            ...
            if state_data and "start_time" in state_data
            else None
        )
        self.restart_count = state_data.get("restart_count", 0) if state_data else 0
        self.last_restart = (
            datetime.fromisoformat(state_data["last_restart"])
            if state_data and "last_restart" in state_data
            else None
        )
        self.log_file = (
            Path(state_data["log_file"])
            if state_data and "log_file" in state_data
            else None
        )
    ...
    def get_uptime(self) -> Optional[float]:
        ...
        if not self.start_time:
            return None
        return (
            datetime.now(timezone.utc) - self.start_time.replace(tzinfo=timezone.utc)
        ).total_seconds()
...
def update_metrics(agent_id: str, status: str, uptime: Optional[float]):
    ...
    if status == "crashed" and (
        not agent_metrics["last_crash"]
        or datetime.fromisoformat(agent_metrics["last_crash"])
...
def show_status(watch: bool = False, interval: int = 5):
    ...
    def display_status():
        ...
        if not state:
            ...
            print(
                "\033[96mpython src/dreamos/tools/launch_all_agents.py\033[0m"
            )  # Cyan command
            return
