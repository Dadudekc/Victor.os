...
DEFAULT_IGNORE = [".git", ".venv", "venv", "__pycache__", "node_modules", ".pytest_cache", "htmlcov", "_archive", "build", "dist", ".log", ".jsonl", ".json", ".lock", ".bak", ".tmp"]

def find_todos_in_file(file_path: Path, patterns: list[str], base_dir: Path) -> list[dict]:
    """Scans a single file for lines matching the specified patterns."""
    findings = []
    pattern_regex = re.compile(r"#\s*({})[:\s]?\s*(.*?)$".format("|".join(patterns)), re.IGNORECASE)
        ...
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line_num, line in enumerate(f, 1):
                ...
                if match:
                    ...
                    relative_path = str(file_path.relative_to(base_dir)).replace("\\", "/") # Normalize path separators
...
def scan_directory(directory: Path, patterns: list[str], log_file: Path, ignore_list: list[str], extensions: list[str]):
    ...
    for root, dirs, files in os.walk(directory, topdown=True):
        ...
        for file in files:
            ...
            if findings:
                logger.info(f"Found {len(findings)} item(s) in {file_path.relative_to(directory)}")
                for finding in findings:
                    write_log_entry(log_file, finding)
                total_findings += len(findings)

    logger.info(f"Scan complete. Scanned {scanned_files} files. Found {total_findings} total items.")

def main():
    parser = argparse.ArgumentParser(description="Scan directories for TODO/FIXME/BUG comments and log them.")
    parser.add_argument(
        "--dir", 
        type=Path, 
        required=True, 
