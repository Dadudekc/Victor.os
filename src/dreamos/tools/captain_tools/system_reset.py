    ...
    def reset_agent_mailbox(self, agent_id: str) -> bool:
        ...
        try:
            ...
            with open(devlog_file, 'w') as f:
                f.write(f"# Agent Development Log\n\n")
...
def send_to_agent(coords: Dict, agent_id: str, message: str, dry_run: bool = False) -> bool:
...
def reset_agent(agent_id: str, dry_run: bool = False) -> bool:
    ...
    if not dry_run:
        logger.info(f"⏳ Waiting {INITIAL_DELAY} seconds before starting supervisor loop for {agent_id}...")
        time.sleep(INITIAL_DELAY)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reset agent mailboxes to a clean state")
    parser.add_argument("--agents", nargs="+", help="Specific agent IDs to reset (e.g., Agent-1 Agent-2)")
