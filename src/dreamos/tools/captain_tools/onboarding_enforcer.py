...
def save_to_thea_outbox(agent_id: str, response: str, tag: str = "pending_onboarding") -> None:
    try:
...
def enforce_onboarding(agent_ids: List[str], dry_run: bool = False) -> int:
    ...
    for agent_id in targets:
        ...
        if response:
            ...
            send_prompt(agent_id, coords, "Resume autonomy — self-prompt loop triggered.", dry_run)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agents", nargs="*", help="Agent IDs (e.g., Agent-1 Agent-2). Defaults to all agents.")
    parser.add_argument("--dry-run", action="store_true", help="Simulate without clicking/pasting")
