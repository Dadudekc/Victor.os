...
def print_env_status(strict: bool = False, verbose: bool = False) -> int:
    all_passed, python_version, pythonpath_status, results, system_info = get_env_status()
    ...
    console.print(f"Python: {system_info['python']['version']} ({system_info['python']['implementation']})")
    ...
    if "disk" in system_info and "error" not in system_info["disk"]:
        ...
        console.print(f"Disk Space: {disk['free_gb']}GB free of {disk['total_gb']}GB ({disk['free_percent']}%)")
...
def generate_md_report() -> str:
    ...
    md.append(f"- **Python:** {system_info['python']['version']} ({system_info['python']['implementation']})")
    md.append(f"- **Processor:** {system_info['hardware']['processor']}")
    
    if "disk" in system_info and "error" not in system_info["disk"]:
        disk = system_info["disk"]
        md.append(f"- **Disk Space:** {disk['free_gb']}GB free of {disk['total_gb']}GB ({disk['free_percent']}%)")
    ...
    md.append(f"- **PYTHONPATH includes 'src':** {'✅' if pythonpath_status else '❌'}\n")
    ...
    # Status Summary
    status = "GREEN" if all_passed and pythonpath_status else ("WARNING" if all_passed else "FAIL")
...
def main():
    ...
    parser.add_argument('--strict', action='store_true', help='Exit with error if any checks fail')
