...
class _MockConfigPaths:
    def __init__(self):
        ...
        # EDIT END
        self.thea_responses_dir = self.project_root / "runtime/mock_thea_responses_standalone"
        self.agent_comms = self.project_root / "runtime/mock_agent_comms_standalone/agent_mailboxes"
...
class _StandaloneDummyBaseAgent:
    def __init__(self, agent_id="TheaRelayAgent_Dummy", log_level=logging.INFO, config=None, agent_bus=None, **kwargs):
        self.agent_id = agent_id
        self.logger = logging.getLogger(agent_id) # Use self.logger consistently
        self.logger.setLevel(log_level)
        self.config = config if config is not None else get_config() # Ensure config is present
        ...
        self.logger.info(f"_StandaloneDummyBaseAgent '{self.agent_id}' initialized with config: {type(self.config).__name__}.")

    async def run(self):
        self.logger.error(f"_StandaloneDummyBaseAgent '{self.agent_id}' run() called - it's a placeholder and TheaRelayAgent should implement its own run logic.")
...
async def _dummy_async_write_mailbox_message(path: Path, content: Dict[str, Any]):
    ...
    if not await asyncio.to_thread(parent_dir.exists):
        logger.info(f"[DUMMY WRITE] Parent directory {parent_dir.resolve()} does not exist. Creating.")
        try:
            await asyncio.to_thread(parent_dir.mkdir, parents=True, exist_ok=True)
            logger.info(f"[DUMMY WRITE] Created parent directory {parent_dir.resolve()}")
        except Exception as e_mkdir:
            logger.error(f"[DUMMY WRITE] Error creating directory {parent_dir.resolve()}: {e_mkdir}", exc_info=True)
    ...
    try:
        ...
        logger.error(f"[DUMMY WRITE] Error writing dummy file {path.resolve()}: {e_write}", exc_info=True)
        raise
...
if __name__ == "__main__":
    print("[TheaRelayAgent INFO] Running in STANDALONE mode. Using dummy/mock implementations for DreamOS core components.")
    logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s') # Setup logging early for standalone
    ...
    print("[TheaRelayAgent INFO] Running in INTEGRATED mode. Attempting to import real DreamOS core components.")
    try:
        ...
        from dreamos.core.coordination import agent_bus as real_agent_bus_module # For AgentBus type
        ...
        print("[TheaRelayAgent INFO] Successfully imported REAL BaseAgent, write_mailbox_message, and agent_bus.AgentBus type.")
    except ImportError as e:
        print(f"[TheaRelayAgent CRITICAL] Failed to import real DreamOS components in INTEGRATED mode: {e}. This agent will likely NOT function correctly.", exc_info=True)
        # Fallback to prevent immediate crash, but this is a fatal error for integration.
        _BaseAgent_cls = _StandaloneDummyBaseAgent
        _write_mailbox_message_fn = _dummy_async_write_mailbox_message
        _AgentBusType = Any
        print("[TheaRelayAgent WARNING] INTEGRATED mode fell back to dummy components due to import error. THIS IS UNEXPECTED AND LIKELY AN ISSUE.")
...
class TheaRelayAgent(_BaseAgent_cls): # Inherits from conditionally chosen BaseAgent
    ...
    def __init__(
        ...
        agent_bus_instance: Optional[_AgentBusType] = None, # Use conditional AgentBusType
    ):
        loaded_config = get_config() # Always use our (mock) get_config for this script's execution
        ...
        self.response_dir = getattr(loaded_config.paths, 'thea_responses_dir', RESPONSE_DIR)
        self.archive_dir = self.response_dir / "archive"
        self.error_dir = self.response_dir / "error"
        self.mailbox_root_dir = getattr(loaded_config.paths, 'agent_comms', MAILBOX_ROOT_DIR)
        
        poll_interval_default = POLLING_INTERVAL_SECONDS
        if hasattr(loaded_config, 'agent_settings') and hasattr(loaded_config.agent_settings, 'thea_relay_poll_interval'):
            poll_interval_default = loaded_config.agent_settings.thea_relay_poll_interval
        self.polling_interval = poll_interval_default

        # _ensure_dirs is async, call in async setup or run loop
        self.logger.info(f"TheaRelayAgent '{self.agent_id}' initialized (using {type(self).__bases__[0].__name__}).")
    ...
    async def _ensure_dirs(self):
        ...
        setup_logger.info(f"Ensuring directories exist: {[str(d.resolve()) for d in dirs_to_create]}")
        all_dirs_ensured = True
        try:
            for d in dirs_to_create:
                # MODIFIED: Use file_io.ensure_directory
                if not await asyncio.to_thread(file_io.ensure_directory, d):
                    setup_logger.error(f"Failed to ensure directory {d.resolve()} using file_io.ensure_directory.")
                    all_dirs_ensured = False # Mark as failed but continue trying others
            
            if all_dirs_ensured:
                setup_logger.info("Monitored directories ensured successfully using file_io.")
            else:
                setup_logger.error("One or more directories could not be ensured. Check previous errors.")
        ...
        except Exception as e: # Catch any unexpected errors from asyncio.to_thread or loop itself
            setup_logger.error(
                f"Unexpected critical error during _ensure_dirs loop: {e}", exc_info=True
            )
    ...
    async def _load_thea_response(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        self.logger.debug(f"Loading response from: {file_path.name} (full path: {file_path.resolve()})")
    ...
    def _validate_response(
        ...
        if not response:
            self.logger.warning(f"Response data is empty for {filename}. Skipping validation.")
            return False
        try:
    ...
    async def _get_mailbox_path(
        ...
        if not re.match(r"^Agent-\\d+$", agent_id): # Ensure regex is correctly escaped for string
            self.logger.warning(
        ...
        try:
            if not await asyncio.to_thread(agent_mailbox_dir.exists):
                self.logger.info(f"Mailbox directory {agent_mailbox_dir.resolve()} does not exist. Creating.")
                await asyncio.to_thread(
                    agent_mailbox_dir.mkdir, parents=True, exist_ok=True
                )
                self.logger.info(f"Created mailbox directory {agent_mailbox_dir.resolve()}.")
        except Exception as e:
            self.logger.error(
    ...
    async def _dispatch_message(self, response: Dict[str, Any], filename: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to write mailbox message to {msg_path.resolve()}: {e}", exc_info=True
            )
            return False
    ...
    async def _move_file(self, file_path: Path, target_dir: Path):
        ...
        """
        self.logger.info(f"Attempting to move {file_path.resolve()} to {target_dir.resolve()} using file_io.move_file")
        ...
        if moved_path:
            self.logger.info(f"Moved {file_path.name} to {target_dir.name} directory ({moved_path.resolve()}).")
        else:
    ...
    async def _process_files(self):
        ...
        if not await asyncio.to_thread(self.response_dir.exists):
            self.logger.error(f"Response directory {self.response_dir.resolve()} does not exist. Cannot process files. Check _ensure_dirs.")
        ...
        if files_found > 0:
            self.logger.info(f"Found {files_found} potential response file(s) in {self.response_dir.resolve()}.")
        ...
        for file_path in files_to_process:
            ...
            if file_path.parent.resolve() != self.response_dir.resolve():
                 self.logger.warning(f"Skipping file not directly in response_dir: {file_path.resolve()} (parent: {file_path.parent.resolve()})")
                 continue


            self.logger.info(f"Processing file: {file_path.name} (from {file_path.resolve()})")
        ...
        if files_found > 0: # Log summary only if files were initially found
            ...
            self.logger.debug(f"No .json files found in {self.response_dir.resolve()} this cycle.")


    async def run(self):
        """Main execution loop for the TheaRelayAgent."""
        self.logger.info(f"TheaRelayAgent '{self.agent_id}' starting run loop (Base class: {type(self).__bases__[0].__name__}).")
        try:
            await self._ensure_dirs()  # Ensure directories are created before starting loop
        except Exception as e_ensure_dirs:
            self.logger.critical(f"Failed during initial directory setup: {e_ensure_dirs}. Agent cannot run.", exc_info=True)
...
if __name__ == "__main__":
    ...
    async def main_standalone_runner():
        relay_agent = TheaRelayAgent(agent_id="TestTheaRelayStandalone", agent_bus_instance=None)
    ...
    try:
        ...
        logging.getLogger("TheaRelayAgentMain").info("TheaRelayAgent standalone test stopped by user.")
