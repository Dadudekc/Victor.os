...
from typing import Optional, Any, Dict # Added Any, Dict for BusMessage payload

from dreamos.core.config import AppConfig # Assuming AppConfig can be imported
from dreamos.core.config import get_config # Corrected import path for get_config
...
try:
    ...
    logger.warning("Failed to import MessageType from enums. Assuming placeholder or string usage.")
...
from pydantic import BaseModel # Import BaseModel for BusMessage
from typing import Optional, Any, Dict # Added Any, Dict for BusMessage payload
...
def handle_gpt_payload(payload: dict, config: Optional[AppConfig] = None):
    ...
    if not content_type or not isinstance(content, str):
        raise ValueError(f"Invalid payload format. Requires 'type' and 'content': {payload}")
    ...
    try:
        publish_bridge_status("INJECTING", {"payload_type": content_type, "content_length": len(prepared_content)})
...
def find_and_focus_cursor_window(config: Optional[AppConfig] = None):
    ...
    try:
        ...
        push_telemetry({"event": "focus_error", "timestamp": datetime.now(timezone.utc).isoformat(), "error_details": str(e)}, config)
        # EDIT END
        logger.error(
    ...
    try:
        ...
        if gui_snippets_dir:
             ...
             if input_field_image_path.is_file():
                 try:
                     logger.debug(f"Attempting to locate input field using image: {input_field_image_path}")
                     # Confidence can be adjusted
                     location = pyautogui.locateCenterOnScreen(str(input_field_image_path), confidence=0.8)
                     if location:
                         input_field_coords = (location.x, location.y)
                         logger.info(f"Located input field via image at: {input_field_coords}")
                     else:
                     ...
                     logger.warning(f"Error locating image {input_field_image_path}: {img_e}")
             else:
                 logger.warning(f"Input field image template not found at: {input_field_image_path}")
        ...
        if input_field_coords is None:
            ...
            if conf_x is None or conf_y is None:
                logger.error("Cursor input field coordinates (input_coord_x, input_coord_y) are not configured in AppConfig under 'tools.cursor_bridge' and image template failed or was not found. Cannot proceed with injection.")
                raise CursorInjectError("Input field coordinates not configured and image location failed.")
            
            input_field_coords = (conf_x, conf_y)
            logger.warning(f"Input field image not found or failed. Using configured input coordinates: {input_field_coords}")
        ...
        push_telemetry({"event": "inject_success", "timestamp": datetime.now(timezone.utc).isoformat(), "details": f"Injected prompt length: {len(prompt)}"}, config)
        # EDIT END
        ...
        # EDIT START: Add Telemetry Push on Failure
        push_telemetry({"event": "inject_error", "timestamp": datetime.now(timezone.utc).isoformat(), "error_details": f"PyAutoGUI error: {e}"}, config)
        # EDIT END
        ...
        # EDIT START: Add Telemetry Push on Failure
        push_telemetry({"event": "inject_error", "timestamp": datetime.now(timezone.utc).isoformat(), "error_details": f"Unexpected error: {e}"}, config)
        # EDIT END
...
def configure_response_area(region: tuple[int, int, int, int]):
    ...
    logger.warning("configure_response_area is deprecated. Set 'tools.cursor_bridge.response_area_region' in config.")
    # if (
...
def capture_response_area(config: Optional[AppConfig] = None, region_override: Optional[tuple[int, int, int, int]] = None) -> Image.Image | None:
    ...
    if not region_to_use:
        ...
        if not region_to_use:
            logger.error("Response area region is not configured ('tools.cursor_bridge.response_area_region') and not found via image.")
            raise CursorExtractError("Response area region not configured or located.")
        if not (isinstance(region_to_use, (list, tuple)) and len(region_to_use) == 4 and all(isinstance(x, int) for x in region_to_use)):
            logger.error(f"Invalid response_area_region format in config: {region_to_use}. Expected [left, top, width, height].")
            raise CursorExtractError("Invalid response_area_region format.")
    
    logger.debug(f"Capturing screenshot of region: {region_to_use}")
    try:
        screenshot = pyautogui.screenshot(region=tuple(region_to_use)) # Ensure it's a tuple
        return screenshot
    except pyautogui.PyAutoGUIException as e:
        # EDIT START: Add Telemetry Push on Failure
        push_telemetry({"event": "capture_error", "timestamp": datetime.now(timezone.utc).isoformat(), "error_details": f"PyAutoGUI error: {e}"}, config)
        # EDIT END
        ...
        # EDIT START: Add Telemetry Push on Failure
        push_telemetry({"event": "capture_error", "timestamp": datetime.now(timezone.utc).isoformat(), "error_details": f"Unexpected error: {e}"}, config)
        # EDIT END
...
def extract_text_from_image(image: Image.Image, config: Optional[AppConfig] = None) -> str:
    ...
    if tesseract_cmd_path_str: # If the path is configured
        ...
        if tesseract_cmd_path_obj.is_file():
            logger.info(f"Using custom Tesseract executable path: {tesseract_cmd_path_obj}")
            ...
            logger.error(f"Configured Tesseract executable path 'tools.cursor_bridge.tesseract_cmd_path': \"{tesseract_cmd_path_str}\" is invalid or not a file. Pytesseract will attempt to use its default (system PATH), which may fail if Tesseract is not installed correctly there.")
            # Not setting pytesseract.tesseract_cmd here means it uses its internal default.
    else:
        logger.debug("No custom Tesseract path configured ('tools.cursor_bridge.tesseract_cmd_path'). Pytesseract will use its default (system PATH).")
    ...
    try:
        ...
        # EDIT START: Add Telemetry Push on Failure
        push_telemetry({"event": "ocr_error", "timestamp": datetime.now(timezone.utc).isoformat(), "error_details": str(e)}, config)
        # EDIT END
...
def monitor_and_extract_response(
    ...
    logger.info(f"Monitoring response area for stable text (Timeout: {timeout_seconds}s, Stability: {stability_threshold}s)")
    ...
    if gui_snippets_dir:
        ...
        if not response_area_image_path.is_file():
            logger.warning(f"Response area image template not found: {response_area_image_path}")
            response_area_image_path = None # Fallback to configured region if image missing

    try:
        publish_bridge_status("EXTRACTING", {"timeout_config_s": timeout_seconds, "stability_config_s": stability_threshold})
        while time.monotonic() - start_time < timeout_seconds:
            try:
                current_region = _get_bridge_config("response_area_region", None, config)
                if response_area_image_path:
                    try:
                        location = pyautogui.locateOnScreen(str(response_area_image_path), confidence=0.8)
                        if location:
                            # Use located region (left, top, width, height)
                            current_region = (location.left, location.top, location.width, location.height)
                            logger.debug(f"Located response area via image at: {current_region}")
                        else:
                            logger.warning("Response area image not found on screen, falling back to configured region.")
                            # Fallthrough to use configured region if image not found
                    except pyautogui.PyAutoGUIException as img_e:
                        logger.warning(f"PyAutoGUI error locating response area image: {img_e}, falling back to configured region.")
                    except Exception as img_e:
                         logger.warning(f"Error locating response area image {response_area_image_path}: {img_e}, falling back to configured region.")
                
                current_image = capture_response_area(config, region_override=current_region) # Pass region override
                ...
                if current_image_hash != last_image_hash:
                    logger.debug(f"Image hash changed. Old: {last_image_hash[:8]}, New: {current_image_hash[:8]}")
                    current_text = extract_text_from_image(current_image, config).strip()
                    if current_text != last_text:
                        logger.debug(f"Text changed (Length: {len(current_text)}). Resetting stability timer.")
                        last_text = current_text
                        last_change_time = time.monotonic()
                    else:
                        logger.debug("Image changed but extracted text is identical. Treating as stable.")
                        # Consider image changed but text didn't as potentially stable
                        pass 
                    last_image_hash = current_image_hash # Update hash only if image really changed
                else:
                ...
                if time.monotonic() - last_change_time >= stability_threshold:
                    if last_text:
                        ...
                        logger.info(f"Response stabilized after {stability_duration:.2f} seconds.")
                        # EDIT START: Add Telemetry Push on Success
                        push_telemetry({"event": "extract_success", "timestamp": datetime.now(timezone.utc).isoformat(), "stability_duration_s": stability_duration, "response_length": len(last_text)}, config)
                        # EDIT END
                        publish_bridge_status("IDLE", {"reason": "Extraction successful"})
                        return last_text
                    else:
                ...
                logger.warning(f"Extraction error during monitoring: {e}. Retry {extract_error_count}/{max_extract_retries}...")
                if extract_error_count >= max_extract_retries:
                     logger.error(f"Maximum extraction retries ({max_extract_retries}) reached. Aborting.")
                     # EDIT START: Add Telemetry Push on Failure (Max Retries)
                     push_telemetry({"event": "extract_error_retries", "timestamp": datetime.now(timezone.utc).isoformat(), "error_details": f"Max retries ({max_extract_retries}) reached: {e}"}, config)
                     # EDIT END
                     publish_bridge_status("ERROR", {"operation": "extract", "error": f"Max retries ({max_extract_retries}) reached: {e}"})
                     raise CursorExtractError(f"Max extraction retries reached: {e}") from e
                # Fallthrough to time.sleep if retry limit not reached
            except Exception as e:
                # EDIT START: Add Telemetry Push on Failure
                push_telemetry({"event": "extract_error_unexpected", "timestamp": datetime.now(timezone.utc).isoformat(), "error_details": f"Unexpected monitoring loop error: {e}"}, config)
                # EDIT END
                logger.error(f"Unexpected error during monitoring loop: {e}", exc_info=True)
                # Decide whether to raise or continue polling
                publish_bridge_status("ERROR", {"operation": "extract_unexpected", "error": str(e)})
        ...
        logger.error(f"Timeout ({timeout_seconds}s) waiting for stable response. Actual duration: {timeout_value:.2f}s")
        # EDIT START: Add Telemetry Push on Failure (Timeout)
        push_telemetry({"event": "extract_timeout", "timestamp": datetime.now(timezone.utc).isoformat(), "timeout_config_s": timeout_seconds, "actual_duration_s": timeout_value}, config)
        # EDIT END
        publish_bridge_status("ERROR", {"operation": "extract", "error": "Timeout occurred but exception not raised properly?"})
    ...
    publish_bridge_status("ERROR", {"operation": "extract", "error": "Timeout occurred but exception not raised properly?"})
    raise CursorExtractError("Timeout waiting for stable response.") # Ensure exception is raised
...
def summarize_cursor_output(raw_text: str, max_length: int = 500) -> str:
    ...
    logger.info(f"Summarized output (length: {len(summary)} chars). Original length: {len(raw_text)} chars.")
    return summary
...
def interact_with_cursor(prompt: str, config: Optional[AppConfig] = None) -> str:
    ...
    try:
        ...
        summarized_response = summarize_cursor_output(raw_response, max_length=summary_max_len)
        # EDIT END
        ...
        logger.critical(f"Critical unexpected error during interaction: {e}", exc_info=True)
...
def parse_bridge_log(log_line: str) -> Optional[dict]:
    ...
    injection_success_match = re.search(r"INFO: Prompt injected and Enter pressed.", log_line)
    if injection_success_match:
        ...
        return {"event": "inject_success", "timestamp": timestamp, "details": "Prompt injection confirmed."} 

    extraction_success_match = re.search(r"INFO: Response stabilized after (\d+\.\d+) seconds.", log_line)
    if extraction_success_match:
        ts_match = re.match(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3})", log_line)
        timestamp = ts_match.group(1) if ts_match else "Unknown"
        stability_time = float(extraction_success_match.group(1))
        return {"event": "extract_success", "timestamp": timestamp, "stability_duration_s": stability_time}
    
    error_match = re.search(r"ERROR: (Cursor interaction failed|Timeout waiting for stable response|.+? error during .+?: .+)", log_line)
    if error_match:
        ts_match = re.match(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3})", log_line)
        timestamp = ts_match.group(1) if ts_match else "Unknown"
        error_details = error_match.group(1).strip()
        event_type = "extract_timeout" if "Timeout" in error_details else "bridge_error"
        return {"event": event_type, "timestamp": timestamp, "error_details": error_details}
...
def push_telemetry(telemetry_data: dict, config: Optional[AppConfig] = None):
    ...
    try:
        ...
        configured_path_str = _get_bridge_config("telemetry_log_file", str(default_log_path), config)
        ...
        if not telemetry_log_path.is_absolute():
             ...
             try:
                 ...
                 logger.debug(f"Resolved relative telemetry path to: {telemetry_log_path}")
             except Exception:
                 logger.warning("Could not resolve relative telemetry path relative to runtime. Using path as is.")

    except Exception as config_e:
        logger.error(f"Error getting telemetry_log_file path from config: {config_e}. Using default: {default_log_path}")
    ...
    try:
        ...
        logger.info(f"Pushed telemetry event: {telemetry_data.get('event')} to {telemetry_log_path}")
    except Exception as e:
...
def run_demo_scenario(task: str, config: Optional[AppConfig] = None):
    ...
    try:
        ...
        extract_response = _get_bridge_config("demo_extract_response", False, config) # Add config option
        if extract_response:
             ...
             logger.info(f"[DEMO] Extracted Cursor response (length: {len(cursor_response)}):\n{cursor_response[:200]}...") # Log truncated response
        else:
...
def validate_full_cycle(task: str, config: Optional[AppConfig] = None):
    ...
    try:
        ...
        logger.info(f"Full cycle validation successful. Final Summary (first 100 chars): {final_result[:100]}")
        ...
        logger.error(f"VALIDATION FAILED: Unexpected error during cycle: {e}", exc_info=True)
    ...
    logger.info(f"=== Full Cycle Validation Complete. Status: {validation_log_entry['status']} ===")
    return error is None # Return True if successful, False otherwise


# --- Updated RELAY FUNCTION with Telemetry ---
def relay_prompt_via_web_and_gui(prompt: str, config: Optional[AppConfig] = None, run_id: str = "standalone_run"):
    """
    ...
    try:
        ...
        CHATGPT_SCRAPER_AVAILABLE = True
        logger.info("Successfully imported ChatGPTScraper for relay_prompt_via_web_and_gui.")
    except ImportError as e:
        logger.error(f"Failed to import ChatGPTScraper for relay_prompt_via_web_and_gui: {e}. Web relay functionality disabled.")
        log_telemetry(run_id, "scraper_init", "ERROR", error_message=f"ChatGPTScraper import failed: {e}")
    ...
    if not CHATGPT_SCRAPER_AVAILABLE or ChatGPTScraper_class is None: # Check ChatGPTScraper_class
        error_msg = "ChatGPTScraper class is not available. Cannot perform web relay."
        log_telemetry(run_id, "scraper_availability_check", "FAIL", error_message=error_msg)
    ...
    if config is None:
        try:
             ...
             log_telemetry(run_id, "config_load_attempt", "FAIL", error_message=error_msg)
             print(f"[BRIDGE_RELAY {run_id}] FAIL - {error_msg}", file=sys.stderr)
    ...
    try:
        ...
        cookie_file_path_str = get_config("tools.chatgpt_scraper.cookie_file", "runtime/config/chatgpt_cookies.json", config_obj=config)
        ...
        if config is None:
            logger.warning("AppConfig not provided to relay_prompt_via_web_and_gui, ChatGPTScraper might not initialize correctly.")
            ...
            raise CursorBridgeError("AppConfig is required for ChatGPTScraper initialization in relay_prompt_via_web_and_gui.")

        scraper_instance = ChatGPTScraper_class(config=config, headless=True) # Pass config
        ...
        log_telemetry(run_id, "scraper_send_prompt", "STARTED", {"prompt_length": len(prompt)})
        print(f"[BRIDGE_RELAY {run_id}] Sending prompt to ChatGPT via web scraper: '{prompt[:50]}...'")
        ...
        print(f"[BRIDGE_RELAY {run_id}] Received response from ChatGPTScraper (...): '{chatgpt_response[:50]}...'")

        # --- Step 3: Inject the *response* into Cursor GUI (Requires GUI Lock) ---
        log_telemetry(run_id, "gui_injection", "START", {"response_length": len(chatgpt_response)})
        print(f"[BRIDGE_RELAY {run_id}] Attempting to acquire GUI lock for injection...")
        with gui_interaction_lock:
             print(f"[BRIDGE_RELAY {run_id}] GUI lock acquired. Injecting scraped response into Cursor GUI...")
             ...
             print(f"[BRIDGE_RELAY {run_id}] Response injected into Cursor GUI. Releasing lock.")
        ...
        print(f"[BRIDGE_RELAY {run_id}] SUCCESS - Successfully relayed prompt via web and injected response into GUI.")
...
def log_telemetry(run_id: str, phase_id: str, status: str, metadata: Optional[dict] = None, error_message: Optional[str] = None):
    ...
    try:
        ...
        print(f"Failed Telemetry Data: {log_entry if 'log_entry' in locals() else 'N/A'}", file=sys.stderr)

# --- Updated Main execution block with Telemetry ---
if __name__ == "__main__":
    main_run_id = f"test_run_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    print(f"--- Running Cursor Bridge Web Relay Test (ID: {main_run_id}) (with Config Bypass) ---")
    ...
    try:
        ...
        logger.warning(f"Could not determine project root via relative path, using CWD: {PROJECT_ROOT}")
    ...
    minimal_config = {
        "paths": {
            ...
            "runtime": Path(os.getenv("DREAMOS_RUNTIME_PATH", PROJECT_ROOT / "runtime")).resolve(),
            "chatgpt_cookie_file": (PROJECT_ROOT / "runtime" / "config" / "chatgpt_cookies.json").resolve()
        },
        "tools": {
            "cursor_bridge": {
    ...
    class ConfigWrapper:
        def __init__(self, config_dict):
            ...
            self.tools = SimpleNamespace(**{k: SimpleNamespace(**v) if isinstance(v, dict) else v for k, v in tools_config.items()})
            self.chatgpt_scraper = SimpleNamespace(**config_dict.get('chatgpt_scraper', {}))
    ...
    try:
        ...
        test_config_obj.paths.chatgpt_cookie_file = minimal_config["paths"]["chatgpt_cookie_file"]
    ...
    try:
        ...
        print(f"Attempting relay_prompt_via_web_and_gui (ID: {main_run_id}) (using config bypass)...")
        ...
        summary_meta = {"steps_executed": steps_executed, "success": (final_status == "SUCCESS")}
        log_telemetry(main_run_id, "test_end", final_status, metadata=summary_meta, error_message=error_details)
