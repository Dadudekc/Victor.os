...
PROJECT_ROOT = SCRIPT_DIR.parents[
    3
]  # Assumes src/dreamos/tools/dreamos_utils structure
...
def archive_mailbox(agent_mailbox_dir: Path):
    ...
    for item in agent_mailbox_dir.iterdir():
        ...
        if item.is_file() and item.suffix == ".msg":
            filename = item.name
...
def main():
    ...
    for agent_dir in MAILBOXES_BASE_DIR.iterdir():
        if (
            agent_dir.is_dir() and not agent_dir.name == ARCHIVE_SUBDIR_NAME
        ):  # Process directories, skip the main archive dir itself
