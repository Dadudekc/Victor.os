...
from dreamos.tools._core.base import BaseTool
...
class ContextPlannerTool(BaseTool):
    ...
    def execute(
        ...
        if not isinstance(context, ToolContext):
            ...
            task_description = context.get_argument("task_description")

        if not task_description:
            raise ValueError(
                f"Tool '{self.name}' requires 'task_description' argument."
            )
    ...
    def _rule_copy_file(
        ...
        if "copy" in task_lower and len(files) == 2:
            ...
            if f"copy {dest_file}" in task_lower and f"to {source_file}" in task_lower:
                ...
                f"copy of {dest_file}" in task_lower
                and f"named {source_file}" in task_lower
            ):
    ...
    def _rule_extract_symbol(
        ...
        if "extract" in task_lower and symbols and len(files) >= 2:
            ...
            try:
                ...
                logger.warning(
                    f"Error parsing extract command structure: {e}. Skipping rule."
                )
        return None  # Rule condition not met

    def _rule_refactor_symbol(
        self, task: str, task_lower: str, files: List[str], symbols: List[str]
    ) -> Optional[List[Dict]]:
        ...
        if "refactor" in task_lower and symbols and files:
            ...
            target_symbol = symbols[0]
            target_file = files[0]
            plan = [{"tool": "read_file", "args": {"filepath": target_file}}]
    ...
    def _rule_migrate_update_generic_refactor(
        ...
        if (is_migration or is_update or (is_refactor and not symbols)) and files:
            ...
            # {{ EDIT START: Improve source/target identification }}
            target_file = None
            source_files = []

            # Try parsing 'from X to Y' pattern
            import re

            match = re.search(
                r"from\s+([\w\./\\_-]+)\s+to\s+([\w\./\\_-]+)", task, re.IGNORECASE
            )
            if match and len(files) >= 2:
                parsed_source = match.group(1).strip()
                parsed_target = match.group(2).strip()
                # Basic check if parsed names roughly match file list
                # More robust check would involve normalization or absolute paths
                if any(parsed_source in f for f in files) and any(
                    parsed_target in f for f in files
                ):
                    target_file = next(
                        (f for f in files if parsed_target in f), files[-1]
                    )
                    source_files = [
                        f for f in files if parsed_source in f and f != target_file
                    ]
                    if (
                        not source_files
                    ):  # If source and target were the same identified file
                        source_files = [
            ...
            # Always read the target file before potentially writing to it
    ...
    def _rule_search(
        ...
        if "search" in task_lower or "find" in task_lower or "grep" in task_lower:
            ...
            plan = [
                {
                    "tool": "grep_search",
                    "args": {"query": search_term, "path": search_path},
                }
            ]
            return plan
        return None

    def _rule_fallback_log(
        self, task: str, task_lower: str, files: List[str], symbols: List[str]
    ) -> Optional[List[Dict]]:
    ...
    def _generate_plan_narration(self, plan: List[Dict[str, Any]], task: str) -> str:
        ...
        if len(plan) == 1:
            ...
            parts.append(
                f"Executing these {len(plan)} steps should accomplish the task."
            )
