...
try:
    ...
    logger.warning("pyttsx3 library not installed. LocalTTS will not be available.")
     ...
     logger.error(f"Error importing pyttsx3: {e}. LocalTTS may not be available.", exc_info=True)
...
class LocalTTS(TTSInterface):
    ...
    def __init__(self):
        ...
        if PYTTSX3_AVAILABLE: # Check library import before init attempt
             try:
                  ...
                  if not hasattr(self.engine, 'proxy') or self.engine.proxy is None or not self.engine.driverName:
                       logger.error("pyttsx3 init succeeded but no valid TTS driver/proxy found. LocalTTS unavailable.")
                       self.engine = None # Mark as unavailable
                  else:
                       logger.info(f"LocalTTS initialized using driver: {self.engine.driverName}")
             except Exception as e:
                  logger.error(f"Failed to initialize pyttsx3 engine: {e}", exc_info=True)
                  self.engine = None # Ensure engine is None if init fails
        else:
             logger.info("LocalTTS not initialized because pyttsx3 library is unavailable.")
    ...
    def synthesize(self, text: str, output_path: str) -> bool:
        ...
        if not self.is_available():
            logger.error("LocalTTS (pyttsx3) is not available (check installation, drivers, and logs).")
            return False
        if not text:
        ...
        if not output_path.lower().endswith(('.wav', '.mp3')):
             ...
             logger.warning(f"Output path {output_path} recommended to end with .wav for pyttsx3 compatibility. Proceeding.")
        ...
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                 ...
                 logger.error("LocalTTS synthesis potentially failed: Output file not created or empty after runAndWait.")
