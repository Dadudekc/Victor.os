...
def read_swarm_state(app_config: Optional[AppConfig] = None) -> Dict[str, Any]:
    ...
    """
    state_file_path_str = _get_sync_config("state_file_path", "runtime/swarm_sync_state.json", app_config)
    ...
    if not state_file_path.is_absolute():
        ...
        try:
            ...
            runtime_base = Path(get_config("paths.runtime", ".", config_obj=app_config if app_config else AppConfig()))
            state_file_path = (runtime_base / state_file_path).resolve()
        except Exception:
            state_file_path = state_file_path.resolve()
            logger.warning(f"Could not robustly resolve relative state_file_path '{state_file_path_str}'. Using CWD-resolved: {state_file_path}")

    if not state_file_path.exists():
        logger.warning(f"Swarm state file not found: {state_file_path}. Returning empty state.")
        return {}
    ...
    try:
        with open(state_file_path, 'r', encoding='utf-8') as f:
            ...
            if not isinstance(state, dict):
                logger.error(f"Swarm state file {state_file_path} does not contain a valid JSON object. Returning empty state.")
                return {}
            logger.debug(f"Read swarm state successfully from {state_file_path}.")
            return state
    except json.JSONDecodeError:
        logger.error(f"Swarm state file {state_file_path} contains invalid JSON. Returning empty state.", exc_info=True)
        return {}
    except OSError as e:
        logger.error(f"Error reading swarm state file {state_file_path}: {e}. Returning empty state.", exc_info=True)
        return {}
    except Exception as e:
        logger.error(f"Unexpected error reading swarm state: {e}. Returning empty state.", exc_info=True)
        return {}
...
def update_agent_state(agent_id: str, 
                       ...
                       status: str, 
                       health_notes: str = "OK",
                       app_config_override: Optional[AppConfig] = None) -> bool:
    ...
    """
    cfg_instance = app_config_override if app_config_override is not None else AppConfig()
    state_file_path_str = _get_sync_config("state_file_path", "runtime/swarm_sync_state.json", cfg_instance)
    state_file_path = Path(state_file_path_str)
    # Path resolution similar to read_swarm_state
    if not state_file_path.is_absolute():
        try:
            runtime_base = Path(get_config("paths.runtime", ".", config_obj=cfg_instance))
            state_file_path = (runtime_base / state_file_path).resolve()
        except Exception:
            state_file_path = state_file_path.resolve()
            logger.warning(f"Could not robustly resolve relative state_file_path '{state_file_path_str}'. Using CWD-resolved: {state_file_path}")
    ...
    try:
        ...
        logger.error(f"Failed to load agent IDs from AgentIdentityStore: {e}", exc_info=True)
        return False

    if not valid_agent_ids:
        logger.error("AgentIdentityStore returned no agent IDs. Cannot validate agent for state update.")
        return False

    if agent_id not in valid_agent_ids:
        logger.error(f"Attempted to update state for unknown or unregistered agent_id: {agent_id}. Valid IDs: {valid_agent_ids}")
        return False
    ...
    for attempt in range(max_attempts):
        logger.debug(f"Attempt {attempt + 1}/{max_attempts} to update state for {agent_id}")
        try:
            current_state = read_swarm_state(app_config=cfg_instance) # Pass config
            current_state[agent_id] = new_state_entry
            tmp_filename = state_file_path.parent / f"{state_file_path.name}.tmp.{os.getpid()}.{uuid.uuid4()}"
            ...
            logger.info(f"Successfully updated state for {agent_id} (Module: {current_module}, Cycle: {current_cycle}, Status: {status})")
            return True
        except OSError as e:
            logger.warning(f"Attempt {attempt + 1} failed for {agent_id}: OS error during file operation: {e}")
            if tmp_filename.exists():
                try: tmp_filename.unlink()
                except OSError: pass 
        except Exception as e:
             logger.error(f"Attempt {attempt + 1} failed for {agent_id}: Unexpected error: {e}", exc_info=True)
             if tmp_filename.exists():
                try: tmp_filename.unlink()
                except OSError: pass 
        if attempt < max_attempts - 1:
            ...
            time.sleep(delay)
    logger.error(f"Failed to update state for {agent_id} after {max_attempts} attempts.")
    return False
...
if __name__ == "__main__":
    ...
    default_state_file_path = Path(_get_sync_config("state_file_path", "runtime/swarm_sync_state.json", None))
    ...
    try:
        ...
        print(f"Error initializing/populating demo identity store: {e}", file=sys.stderr)
    ...
    current_swarm_state = read_swarm_state() # Uses default AppConfig implicitly via _get_sync_config
