...
class DriftDetector:
    ...
    def __init__(self, window_size: int = 100, threshold: float = 0.1):
        self.window_size = window_size  # Number of actions to consider for drift detection
        self.threshold = threshold      # Threshold for drift detection
        self.action_history: Dict[str, List[Tuple[datetime, str, float]]] = defaultdict(list)
        self.violation_patterns: Dict[str, List[Tuple[str, int]]] = defaultdict(list)
        
    def add_action(self, agent_id: str, action_type: str, compliance_score: float) -> Optional[Dict]:
        """
        ...
        self.action_history[agent_id] = [
            (t, a, s) for t, a, s in self.action_history[agent_id]
            if t > cutoff
        ]
    ...
    def add_violation(self, agent_id: str, violation_type: str) -> Optional[Dict]:
        ...
        self.violation_patterns[agent_id] = [
            (v, t) for v, t in self.violation_patterns[agent_id]
            if t > cutoff
        ]
    ...
    def _generate_drift_recommendation(self, trend: float, std: float) -> str:
        ...
        if trend < -self.threshold * 2:
            return "Severe compliance drift detected. Immediate intervention recommended."
        elif trend < -self.threshold:
            return "Moderate compliance drift detected. Monitor closely."
        elif std > self.threshold:
            return "High variance in compliance scores. Consider reviewing recent actions."
    ...
    def predict_compliance(self, agent_id: str, horizon: int = 10) -> Dict:
        ...
        recent_actions = self.action_history[agent_id][-min(self.window_size, len(self.action_history[agent_id])):]
        scores = [s for _, _, s in recent_actions]
        ...
        return {
            ...
            "trend": float(z[0]),
            "warning": "High risk of compliance drop" if z[0] < -self.threshold else None
        } 