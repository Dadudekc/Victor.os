...
def get_class_attributes(cls: Type[Any]) -> List[str]:
    ...
    return [name for name, _ in inspect.getmembers(cls, lambda x: not inspect.isroutine(x))]

def get_method_signature(cls: Type[Any], method_name: str) -> Optional[inspect.Signature]:
