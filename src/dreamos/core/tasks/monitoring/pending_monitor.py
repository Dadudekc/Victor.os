...
class PendingTaskMonitor:
    ...
    async def _handle_stalled_task(self, task: Task) -> None:
        ...
        logger.info(f"Handling stalled task {task.task_id} with strategy: {escalation_strategy}")
        ...
        if escalation_strategy == "log_only":
            ...
            try:
                ...
                logger.info(f"Created escalation task {escalation_task['task_id']} for stalled task {task.task_id}")
            except Exception as e:
                logger.error(f"Failed to create escalation task for {task.task_id}: {e}")
        else:
