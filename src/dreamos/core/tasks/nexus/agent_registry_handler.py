...
class AgentRegistryHandler:
    ...
    async def initialize(self):
        ...
        async with getattr(
            ...
            logger.info(
                f"AgentRegistryHandler initialized (TTL: {self.heartbeat_ttl}s)"
            )
    ...
    async def _load_agents_from_db(self) -> None:
        ...
        try:
            self.agents = (
                self.adapter.get_all_agents()
            )  # Assumes adapter method is sync
    ...
    async def record_heartbeat(
        ...
        if not self._initialized:
            logger.error(
                "AgentRegistryHandler not initialized. Call await initialize() first."
            )
            return False
        if timestamp is None:
            timestamp = time.time()
        try:
            self.adapter.update_agent_heartbeat(
    ...
    async def reclaim_stale_tasks(
        self,
    ) -> List[Dict[str, Optional[str]]]:  # Updated return type hint
