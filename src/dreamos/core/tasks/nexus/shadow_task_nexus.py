...
from typing import Any, Dict, List, Optional, TypedDict  # Updated typing imports
...
class ShadowTaskNexus:
    ...
    DEFAULT_BACKLOG_PATH = str(
        PROJECT_ROOT / "runtime" / "tasks" / "shadow_backlog.json"
    )  # Updated path
    ...
    def _ensure_backlog_exists(self):
        ...
        if not self.backlog_path.exists():
            logger.warning(
                f"Shadow backlog file not found at {self.backlog_path}. Creating."
            )
            try:
                ...
                logger.error(
                    f"Failed to create shadow backlog file at {self.backlog_path}: {e}",
                    exc_info=True,
                )
    ...
    def load_tasks(self) -> bool:
        ...
        if not self.validate_shadow_backlog(self.backlog_path):
            logger.error(
                f"Shadow backlog file {self.backlog_path} failed validation. Cannot load tasks."
            )
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded {len(self.tasks)} tasks from {self.backlog_path}"
            )
            return True
        except (IOError, json.JSONDecodeError) as e:
            logger.error(
                f"Failed to load tasks from {self.backlog_path}: {e}", exc_info=True
            )
    ...
    def add_task(self, task: TaskDict) -> bool:
        ...
        if self.backlog_path.exists():
            if not self.validate_shadow_backlog(self.backlog_path):
                logger.warning(
                    f"Shadow backlog {self.backlog_path} is invalid. Attempting to overwrite with new task list."
                )
                ...
                try:
                    ...
                    if not isinstance(current_tasks, list):
                        logger.warning(
                            f"Shadow backlog {self.backlog_path} is not a list. Overwriting."
                        )
                        current_tasks = []
                except (IOError, json.JSONDecodeError) as e:
                    logger.error(
                        f"Error reading shadow backlog before add: {e}. Attempting to overwrite."
                    )
                    current_tasks = []
        ...
        if any(t.get("task_id") == task["task_id"] for t in current_tasks):
            logger.warning(
                f"Task ID {task['task_id']} already exists in shadow backlog. Skipping addition."
            )
        ...
        try:
            ...
            logger.info(
                f"Successfully added task {task['task_id']} to {self.backlog_path}"
            )
            ...
            logger.error(
                f"Failed to write updated shadow backlog to {self.backlog_path}: {e}",
                exc_info=True,
            )
            return False
    ...
    def validate_shadow_backlog(file_path: str = DEFAULT_BACKLOG_PATH) -> bool:
        ...
        try:
            ...
            if not isinstance(data, list):
                logger.error(
                    f"Shadow backlog validation failed: Root element is not a list in {path}."
                )
                return False
            ...
            logger.error(
                f"Shadow backlog validation failed: JSONDecodeError in {path}: {e}"
            )
            return False
        except IOError as e:
            logger.error(
                f"Shadow backlog validation failed: IOError reading {path}: {e}"
            )
            return False
        except Exception as e:
            logger.error(
                f"Shadow backlog validation failed: Unexpected error reading {path}: {e}",
                exc_info=True,
            )
            return False
