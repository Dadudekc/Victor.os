...
from PyQt5.QtGui import QFont, QIcon, QColor # Import QIcon and QColor
...
class DreamOSMainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        async def _on_prompt_event(evt):
            ...
            if agent_id not in self.agent_counters:
                self.agent_counters[agent_id] = {"success": 0, "failure": 0, "escalation": 0}
            # handle event types
            if etype == 'prompt_success':
                ...
                self.agent_counters[agent_id]['last_escalation'] = datetime.now().timestamp()
                # EDIT END
        ...
        def _subscribe_prompt_event():
            try:
                asyncio.create_task(self._agent_bus.register_handler(EventType.SYSTEM, _on_prompt_event))
    ...
    def _create_navigation(self):
        ...
        self.add_navigation_item("Escalated Agents", "icons/escalation.png", escalated_widget)
