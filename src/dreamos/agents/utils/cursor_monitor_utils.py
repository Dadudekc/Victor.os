...
class CursorStateMonitor:
    ...
    def _capture_cursor_image(self, coords: Dict[str, Dict[str, int]]) -> Optional[Image.Image]:
    ...
    async def _monitor_agent_state(self, agent_id: str, coords: Dict[str, Dict[str, int]]) -> None:
        ...
        try:
                ...
                if current_img:
                    ...
                    if is_generating:
                        if agent_id not in self.stuck_timers:
                            self.stuck_timers[agent_id] = asyncio.get_event_loop().time()
                        elif asyncio.get_event_loop().time() - self.stuck_timers[agent_id] > 60:
            ...
            logger.error(f"Error monitoring cursor agent {agent_id}: {e}", exc_info=True)
        finally:
            self.stuck_timers.pop(agent_id, None)
    ...
    async def _handle_stuck_agent(self, agent_id: str) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to publish stuck alert for agent {agent_id}: {e}", exc_info=True)
    ...
    async def start_monitoring(self, agent_id: str, coords: Dict[str, Dict[str, int]]) -> None:
