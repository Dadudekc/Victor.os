...
class CursorCoordinator:
    ...
    async def register_agent(
        self, agent_id: str, coords: Dict[str, Dict[str, int]], task: TaskMessage
    ) -> None:
    ...
    async def wait_for_response(self, agent_id: str) -> Optional[str]:
        ...
        try:
            response = await asyncio.wait_for(
                self.response_futures[agent_id], timeout=self.response_timeout
            )
            return response
    ...
    async def _copy_agent_response(self, agent_id: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to copy response from agent {agent_id}: {e}", exc_info=True
            )
    ...
    def _should_send_to_chatgpt(self, task: TaskMessage) -> bool:
        ...
        if response:
            ...
            return any(
                re.search(pattern, response, re.IGNORECASE) for pattern in thea_patterns
            )
