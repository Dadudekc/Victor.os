...
class TaskPromoterAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs # Capture any other BaseAgent args
        ...
        try:
             super().__init__(agent_id=agent_id, config=effective_config, pbm=effective_pbm, **kwargs)
        except TypeError as e:
             logger.error(f"Error calling BaseAgent init, potential signature mismatch: {e}. Falling back to basic init.")
        ...
        self.logger.info(f"TaskPromoterAgent initialized. Poll interval: {self.poll_interval}s")
    ...
    def _get_completed_task_ids(self) -> Set[str]:
        ...
        try:
            ...
            return {task.get("task_id") for task in completed_tasks if task.get("task_id")}
    ...
    def promote_eligible_tasks(self):
        ...
        if not completed_ids and self.pbm.completed_tasks_path.exists():
             ...
             self.logger.warning("Completed task IDs set is empty, promotion check might be inaccurate.")
        ...
        for task in pending_tasks:
            ...
            if not isinstance(dependencies, list):
                 self.logger.warning(f"Task {task_id} has invalid dependencies format: {dependencies}. Skipping.")
                 continue

            # Check if all dependencies are in the completed set
            if not dependencies or all(dep_id in completed_ids for dep_id in dependencies):
                self.logger.info(f"Task {task_id} dependencies met. Attempting promotion...")
                try:
                    # Use the PBM's public method for promotion
                    if self.pbm.promote_task_to_ready(task_id):
                        self.logger.info(f"✅ Successfully promoted task {task_id} to ready queue.")
                        promoted_count += 1
                    else:
                        # This path might not be reachable if promote_task_to_ready raises exceptions on failure
                         self.logger.warning(f"Promotion call for {task_id} returned False.")
                    ...
                    self.logger.error(f"❌ Unexpected error promoting task {task_id}: {e}", exc_info=True)
            else:
                 self.logger.debug(f"Task {task_id} dependencies not met: {dependencies}. Required: {completed_ids}")

        if promoted_count > 0:
             self.logger.info(f"Promotion cycle complete. Promoted {promoted_count} task(s).")
        else:
    ...
    async def run_main_loop(self):
        ...
        self.logger.info(f"🔁 TaskPromoterAgent run_main_loop started. Agent ID: {self.agent_id}")
        self._running = True
        while self._running:
    ...
    def run_standalone(self):
         ...
         self.logger.info(f"🔁 TaskPromoterAgent standalone loop started. Agent ID: {self.agent_id}")
         while True: # Simple infinite loop for standalone execution
             try:
                 self.promote_eligible_tasks()
                 time.sleep(self.poll_interval)
             except KeyboardInterrupt:
                 self.logger.info("KeyboardInterrupt received. Stopping standalone loop.")
                 break
             except Exception as e:
