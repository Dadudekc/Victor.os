...
class SupervisorAgent:
    ...
    def __init__(self, config: AppConfig):
        ...
        try:
            ...
            logger.info(f"SupervisorAgent using blob storage path: {local_blob_path_str}")
        ...
        if local_blob_path_str:
            try:
                self.channel = LocalBlobChannel(base_dir=local_blob_path_str)
                 ...
                 raise ConfigurationError(f"Failed to initialize LocalBlobChannel: {e}") from e
        else:
    ...
    def load_directives(self):
        ...
        try:
            ...
            logger.warning(f"Directive file not found: {self.directive_path}. Returning empty list.")
            return []
        except json.JSONDecodeError as e:
             logger.error(f"Error decoding JSON from directive file {self.directive_path}: {e}")
             return []
        except Exception as e:
            logger.error(f"Failed to load directives from {self.directive_path}: {e}", exc_info=True)
            return []

    def dispatch_tasks(self, tasks):
        """Dispatches new tasks via the communication channel."""
        if not self.channel:
            logger.error("Cannot dispatch tasks: Communication channel not initialized.")
            return
        ...
        for task in tasks:
            ...
            try:
                ...
                self.dispatched.add(task_id)
                 ...
                 logger.error(f"Failed to push task {task_id} to channel: {e}", exc_info=True)
    ...
    def gather_results(self):
        ...
        if not self.channel:
            logger.error("Cannot gather results: Communication channel not initialized.")
            return []
                ...
                for r in results:
                    ...
                    if isinstance(r, dict) and 'id' in r:
                        ...
                        collected.append(r)
                         ...
                         logger.warning(f"Received invalid result format from channel: {r}")
                results = self.channel.pull_results()
        except Exception as e:
    ...
    def save_results(self, results):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', delete=False, dir=output_path.parent, suffix='.tmp') as tmp_file:
            ...
            logger.error(f"Failed to write results to {self.results_output_path}: {e}", exc_info=True)
            # Clean up temporary file if it exists and possible
            if 'temp_file_path' in locals() and os.path.exists(temp_file_path):
                 try:
                      os.remove(temp_file_path)
                 except Exception as e_remove:
                      logger.error(f"Error removing temporary results file {temp_file_path}: {e_remove}")
    ...
    def run_loop(self, interval=5):
        ...
        if not self.channel or not self.directive_path or not self.results_output_path:
            logger.critical("SupervisorAgent cannot run: Initialization failed (missing channel or paths). Exiting loop.")
            return
