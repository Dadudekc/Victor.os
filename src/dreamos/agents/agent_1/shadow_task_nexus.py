...
try:
    ...
    logger.warning("Could not import Task model from core nexus, defining locally for ShadowTaskNexus.")
...
class ShadowTaskNexus:
            ...
            if not self.task_file_path.exists():
                ...
                try:
                    ...
                    with open(self.task_file_path, "w", encoding="utf-8") as f:
                        json.dump([], f)
                    logger.info(f"Created empty shadow backlog file: {self.task_file_path}")
                except OSError as e:
                    logger.error(f"Failed to create shadow backlog directory/file {self.task_file_path}: {e}", exc_info=True)
            ...
            try:
                with open(self.task_file_path, "r", encoding="utf-8") as f:
                    ...
                    if not content.strip(): # Handle empty file case
                         logger.warning(f"Shadow task file {self.task_file_path} is empty, returning empty list.")
                         return []
                ...
                for i, task_data in enumerate(tasks_data):
                     try:
                         ...
                         validation_errors += 1
                         logger.error(f"Validation error loading task #{i+1} from shadow backlog {self.task_file_path}: {e}")
                         # Optionally skip invalid tasks or halt? Skipping for resilience.

                if validation_errors > 0:
                     logger.warning(f"Completed loading shadow tasks with {validation_errors} validation errors.")

                return validated_tasks
            except json.JSONDecodeError as e:
                 logger.error(f"JSON decode error loading shadow tasks from {self.task_file_path}: {e}. File content might be corrupted. Returning empty list.", exc_info=True)
                 return []
            except FileNotFoundError: # Should be caught by exists() check, but belt-and-suspenders
                 logger.error(f"Shadow task file {self.task_file_path} disappeared between checks? Returning empty list.", exc_info=True)
                 return []
            except Exception as e:
                logger.exception(f"Unexpected error loading shadow tasks from {self.task_file_path}: {e}")
                return []
    ...
    def _save(self):
        ...
        with self._lock:
            try:
                ...
                logger.debug(f"Saved {len(self.tasks)} shadow tasks to {self.task_file_path}")
            except OSError as e:
                 logger.error(f"OS error saving shadow tasks to {self.task_file_path}: {e}", exc_info=True)
            except Exception as e:
                logger.exception(f"Unexpected error saving shadow tasks to {self.task_file_path}: {e}")
        ...
        with self._lock:
            for task in self.tasks:
                if task.status == "pending":
                    ...
                    task.status = "claimed"
                    task.claimed_by = agent_id if agent_id else "unknown_shadow_agent" # Identify shadow claim
                    task.updated_at = datetime.utcnow().isoformat()
                    self._save()  # Save immediately after claiming
                    logger.info(f"Shadow task {task.task_id} claimed by {task.claimed_by}")
        ...
        with self._lock:
            try:
                ...
                if any(t.task_id == new_task.task_id for t in self.tasks):
                    logger.warning(f"Shadow task {new_task.task_id} already exists. Skipping add.")
                    return next((t.model_copy(deep=True) for t in self.tasks if t.task_id == new_task.task_id), None)
                ...
                logger.error(f"Failed to validate and add shadow task: {e}", exc_info=True)
                ...
                if task.task_id == task_id:
                    ...
                    if result is not None:
                        ...
                        try:
                             ...
                             logger.warning(f"Result for shadow task {task_id} is not JSON serializable: {json_err}. Storing as string.")
    ...
    def get_all_tasks(self, status: Optional[str] = None) -> List[Task]:
        ...
        if status:
            return [task for task in tasks_copy if task.status.lower() == status.lower()]
        return tasks_copy
...
if __name__ == '__main__':
    ...
    # Test update status
    if next_t:
        updated = nexus.update_task_status(next_t.task_id, "completed", result={"output": "success"})
    ...
    print(f"Retrieved Task Status: {retrieved_task.status if retrieved_task else 'Not Found'}")
