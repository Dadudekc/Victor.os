...
class ConfigManager:
    ...
    def __init__(
        ...
        if cache_provider is None:
            self.cache_provider = InMemoryCacheProvider(max_cache_size=max_in_memory_cache_size)
        else:
    ...
    def validate_config(self, config: Dict[str, Any], schema: Dict[str, Any]) -> bool:
        def _validate_type(value: Any, expected_type: Any) -> bool:
            ...
            elif expected_type == "object": return isinstance(value, dict)
            elif expected_type == "array": return isinstance(value, list)
            return False
        def _validate_schema(data: Any, schema_node: Any) -> bool:
            if isinstance(schema_node, dict):
                ...
                for key, value_schema in schema_node.items():
                    ...
                    if not _validate_schema(data[key], value_schema): return False
            elif isinstance(schema_node, list):
                if not isinstance(data, list): return False
                if len(schema_node) > 0:
                    item_schema = schema_node[0]
                    for item in data:
                        if not _validate_schema(item, item_schema): return False
            else: return _validate_type(data, schema_node)
            return True
