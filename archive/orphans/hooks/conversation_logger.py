...
# event loop. Consider using an async SQLite library (e.g., aiosqlite)
# and making the handlers async if performance/responsiveness is critical.
...
class ConversationLogger:
    ...
    def register_event_handlers(self):
        ...
        self.agent_bus.register_handler(
            EventType.AGENT_PROMPT_REQUEST.value, self._handle_agent_prompt_request
        )
        self.agent_bus.register_handler(
    ...
    def _log_turn(
        ...
        try:
            ...
            logger.error(
                f"Unexpected error logging conversation turn: {e}", exc_info=True
            )

    # --- Updated Event Handlers ---

    def _handle_agent_prompt_request(self, event: BaseDreamEvent):
        """Handles AGENT_PROMPT_REQUEST event."""
        try:
            logger.debug(
                f"Received AGENT_PROMPT_REQUEST data: {event.data}"
            )  # Verify payload
            ...
            target_agent_id = event.data.get(
                "target_agent_id"
            ...
            logger.error(
                f"Error handling {EventType.AGENT_PROMPT_REQUEST.value} event: {e}",
                exc_info=True,
            )

    def _handle_agent_prompt_response(self, event: BaseDreamEvent):
        """Handles AGENT_PROMPT_RESPONSE event."""
        try:
            logger.debug(
                f"Received AGENT_PROMPT_RESPONSE data: {event.data}"
            )  # Verify payload
            # Extract data based on expected BaseEvent structure
            # We need to KNOW what fields are in event.data for this event type
            task_id = event.data.get("task_id")
            conversation_id = event.data.get("conversation_id")  # If available
            reply_content = event.data.get(
                "response"
            )  # Assuming field name is 'response'
            agent_id = event.source_id  # Agent providing the response
            metadata = event.data.get("metadata", {})
            # Add event details to metadata for context
            metadata["event_id"] = event.event_id
            metadata["request_event_id"] = event.data.get(
                "request_event_id"
            ...
            logger.error(
                f"Error handling {EventType.AGENT_PROMPT_RESPONSE.value} event: {e}",
                exc_info=True,
            )
