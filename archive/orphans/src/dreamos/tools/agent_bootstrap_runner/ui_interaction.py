...
from .config import RESPONSE_WAIT_SEC, AgentConfig
...
class UIFailureLogger:
    ...
    def log_failure(
        self, stage: str, error: Exception, retries: int, **extra: Dict[str, Any]
    ):
...
class AgentUIInteractor:
    ...
    def _validate_coordinates(self) -> bool:
        ...
        if not self.config.copy_coords_file.exists():
            self.logger.error(
                f"Copy coordinates file not found: {self.config.copy_coords_file}"
            )
            return False
        ...
        for attempt in range(UI_CONFIG["retries"]):
            try:
                ...
                if not self.window_manager.ensure_window_focus():
                    self.logger.warning(
                        f"Could not focus window for {self.config.agent_id}"
                    )
                    continue
    ...
    async def retrieve_response(self, bus: AgentBus) -> Optional[str]:
        ...
        for attempt in range(UI_CONFIG["retries"]):
            try:
                ...
                self.failure_logger.log_failure(
                    "retrieve", e, attempt + 1, screenshot=screenshot_path
                )
