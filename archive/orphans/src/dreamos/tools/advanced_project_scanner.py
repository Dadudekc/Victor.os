...
from typing import Any, Dict, List, Optional, Set, Tuple, Union  # Added Set and Tuple
...
class LanguageAnalyzer:
    ...
    def __init__(self):
        ...
        self.rust_parser = self._init_tree_sitter_language(
            "rust", "path/to/tree-sitter-rust.so"
        )
        self.js_parser = self._init_tree_sitter_language(
    ...
    def _init_tree_sitter_language(
        self, lang_name: str, grammar_path: str
    ) -> Optional[Parser]:
        """
        ...
        if not Path(grammar_path).exists():
            logger.warning(
                f"⚠️ {lang_name} grammar not found at {grammar_path}. Ensure paths are correct."
            )
            return None
        ...
        try:
            ...
            logger.info(
                f"Initialized tree-sitter {lang_name} parser from {grammar_path}"
            )
            return parser
        except Exception as e:
            logger.error(
                f"⚠️ Failed to initialize tree-sitter {lang_name} parser from {grammar_path}: {e}"
            )
            return None
    ...
    def _analyze_python(self, source_code: str) -> Dict:
        ...
        complexity = len(functions) + sum(
            len(c.get("methods", [])) for c in classes.values()
        )
        return {
            "language": ".py",
            "functions": functions,
    ...
    def _analyze_rust(self, source_code: str) -> Dict:
        ...
        functions: List[str] = []
        structs_and_impls: Dict[str, List[str]] = defaultdict(
            list
        )  # Using dict to store methods per struct/impl
        ...
        def _traverse_rust(node):
            if node.type == "function_item":
                ...
                name_node = node.child_by_field_name("name")
                if name_node:
                    structs_and_impls[name_node.text.decode("utf-8")] = (
                        []
                    ...
                    if body_node:
                        for child_node in body_node.children:
                            if child_node.type == "function_item":
                                method_name_node = child_node.child_by_field_name(
                                    "name"
                                )
                                if method_name_node:
                                    structs_and_impls[type_name].append(
                ...
                path_node = node.child_by_field_name(
                    "argument"
                )  # tree-sitter-rust might use 'path' or 'argument'
                if (
                    not path_node and len(node.children) > 1
                ):  # Fallback for some structures
                    path_node = node.children[1]  # often the path part
                if path_node:
                    imports.append(
                        {"type": "use", "path": path_node.text.decode("utf-8")}
                    )
        ...
        complexity = len(functions) + sum(
            len(methods) for methods in structs_and_impls.values()
        )
        return {
            "language": ".rs",
            "functions": functions,
            "classes": structs_and_impls,
            "routes": [],
            "imports": imports,
            "complexity": complexity,
        }
    ...
    def _analyze_javascript(self, source_code: str) -> Dict:
        ...
        def _traverse_js(node):
            ...
            if node_type in (
                ...
                ):  # Try to get var name for anonymous arrow funcs
                    parent = node.parent
                    ...
                    if body_node and hasattr(body_node, "children"):
                        for member_node in body_node.children:
                            if member_node.type == "method_definition":
                                method_name_node = member_node.child_by_field_name(
                                    "name"
                                )
                                if method_name_node:
                                    classes[class_name].append(
                                        get_node_text(method_name_node)
                                    )
                    ...
                    if child.type == "import_clause":
                        for named_import_node in child.children:
                            if named_import_node.type == "named_imports":
                                for specifier in named_import_node.children:
                                    if specifier.type == "import_specifier":
                                        ...
                                        alias = (
                                            ...
                                            if specifier.child_by_field_name("alias")
                                            else None
                                        )
                                        imports.append(
                                            {
                                ...
                                named_import_node.type == "identifier"
                            ):  # Default import
                                imports.append(
                                    {
                                        "type": "default",
                                        "source": source,
                                        "name": get_node_text(named_import_node),
                                    }
                                )
                    elif (
                        child.type == "string"
                        ...
                        imports.append(
                            {"type": "side-effect", "source": get_node_text(child)}
                        )
                ...
                if func_node and get_node_text(func_node) == "require":
                    ...
                    if args_node and args_node.children:
                        first_arg = args_node.children[
                            0
                        ]  # Assuming first arg is the module path string
...
class BotWorker(threading.Thread):
    ...
    ):  # scanner_instance to call _process_file_internal
        super().__init__()
        ...
        while True:
            try:
                file_path = self.task_queue.get(
                    timeout=1
                )  # Timeout to allow thread to exit
            except queue.Empty:
            ...
            try:
                ...
                logger.error(
                    f"Error processing {file_path} in {self.name}: {e}", exc_info=True
                )
            finally:
                self.task_queue.task_done()
...
class MultibotManager:
    ...
    def __init__(
        self, scanner_instance, num_workers=os.cpu_count() or 1, status_callback=None
    ):
        self.task_queue = queue.Queue()
        self.results_list: List[Tuple[Path, Dict]] = (
            []
        )  # Expect tuples of (file_path, analysis_data)
...
class FileProcessor:
    ...
    def __init__(
        ...
        self.ignore_dirs = {
            ...
            "examples",
            "samples",
            "scripts",  # Common general ignores
            ".vscode",
            ".idea",
            ".devcontainer",  # IDE specific
    ...
    def should_exclude(self, file_path: Path) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Pathlib .parts failed for {file_path}, using string split fallback."
            )
...
class ReportGenerator:
    ...
    def __init__(
        self, project_root: Path, analysis_results: List[Tuple[Path, Dict[str, Any]]]
    ):
        self.project_root = project_root
        # Convert list of tuples to dict for easier lookup by relative path
        self.analysis_data: Dict[str, Dict[str, Any]] = {
            str(path.relative_to(project_root)): data
            for path, data in analysis_results
            if data
        }
    ...
    def save_project_analysis_report(self):
        ...
        try:
            ...
            logger.info(
                f"Project analysis report saved to {self.project_analysis_file}"
            )
        except IOError as e:
            logger.error(
                f"Could not save project analysis report to {self.project_analysis_file}: {e}"
            )
    ...
    def export_chatgpt_context(
        ...
        final_output_path = (
            Path(output_path) if output_path else self.chatgpt_context_file
        )
            ...
            if template_p.exists():
                try:
                    ...
                    output_content = template_content.replace(
                        "{{project_context_json}}", json.dumps(context_data, indent=2)
                    )
                except Exception as e:
                    logger.error(
                        f"Error processing template {template_path}: {e}. Falling back to raw JSON."
                    )
                    output_content = json.dumps(
                        {"project_context": context_data}, indent=4
                    )
            else:
                logger.warning(
                    f"Template file {template_path} not found. Exporting raw JSON."
                )
        ...
        try:
            ...
            logger.error(
                f"Could not export ChatGPT project context to {final_output_path}: {e}"
            )
...
class ProjectScanner:
    ...
    DEFAULT_IGNORE_DIRS = {
        ...
        "tests",
        "examples",
        "samples",  # Often excluded from primary source analysis
        ".vscode",
        ".idea",
        ".devcontainer",
        "site",
        ...
        "migrations",
        "static",
        "media",  # Common web framework folders
    }
    DEFAULT_IGNORE_FILES = {
        ...
        "requirements.txt",
        "Pipfile",
        "Pipfile.lock",  # Often not primary source
    }
    ...
    def __init__(
        ...
        tree_sitter_grammar_paths: Optional[Dict[str, str]] = None,
    ):  # e.g. {"rust": "/path/to/rust.so"}

        self.project_root = Path(project_root).resolve()
        self.cache_file = self.project_root / cache_file_name
        self.analysis_file = (
            self.project_root / analysis_file_name
        )  # For ReportGenerator
        self.chatgpt_context_file = (
            self.project_root / chatgpt_context_file_name
        )  # For ReportGenerator
        ...
        self.include_extensions = self.DEFAULT_INCLUDE_EXTENSIONS.union(
            include_extensions or set()
        )
        ...
        if tree_sitter_grammar_paths:
            if (
                "rust" in tree_sitter_grammar_paths
                and self.language_analyzer.rust_parser
            ):
                self.language_analyzer.rust_parser = (
                    self.language_analyzer._init_tree_sitter_language(
                        "rust", tree_sitter_grammar_paths["rust"]
                    )
                )
            if (
                "javascript" in tree_sitter_grammar_paths
                and self.language_analyzer.js_parser
            ):
                self.language_analyzer.js_parser = (
                    self.language_analyzer._init_tree_sitter_language(
                        "javascript", tree_sitter_grammar_paths["javascript"]
                    )
                )
        ...
        self.analysis_results: List[Tuple[Path, Dict[str, Any]]] = (
            []
        )  # Stores (file_path, analysis_data)
    ...
    def load_cache(self) -> Dict:
        if self.cache_file.exists():
            try:
                with (
                    self.cache_lock
                ):  # Ensure thread-safe read if ever needed, though typically init is single-threaded
                    with open(self.cache_file, "r", encoding="utf-8") as f:
                        loaded_cache = json.load(f)
                        logger.info(
                            f"Loaded cache from {self.cache_file} with {len(loaded_cache)} entries."
                        )
                        return loaded_cache
            except (json.JSONDecodeError, IOError) as e:
                logger.warning(
                    f"Could not load cache file {self.cache_file}: {e}. Starting with empty cache."
                )
        return {}
        ...
        try:
            with self.cache_lock:
                with open(self.cache_file, "w", encoding="utf-8") as f:
                    json.dump(
                        self.cache, f, indent=2, sort_keys=True
                    )  # Using indent 2 for cache
    ...
    def scan_project(
        ...
        """
        logger.info(
            f"Starting project scan at {self.project_root} with {self.num_workers} workers."
        )
        ...
        logger.info(
            f"Found {file_count} total files. Queued {processed_count} files for analysis."
        )

        self.analysis_results = (
            manager.wait_for_completion()
        )  # This now returns list of (path, data) tuples
        ...
        report_generator.project_analysis_file = (
            self.analysis_file
        )  # Ensure it uses the configured name
        report_generator.chatgpt_context_file = (
            self.chatgpt_context_file
        )  # Ensure it uses the configured name
    ...
    def export_chatgpt_context(
        ...
        if not self.analysis_results:
            logger.warning(
                "No analysis results available to export ChatGPT context. Run scan_project() first."
            )
            # Or, attempt to load from project_analysis.json if it exists
            if self.analysis_file.exists():
                logger.info(
                    f"Attempting to load analysis results from {self.analysis_file} for ChatGPT context export."
                )
                try:
                    with open(self.analysis_file, "r", encoding="utf-8") as f:
                    ...
                    if "analysis_details" in loaded_analysis:
                        temp_results = []
                        for path_str, data_dict in loaded_analysis[
                            "analysis_details"
                        ].items():
                            temp_results.append(
                                (self.project_root / path_str, data_dict)
                            )
                        ...
                        report_generator.chatgpt_context_file = (
                            Path(output_path)
                            if output_path
                            else self.chatgpt_context_file
                        )
                        report_generator.export_chatgpt_context(
                            template_path=template_path
                        )  # output_path is handled by ReportGenerator now
                        return
                    else:
                        logger.error(
                            "Loaded analysis report does not contain 'analysis_details'. Cannot export context."
                        )
                        return
                except Exception as e:
                    logger.error(
                        f"Failed to load or process {self.analysis_file} for context export: {e}"
                    )
                    return
            else:
                logger.error(
                    f"{self.analysis_file} does not exist. Cannot export context."
                )
                return
        ...
        logger.info(
            f"ChatGPT context exported to {output_path or self.chatgpt_context_file}"
        )

    def find_name_collisions(
        self, scan_path: Optional[Union[str, Path]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        item_map = defaultdict(
            lambda: {
                "dirs": set(),
                "files_no_ext": set(),
                "files_with_ext": defaultdict(set),
            }
        )
        ...
        if not hasattr(self, "file_processor") or self.file_processor is None:
            ...
            logger.warning(
                "FileProcessor not initialized by scan_project. Initializing with defaults for name collision scan."
            )
        ...
        for name, types in item_map.items():
            ...
            found_files_matching_name_exactly = (
                item_map[name]["files_no_ext"] if name in item_map else set()
            )

            # Files like 'name.py', 'name.json' where 'name' is the base
            # These are already collected under item_map[name]["files_with_ext"]
            found_files_with_base_as_name = (
                item_map[name]["files_with_ext"].keys() if name in item_map else set()
            )
        ...
        logger.info(
            f"Name collision scan completed. Found {len(collisions)} potential collision sets."
        )
