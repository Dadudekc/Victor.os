...
async def websocket_endpoint(
    websocket: WebSocket, agent_id: str, memory: AgentMemory = Depends(get_memory)
):
...
async def get_agent_dna_profile(
    agent_id: str, memory: AgentMemory = Depends(get_memory)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get agent DNA profile")


@router.get("/agents/{agent_id}/history")
async def get_agent_dna_history(
    agent_id: str, days: int = 30, memory: AgentMemory = Depends(get_memory)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get agent DNA history")
...
async def get_agent_drift_analysis(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get agent drift analysis"
        )
...
async def get_agent_behavioral_patterns(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get agent behavioral patterns"
        )
