...
class WebSocketManager:
            ...
            if log_data["type"] == "compliance":
                ...
                if drift_warning:
                    await self.broadcast(
                        {"type": "drift_warning", "data": drift_warning}
                    )

                # Get compliance prediction
                prediction = self.predictive_model.predict_drift(
                    log_data["agent_id"], log_data["metrics"]
                )

                await self.broadcast(
                    {"type": "compliance_prediction", "data": prediction}
                )
                ...
                if pattern_warning:
                    await self.broadcast(
                        {"type": "pattern_warning", "data": pattern_warning}
                    )

            # Get agent insights
            insights = self.predictive_model.get_agent_insights(log_data["agent_id"])
            await self.broadcast({"type": "agent_insights", "data": insights})

        except Exception as e:
            logger.error(f"Error processing log update: {str(e)}")
            # Broadcast error to all connected clients
            await self.broadcast({"type": "log_update", "error": str(e)})
    ...
    async def process_log_update(self, websocket: WebSocket, log_path: str) -> None:
        ...
        try:
            ...
            # Get agent insights
            insights = self.predictive_model.get_agent_insights(log_data["agent_id"])
            await websocket.send_json({"type": "agent_insights", "data": insights})
