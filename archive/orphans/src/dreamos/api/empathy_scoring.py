...
async def get_agent_score(
    agent_id: str, days: int = Query(30, description="Days of history to include")
) -> ScoreDetails:
    """
    ...
    try:
        ...
        if agent_id in score_cache and datetime.now() - datetime.fromisoformat(
            score_cache[agent_id]["timestamp"].replace("Z", "+00:00")
...
async def get_agent_comparison() -> AgentComparison:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error creating comparison: {str(e)}"
        )
...
async def recalculate_agent_score(agent_id: str) -> ScoreDetails:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error recalculating score: {str(e)}"
        )
...
async def get_empathy_threshold_status() -> Dict[str, Any]:
    ...
    try:
        ...
        avg_score = sum(data["score"] for data in score_cache.values()) / len(
            score_cache
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting threshold status: {str(e)}"
        )
