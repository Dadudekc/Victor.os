...
import pytest
from src.dreamos.integrations.cursor.bridge.feedback.thea_response_handler import (
    TheaResponseHandler,
)
from src.dreamos.integrations.cursor.bridge.relay.response_relay import ResponseRelay
from src.dreamos.integrations.cursor.bridge.schemas.thea_response_schema import (
    ResponseStatus,
    ResponseType,
    TheaResponse,
)
...
def test_full_pipeline(bridge_components):
    ...
    for i in range(3):
        ...
        inbox_file = (
            agent_inbox_base / f"Agent-{agent_id}" / "inbox" / f"{task_id}.json"
        )
