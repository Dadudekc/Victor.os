...
def test_bridge_prompt_response_flow(bridge_loop, tmp_path):
    ...
    prompt_data = {"prompt_id": prompt_id, "prompt": TEST_PROMPT}
    ...
    try:
        ...
        while time.time() - start_time < TEST_TIMEOUT:
            ...
            response_files = list(
                Path("runtime/bridge_outbox/test").glob(f"*{prompt_id}*.json")
            )
            if response_files:
        ...
        if prompt_file.exists():
            prompt_file.unlink()
        for response_file in Path("runtime/bridge_outbox/test").glob(
            f"*{prompt_id}*.json"
        ):
...
def test_bridge_config_loading(bridge_config):
    ...
    assert (
        app_config.paths.task_list_file_for_bridge
        == "runtime/tasks/bridge_test_tasks.json"
    )
