...
class AgentLoop:
    ...
    async def _validate_completed_tasks(self) -> None:
        ...
        for task_id, task in self.agent._active_tasks.items():
            if task.get("status") == "completed":
                await self._validate_task_completion(
                    task_id, task, task.get("result", {})
                )

    async def _validate_task_completion(
        self, task_id: str, task: Dict[str, Any], result: Dict[str, Any]
    ) -> None:
        ...
        validation_result = self.validator.validate_improvement(
            improvement_id=task_id, **validation_data
        )
    ...
    async def _log_validation_violation(
        self, task_id: str, validation_result: ValidationResult
    ) -> None:
        ...
        if self.agent_bus:
            await self.agent_bus.publish(
                "thea.validation.escalation",
                {
                    ...
                    "timestamp": datetime.utcnow().isoformat(),
                    "validation_result": validation_result.details,
                    "validation_history": self.agent._active_tasks[task_id].get(
                        "validation_history", []
                    ),
                },
            )
    ...
    async def _handle_task_error(self, task_id: str, error: Exception) -> None:
        ...
        if self.pbm:
            await self.pbm.update_task_status(
                task_id=task_id, status="error", details={"error": str(error)}
            )
        ...
        if self.agent_bus:
            await self.agent_bus.publish(
                "agent.error",
                {
                    ...
                    "error_type": type(error).__name__,
                    "error_message": str(error),
                    "timestamp": self.agent._active_tasks[task_id]["error"][
                        "timestamp"
                    ],
                },
            )
