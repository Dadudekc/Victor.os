...
class MemoryManager:
    ...
    def retrieve_recent(self, count: int = 5) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            sorted_items = sorted(
                self._memory_items, key=lambda x: x.get("timestamp", ""), reverse=True
            )
        ...
        try:
            with self._lock:
                ...
                try:
                    ...
                    logger.info(
                        f"Successfully loaded {len(self._memory_items)} memory items"
                    )
                    return True
                except Exception as e:
        ...
        try:
            with self._lock:
                ...
                try:
                    ...
                    if file_path.exists():
                        logger.info(
                            f"Successfully saved {len(self._memory_items)} memory items"
                        )
                        return True
                    else:
    ...
    def _compact_memory(self) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Compacting memory from {len(self._memory_items)} items to {self._max_items} items"
            )

            # Sort by timestamp (newest first)
            self._memory_items.sort(key=lambda x: x.get("timestamp", ""), reverse=True)
    ...
    def archive_old_memories(
        self, archive_path: Path, older_than_days: int = 30
    ) -> bool:
        ...
        try:
            with self._lock:
                logger.info(
                    f"Archiving memories older than {older_than_days} days to {archive_path}"
                )
                ...
                if archive_items:
                        ...
                        try:
                            with open(archive_path, "r", encoding="utf-8") as f:
                                ...
                                if isinstance(archive_data, list):
                                    ...
                                    and "memory_items" in archive_data
                                ):
                    ...
                    logger.info(
                        f"Archived {len(archive_items)} items, keeping {len(current_items)} items"
                    )
                    return True
                else:
