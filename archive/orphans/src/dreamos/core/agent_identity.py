...
class AgentIdentity:
    ...
    def _initialize_identity(self) -> Dict:
        ...
        try:
            ...
            identity = {
                ...
                "core_values": {
                    value: 1.0 for value in ethos.get("core_values", {}).keys()
                },
                "principles": {
                    principle: 1.0
                    for principle in ethos.get("operational_principles", {}).keys()
                },
            ...
            self._log_identity_update(
                "Initialized agent identity with ethos version "
                + identity["ethos_version"]
            )
    ...
    def _log_identity_update(self, message: str, **additional_data) -> None:
        ...
        try:
            ...
            log_data = {"identity_state": self.identity_data}
            ...
            if "score_delta" in additional_data:
                ...
                try:
                    with open(log_file, "a") as f:
                        ...
                        f.write(
                            f"  Previous: {self.previous_score:.1f}, Current: {self.identity_data['empathy_score']:.1f}, Delta: {score_delta:.1f}\n"
                        )
                        ...
                        if len(self.score_history) >= 3:
                            recent_deltas = [
                                entry["delta"] for entry in self.score_history[-3:]
                            ]
                            avg_delta = sum(recent_deltas) / len(recent_deltas)
                            trend = (
                                "improving"
                                if avg_delta > 0
                                else "declining" if avg_delta < 0 else "stable"
                            )
                            f.write(
                                f"  Trend: {trend} (avg Δ: {avg_delta:.1f} over last 3 updates)\n"
                            )
    ...
    def _escalate_violations(self, violations: List[str]) -> None:
        ...
        logger.warning(
            f"ESCALATING {len(violations)} ethos violations for agent {self.agent_id}"
        )
    ...
    def get_identity_summary(self) -> Dict:
        ...
        return {
            ...
            "validation_status": (
                self.identity_data["validation_history"][-1]["valid"]
    ...
    def get_score_history(self, limit: int = 10) -> List[Dict]:
        ...
        """
        return (
            self.score_history[-limit:]
            if len(self.score_history) > limit
