...
class EmpathyScorer:
    ...
    def _parse_log_content(self, content: str) -> Dict[str, Any]:
        ...
        metrics = {
            "loop_duration": (
                float(
                    re.search(
                        r"(?:\*\*Loop Duration\*\*|Loop Duration): ([\d.]+)", content
                    ).group(1)
                )
                if re.search(
                    r"(?:\*\*Loop Duration\*\*|Loop Duration): ([\d.]+)", content
            ...
            "reflection_gap": (
                float(
                    re.search(
                        r"(?:\*\*Reflection Gap\*\*|Reflection Gap): ([\d.]+)", content
                    ).group(1)
                )
                if re.search(
                    r"(?:\*\*Reflection Gap\*\*|Reflection Gap): ([\d.]+)", content
            ...
            "task_complexity": (
                float(
                    re.search(
                        r"(?:\*\*Task Complexity\*\*|Task Complexity): ([\d.]+)",
                        content,
                    ).group(1)
                )
                if re.search(
                    r"(?:\*\*Task Complexity\*\*|Task Complexity): ([\d.]+)", content
                )
                else 0.0
            ),
            "compliance_score": (
                float(
                    re.search(
                        r"(?:\*\*Compliance Score\*\*|Compliance Score): ([\d.]+)",
                        content,
                    ).group(1)
                )
                if re.search(
                    r"(?:\*\*Compliance Score\*\*|Compliance Score): ([\d.]+)", content
                )
                else 0.0
            ),
        }
    ...
    def _create_default_score(self, agent_id: str) -> Dict:
        ...
        return {
            ...
            "frequency": {"violation_rate": 0.0, "compliance_rate": 1.0},
            "trend": {"overall": 0.0, "weekly": 0.0, "daily": 0.0},
            "recovery": {
                "recovery_attempts": 0,
                "successful_recoveries": 0,
                "recovery_rate": 0.0,
            },
            "context": {"awareness_score": 0.0, "context_metrics": {}},
            "weighted_components": {
                "core_values": 75.0,
                "frequency": 100.0,
                "trend": 0.0,
    ...
    def _calculate_frequency_metrics(self, logs: List[Dict]) -> Dict:
        ...
        return {
            "violation_rate": (
                violation_count / total_entries if total_entries > 0 else 0.0
            ),
            "compliance_rate": (
                compliance_count / total_entries if total_entries > 0 else 1.0
            ),
        ...
        for period_name, period_days in periods.items():
            ...
            for log, days_ago in period_logs:
                ...
                if log["type"] == "violation":
                    ...
                    severity = log.get("severity", "low")
                    severity_weight = 1.0 - WEIGHTS["violation_severity"].get(
                        severity, 0.95
                    )
    ...
    def _calculate_context_metrics(self, logs: List[Dict]) -> Dict:
        ...
        context_metrics = {"awareness_score": 0.0, "context_metrics": {}}

        # Extract context awareness scores
        awareness_scores = [log.get("context_awareness", 0.0) for log in logs]

        if awareness_scores:
            context_metrics["awareness_score"] = sum(awareness_scores) / len(
                awareness_scores
            )
    ...
    def _calculate_weighted_score(
        ...
        weighted_total = sum(
            score * weights[component] for component, score in components.items()
        )

        return {"total": round(weighted_total, 1), "components": components}
    ...
    def _generate_score_summary(
        self, agent_id: str, weighted_score: Dict, trend_metrics: Dict
    ) -> str:
    ...
    def calculate_all_agent_scores(
        self, agent_ids: List[str], days: int = 30
    ) -> Dict[str, Dict]:
    ...
    def get_agent_comparisons(self, agent_ids: List[str]) -> Dict:
        ...
        avg_score = (
            sum(data["score"] for data in scores.values()) / len(scores)
            if scores
            else 0
        )
        ...
        for category in categories:
            if scores:
                leader = max(
                    scores.items(), key=lambda x: x[1]["weighted_components"][category]
                )
