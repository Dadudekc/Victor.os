...
class PredictiveModel:
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.compliance_predictor = joblib.load(
                self.model_dir / "compliance_predictor.joblib"
            )
    ...
    def _save_models(self):
        ...
        joblib.dump(
            self.compliance_predictor, self.model_dir / "compliance_predictor.joblib"
        )
    ...
    def extract_features(self, agent_id: str, action_data: Dict) -> Dict[str, float]:
        ...
        if (
            agent_id in self.feature_history
            and len(self.feature_history[agent_id]) >= 5
        ):
    ...
    def _calculate_confidence(
        self, agent_id: str, current_features: Dict[str, float]
    ) -> float:
        """Calculate prediction confidence based on feature stability."""
        if (
            agent_id not in self.feature_history
            or len(self.feature_history[agent_id]) < 5
        ):
            return 0.5
    ...
    def _generate_warning(
        self, drift_score: float, compliance_prob: float, confidence: float
    ) -> Optional[str]:
    ...
    def get_agent_insights(self, agent_id: str) -> Dict:
        ...
        """
        if (
            agent_id not in self.prediction_history
            or len(self.prediction_history[agent_id]) < 5
        ):
            return {
                "drift_trend": 0.0,
        ...
        drift_trend = np.polyfit(
            range(len(predictions)), [p["drift_score"] for p in predictions], 1
        )[0]
        ...
        drift_pattern = (
            ...
            else "improving" if drift_trend > 0 else "degrading"
        )
        compliance_pattern = (
            "stable"
            if abs(compliance_trend) < 0.1
            else "improving" if compliance_trend > 0 else "degrading"
        )
