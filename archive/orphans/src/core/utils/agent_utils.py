...
from typing import (  # Added Set for potential use in AgentMonitor
    Any,
    Dict,
    Optional,
    Set,
...
class AgentMonitor:
    ...
    def log_action(self, action: str, details: Optional[Dict[str, Any]] = None) -> None:
        ...
        log_file = (
            self.log_dir / f"actions_{datetime.utcnow().strftime('%Y-%m-%d')}.log.yaml"
        )
        with open(log_file, "a") as f:
            yaml.dump(
                [log_entry], f
            )  # Store as a list item for easier YAML stream reading

    def start_task(self, task_id: str) -> None:
        """Mark a task as started."""
        self.active_tasks.add(task_id)
        self.log_action(
            "task_started", {"task_id": task_id, "active_count": len(self.active_tasks)}
        )
    ...
    def complete_task(self, task_id: str) -> None:
        ...
        self.log_action(
            "task_completed",
            {"task_id": task_id, "active_count": len(self.active_tasks)},
        )
...
class CommandSupervisor:
    ...
    def update_command_status(
        self, command_id: str, status: str, result: Optional[Any] = None
    ) -> bool:
