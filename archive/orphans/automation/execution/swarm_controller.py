...
class SwarmController:
    ...
    def __init__(
        ...
        self.workers: List[
            GUIController
        ] = []  # Still uses GUIController type hint despite missing file?
        ...
        try:
            ...
            logger.info(
                f"SwarmController agent registry populated with {len(self.agents)} agents."
            )
        except Exception as factory_err:
            logger.critical(
                f"Failed to initialize agents via AgentFactory: {factory_err}",
                exc_info=True,
            )
            # Decide if agent loading failure is fatal
            raise RuntimeError(
                f"AgentFactory failed during initialization: {factory_err}"
            ) from factory_err
        # EDIT END
