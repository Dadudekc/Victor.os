...
class TestPyAutoGUIControlModulePart2(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_type_text_press_enter_true(self):
        ...
        with patch.object(
            self.module, "_press_key", return_value=None
        ) as mock_press_key:
    ...
    async def test_type_text_clear_before_typing_true(self):
        ...
        with patch.object(
            self.module, "_press_hotkey", return_value=None
        ) as mock_press_hotkey, patch.object(
            self.module, "_press_key", return_value=None
        ) as mock_press_key:
    ...
    async def test_press_hotkey_success_single_key_passthrough(self):
        ...
        hotkey_call_args_list = [
            call.args[0]
            for call in self.mock_run_blocking_io.call_args_list
            if hasattr(call.args[0], "__name__") and call.args[0].__name__ == "hotkey"
        ]
    ...
    async def test_press_key_multiple_presses(self):
        ...
        self.mock_run_blocking_io.assert_any_call(
            self.mock_pyautogui.press, "down", presses=3
        )
    ...
    async def test_get_clipboard_text_exception_propagated(self):
        ...
        async def mock_run_blocking_io_paste_fail(*args, **kwargs):
            if args[0] == self.mock_pyperclip.paste:
                raise self.mock_pyperclip.PyperclipException("Paste failed")
            return None
        ...
        self.assertIsInstance(
            cm.exception.__cause__, self.mock_pyperclip.PyperclipException
        )
    ...
    async def test_set_clipboard_text_non_string_converted(self):
        ...
        self.mock_run_blocking_io.assert_any_call(
            self.mock_pyperclip.copy, expected_string
        )
    ...
    async def test_set_clipboard_text_exception_propagated(self):
        ...
        async def mock_run_blocking_io_copy_fail(*args, **kwargs):
            if args[0] == self.mock_pyperclip.copy:
                raise self.mock_pyperclip.PyperclipException("Copy failed")
            return None
    ...
    async def test_find_type_and_enter_success(self):
        ...
        with patch.object(
            self.module, "click_element", return_value=None
        ) as mock_click, patch.object(
            self.module, "type_text", return_value=None
        ) as mock_type:
    ...
    async def test_find_type_and_enter_image_not_found(self):
        ...
        with patch.object(
            self.module, "click_element", side_effect=ImageNotFoundError("Not found")
        ) as mock_click, patch.object(self.module, "type_text") as mock_type:
            result = await self.module.find_type_and_enter(image_path, text_to_type)
    ...
    async def test_find_type_and_enter_click_fails(self):
        ...
        with patch.object(
            self.module,
            "click_element",
            side_effect=PyAutoGUIActionFailedError("Click failed"),
        ) as mock_click, patch.object(self.module, "type_text") as mock_type:
            result = await self.module.find_type_and_enter(image_path, text_to_type)
            self.assertFalse(result)
            mock_click.assert_awaited_once()  # Ensure click was attempted
    ...
    async def test_find_type_and_enter_type_fails(self):
        ...
        with patch.object(
            ...
            self.module,
            "type_text",
            side_effect=PyAutoGUIActionFailedError("Type failed"),
        ) as mock_type:
            result = await self.module.find_type_and_enter(image_path, text_to_type)
            self.assertFalse(result)
            mock_click.assert_awaited_once()  # Ensure click was attempted
    ...
    async def test_find_click_select_all_copy_success(self):
        ...
        with patch.object(
            ...
            self.module, "_press_hotkey"
        ) as mock_press_hotkey, patch.object(
            self.module, "get_clipboard_text", return_value=expected_clipboard_text
        ) as mock_get_clip, patch.object(
            self.module, "_set_clipboard_text", return_value=None
        ) as mock_set_clip:  # Mock set too for priming
    ...
    async def test_find_click_select_all_copy_with_options(self):
        ...
        with patch.object(
            ...
            self.module, "_set_clipboard_text", return_value=None
        ) as mock_set_clip, patch("asyncio.sleep") as mock_sleep:
            result = await self.module.find_click_select_all_copy(
                ...
                timeout=custom_timeout,
                offset=custom_offset,
                pause_after_copy_sec=pause_duration,
            )
            self.assertEqual(result, expected_clipboard_text)
    ...
    async def test_find_click_select_all_copy_image_not_found(self):
        ...
        with patch.object(
            ...
            self.module, "get_clipboard_text"
        ) as mock_get_clip:
            result = await self.module.find_click_select_all_copy(image_path)
    ...
    async def test_find_click_select_all_copy_clipboard_fails(self):
        ...
        with patch.object(
            ...
            self.module, "get_clipboard_text", side_effect=ClipboardError("Get failed")
        ) as mock_get_clip, patch.object(
            self.module, "_set_clipboard_text", return_value=None
        ) as mock_set_clip:  # Mock set too for priming
            result = await self.module.find_click_select_all_copy(image_path)
    ...
    async def test_capture_region_success_with_filename(self):
        ...
        with patch.object(Path, "mkdir") as mock_mkdir:
            returned_path = await self.module.capture_region(
                test_region, filename=filename
            )

            mock_mkdir.assert_called_once_with(parents=True, exist_ok=True)
            self.mock_run_blocking_io.assert_any_call(
                self.mock_pyautogui.screenshot, region=test_region
            )
    ...
    async def test_capture_region_success_no_filename_returns_object(self):
        ...
        self.assertIs(
            returned_image, mock_screenshot_obj
