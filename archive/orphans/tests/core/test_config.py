...
from dreamos.core.config import (
    AppConfig,
    CoreConfigurationError,  # import CoreConfigurationError
    get_config,
    load_config,
)
    ...
    def _create(filename: str, data: Optional[Dict] = None, malformed: bool = False):
        ...
        if data is not None:
            with open(file_path, "w") as f:
                if malformed:
                    f.write(
                        "logging: {level: DEBUG : # Unbalanced bracket"
                    )  # Invalid YAML
                else:
                    yaml.dump(data, f)
...
def test_get_config_loads_default_if_available(
    ...
    test_default_path = create_test_yaml_file(
        "default_config_for_test.yaml", sample_config_data
    )
...
def test_load_config_specific_path(
    clear_global_config_singleton, create_test_yaml_file, sample_config_data
):
...
def test_get_config_returns_same_instance(
    ...
    test_default_path = create_test_yaml_file(
        "default_config_for_test_singleton.yaml", sample_config_data
    )
    monkeypatch.setattr("dreamos.core.config.DEFAULT_CONFIG_PATH", test_default_path)
...
def test_load_config_no_file_loads_defaults(clear_global_config_singleton, monkeypatch):
    ...
    monkeypatch.setattr(
        "dreamos.core.config.DEFAULT_CONFIG_PATH", Path("path/that/does/not/exist.yaml")
    )
