...
def sample_compliance_report():
    ...
    return {
        ...
        "metrics": {"human_agency": 0.85, "transparency": 0.92, "safety": 0.78},
        "recommendations": [
...
class TestDevlogFormatter:
    ...
    def test_format_and_write_violation(
        self, formatter, temp_log_dir, sample_violation
    ):
    ...
    def test_get_recent_violations(self, formatter, temp_log_dir):
        ...
        for i in range(15):
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(
                    f"Test violation {i}"
                )  # Content is "Test violation 0", "Test violation 1", ...
    ...
    def test_get_compliance_history(self, formatter, temp_log_dir):
        ...
        with patch("dreamos.core.devlog_formatter.datetime") as mock_datetime_module:
            ...
            mock_datetime_module.fromtimestamp = (
                datetime.fromtimestamp
            )  # Keep original fromtimestamp
            ...
            for i in range(10):  # i from 0 (today) to 9 (9 days ago)
                ...
                file_path = (
                    temp_log_dir / f"compliance_{filename_timestamp_str}_{i:02d}.md"
                )

                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(
                        f"Test compliance {i} (Date: {file_date_target.isoformat()})"
                    )

                # Set the modification time to the start of the target day
                mtime_target = datetime.combine(
                    file_date_target, datetime.min.time()
                ).timestamp()
            ...
            assert (
                len(history) == 7
            ), f"Expected 7 files, got {len(history)}. Cutoff date used by formatter: {mock_now.date() - timedelta(days=7-1)}"

            # Test ordering (newest first)
            # File for i=0 (2024-03-15) should be first in history
            assert (
                f"Test compliance 0 (Date: {(mock_now - timedelta(days=0)).date().isoformat()})"
                in history[0]
            )
            # File for i=6 (2024-03-09) should be last in the 7-day history
            assert (
                f"Test compliance 6 (Date: {(mock_now - timedelta(days=6)).date().isoformat()})"
                in history[6]
            )
    ...
    @pytest.mark.skip(
        reason="os.chmod for directory read-only is unreliable on Windows for preventing file creation within."
    )
