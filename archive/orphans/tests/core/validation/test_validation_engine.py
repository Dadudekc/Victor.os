...
def validation_engine(mock_config, mock_ethos_validator, mock_event_bus):
    ...
    with (
        patch(
            "dreamos.core.validation.validation_engine.EthosValidator",
            return_value=mock_ethos_validator,
        ),
        patch(
            "dreamos.core.validation.validation_engine.AgentBus.get_instance",
...
async def test_validate_task_success(validation_engine, mock_ethos_validator):
    ...
    # Setup
    task_data = {"task_id": "test_task", "type": "test_type", "data": {"key": "value"}}

    mock_ethos_validator.validate_task.return_value = ValidationResult(
        is_valid=True, issues=[], warnings=[], context={}, timestamp=datetime.now()
    )
...
@pytest.mark.asyncio
async def test_validate_task_failure(
    validation_engine, mock_ethos_validator, mock_event_bus
):
...
async def test_validate_agent_improvement_success(validation_engine):
    ...
    improvement_data = {
        "type": "performance",
        "metrics": {"accuracy": 0.95, "speed": "2x faster"},
        "demonstration": {"before": {"accuracy": 0.85}, "after": {"accuracy": 0.95}},
    }

    # Execute
    result = await validation_engine.validate_agent_improvement(
        agent_id, improvement_data
    )
