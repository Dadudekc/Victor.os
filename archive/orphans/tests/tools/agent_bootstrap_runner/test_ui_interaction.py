...
class TestAgentUIInteractor:
    ...
    def test_initialization_missing_files(self, mock_config, mock_logger, monkeypatch):
        ...
        monkeypatch.setattr(
            mock_config, "coords_file", Path("/nonexistent/coords.json")
        )
        monkeypatch.setattr(
            mock_config, "copy_coords_file", Path("/nonexistent/copy.json")
        )
    ...
    async def test_inject_prompt_success(self, mock_config, mock_logger, mock_bus):
        ...
        assert any(event[0] == "agent.inject.ok" for event in mock_bus.published_events)
    ...
    async def test_inject_prompt_failure(self, mock_config, mock_logger, mock_bus):
        ...
        assert any(
            event[0] == "agent.inject.fail" for event in mock_bus.published_events
        )
    ...
    async def test_retrieve_response_success(self, mock_config, mock_logger, mock_bus):
        ...
        # Verify event was published
        assert any(
            event[0] == "agent.retrieve.ok" for event in mock_bus.published_events
        )
    ...
    async def test_retrieve_response_max_retries(
        self, mock_config, mock_logger, mock_bus
    ):
        ...
        # Verify error event was published
        assert any(
            event[0] == "agent.retrieve.fail" for event in mock_bus.published_events
        )
