...
class ValueCompliance(BaseCompliance):
    ...
    def _check_collaboration(
        ...
        if "decision_type" in action:
            if action["decision_type"] == "high_stakes" and not action.get(
...
class PrincipleCompliance(BaseCompliance):
    ...
    def _check_uncertainty_handling(
        ...
        if action.get("confidence", 1.0) < 0.8:
            ...
            return "uncertainty_handling" in action and action[
    ...
    def _check_continuous_learning(
        ...
        return "learning_opportunity" in action and "feedback_mechanism" in action

    def _respects_preferences(
        self, action: Dict[str, Any], preferences: Dict[str, Any]
    ) -> bool:
