...
def copy_thea_reply(
    ...
    if not PYAUTOGUI_AVAILABLE or not PYPERCLIP_AVAILABLE:
        logger.error("Cannot copy THEA reply: pyautogui or pyperclip is not available.")
        return None
    ...
    if (
        config
        and hasattr(config, "gui_automation")
        and config.gui_automation
        and hasattr(config.gui_automation, "thea_copy")
    ):
    ...
    anchor_img_rel_path = getattr(
        thea_cfg, "anchor_image_path", "assets/thea_reply_anchor.png"
    )
    ...
    if not anchor_image_abs_path.exists():
        logger.error(
            f"Anchor image for copy_thea_reply not found: {anchor_image_abs_path}"
        )
        return None
    ...
    for attempt in range(retry_count + 1):
        try:
            ...
            if anchor_location:
                ...
                logger.warning(
                    f"Anchor image not found on screen (Attempt {attempt + 1})"
                )

        except pyautogui.PyAutoGUIException as e:
            logger.error(
                f"PyAutoGUI error during copy_thea_reply (Attempt {attempt + 1}): {e}"
            )
        except Exception as e:
            logger.error(
        ...
        if attempt < retry_count:
            logger.debug("Retrying copy_thea_reply after delay...")
