...
class AzureBlobClient:
    def __init__(self):
        ...
        try:
            ...
            conn_str_secret = (
                config.integrations.azure_blob.connection_string
                if hasattr(config.integrations, "azure_blob")
                else None
            )
            self.connection_string = (
                conn_str_secret.get_secret_value() if conn_str_secret else None
            )

            if not self.connection_string:
                logger.warning(
                    "Azure Blob connection string not found in config. Client will be non-functional."
                )
            else:
            ...
            logger.error(
                f"Failed to initialize Azure Blob client using get_config: {e}",
                exc_info=True,
            )
        ...
        if not self._functional:
            logger.warning(
                "AzureBlobClient is non-functional after attempting config load."
            )
