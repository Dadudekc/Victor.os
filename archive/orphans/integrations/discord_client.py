...
class DiscordClient:
    def __init__(self):
        ...
        try:
            config = get_config()
            self.bot_token = (
                config.integrations.discord.bot_token.get_secret_value()
                if config.integrations.discord.bot_token
                else None
            )
            self.webhook_url = (
                config.integrations.discord.webhook_url
                if hasattr(config.integrations.discord, "webhook_url")
                else None
            )

            if not self.bot_token and not self.webhook_url:
                logger.warning(
                    "Discord bot token and webhook URL not found in config. Client will be non-functional."
                )
            else:
                self._functional = True
                logger.info(
                    f"DiscordClient initialized (Token: {'set' if self.bot_token else 'unset'}, Webhook: {'set' if self.webhook_url else 'unset'})"
                )

        except Exception as e:
            logger.error(
                f"Failed to initialize DiscordClient using get_config: {e}",
                exc_info=True,
            )
