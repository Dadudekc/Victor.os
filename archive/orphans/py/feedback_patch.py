...
)  # Go up two levels for bridge dir
...
try:
    ...
    logger.error(
        f"FATAL: Could not load feedback schema from {SCHEMA_PATH}: {e}. Exiting."
    )
    exit(1)
            ...
            for i, line in enumerate(f):
                try:
                    if line.strip():
                        ...
                        record["_original_line"] = (
                            i  # Store original line number for updating
                        )
                        data.append(record)
                except json.JSONDecodeError as e:
                    logger.error(
                        f"Skipping invalid JSON line in {file_path} at line {i+1}: {e}"
                    )
    ...
    if "required" in schema:
        for key in schema["required"]:
            if key not in patched_data:
                default_value = None
                if key == "timestamp":
                    default_value = datetime.now(timezone.utc).isoformat()
                elif key == "status":
                    default_value = "error"  # Default to error if missing
                elif key == "result":
                    default_value = {"message": "Result missing, patched with default."}
                elif key == "request_id":
                    default_value = "unknown_request_id_patched"
                elif key == "command_type":
                    default_value = "unknown_command_patched"
                # Add other defaults based on schema if needed

                if default_value is not None:
                    patched_data[key] = default_value
                    logger.info(
                        f"Patched missing required field '{key}' with default: {default_value}"
                    )
                    is_patched = True
                else:
                    logger.warning(
                        f"Missing required field '{key}' and no default defined."
                    )
                    # Mark as unpatchable for now?

    # Validate/fix enum fields (e.g., status)
    if (
        "status" in patched_data
        and "properties" in schema
        and "status" in schema["properties"]
    ):
        status_enum = schema["properties"]["status"].get("enum")
        if status_enum and patched_data["status"] not in status_enum:
            logger.warning(
                f"Invalid status '{patched_data['status']}'. Resetting to 'error'."
            )
...
def patch_feedback_anomalies():
    ...
    if not anomaly_data:
        logger.info(
            f"Anomaly log {ANOMALY_LOG} is empty or not found. Nothing to patch."
        )
        return
    ...
    for i, anomaly in enumerate(anomaly_data):
        if patched_count >= MAX_PATCHES_PER_CYCLE:
            logger.info(
                f"Reached max patches for this cycle ({MAX_PATCHES_PER_CYCLE})."
            )
            break
        ...
        if (
            anomaly.get("status") == "rejected"
            and anomaly.get("rejected_by") == "HEXMIRE"
        ):
            logger.info(
                f"Skipping anomaly rejected by HEXMIRE: {anomaly.get('original_filename')}"
            )
            continue
        ...
        if not original_filename:
            logger.error(
                f"Anomaly record missing 'original_filename'. Cannot process. Record: {anomaly}"
            )
            continue
        ...
        try:
            ...
            patched_payload, success = apply_schema_defaults_and_cleanup(
                feedback_payload, feedback_schema
            )
            ...
            if success:
                ...
                logger.error(
                    f"Failed to patch {original_filename}. Schema validation failed after cleanup attempts."
                )
            ...
            logger.error(
                f"Feedback file {original_filename} listed in anomaly log not found in {FEEDBACK_DIR}."
            )
            ...
            logger.error(
                f"Could not decode JSON from {original_filename}: {e}. Cannot patch this file currently."
            )
            ...
            logger.error(
                f"Unexpected error processing {original_filename}: {e}", exc_info=True
            )
        ...
        for record in anomaly_data:
            ...
            if line_num not in processed_lines:
                ...
                if current_index < len(updated_data):
                    ...
                    logger.error(
                        "Index mismatch while reconstructing anomaly log - potential data loss."
                    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(ANOMALY_LOG):
        ...
        mock_anomalies = [
            {
                "original_filename": "feedback_malformed_test_001.json",
                "error_type": "Missing Timestamp",
                "details": "timestamp field absent",
                "patched": False,
            },
            {
                "original_filename": "feedback_malformed_test_002.json",
                "error_type": "Invalid Status",
                "details": "status field was 'PENDING'",
                "patched": False,
            },
            {
                "original_filename": "feedback_already_patched_003.json",
                "error_type": "N/A",
                "details": "Previously patched example",
                "patched": True,
            },
            {
                "original_filename": "feedback_rejected_hexmire_004.json",
                "error_type": "Validation Failed",
                "details": "Schema validation failed structurally",
                "status": "rejected",
                "rejected_by": "HEXMIRE",
                "patched": False,
            },
            {
                "original_filename": "feedback_needs_patch_005.json",
                "error_type": "Missing Result",
                "details": "result field absent",
                "patched": False,
            },
            {
                "original_filename": "feedback_needs_patch_006.json",
                "error_type": "Missing Request ID",
                "details": "request_id absent",
                "patched": False,
            },  # Example for >5 limit
        ]
        ...
        with open(
            os.path.join(FEEDBACK_DIR, "feedback_malformed_test_001.json"), "w"
        ) as f:
            json.dump(
                {
                    "request_id": "req-001",
        ...
        with open(
            os.path.join(FEEDBACK_DIR, "feedback_malformed_test_002.json"), "w"
        ) as f:
            json.dump(
                {
                    "request_id": "req-002",
        ...
        with open(
            os.path.join(FEEDBACK_DIR, "feedback_already_patched_003.json"), "w"
        ) as f:
            json.dump(
                {
                    "request_id": "req-003",
        ...
        with open(
            os.path.join(FEEDBACK_DIR, "feedback_rejected_hexmire_004.json"), "w"
        ) as f:
            json.dump(
                {
                    "request_id": "req-004",
        ...
        with open(
            os.path.join(FEEDBACK_DIR, "feedback_needs_patch_005.json"), "w"
        ) as f:
            json.dump(
                {
                    "request_id": "req-005",
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                    "command_type": "grep_search",
                    "status": "success",
                },
                f,
            )
        with open(
            os.path.join(FEEDBACK_DIR, "feedback_needs_patch_006.json"), "w"
        ) as f:
            json.dump(
                {
                    "timestamp": datetime.now(timezone.utc).isoformat(),
