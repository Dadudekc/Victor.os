...
def run_loop(shutdown_event):
    ...
    try:
        ...
        if (
            config
            and config.cursor_executable_path
            and os.path.exists(config.cursor_executable_path)
        ):
            cursor_path = config.cursor_executable_path
            logger.info(f"Using Cursor path from AppConfig: {cursor_path}")
        else:
            logger.warning(
                "Cursor path from AppConfig is not valid or not set. Will try OS-specific fallbacks."
            )
    except Exception as e:
        logger.warning(
            f"Could not load AppConfig to get Cursor path: {e}. Will try OS-specific fallbacks."
        )

    if not cursor_path:
        logger.info(
            "Attempting to find Cursor path using OS-specific fallbacks as AppConfig was not used or path invalid."
        )
        ...
        cursor_path = next(
            (p for p in fallback_cursor_paths if os.path.exists(p)), None
        )
    # EDIT END

    if not cursor_path:
        logger.error(
            "Cursor executable not found via AppConfig or fallbacks. Please configure `cursor_executable_path` in AppConfig or ensure Cursor is in a standard location."
        )
        return
