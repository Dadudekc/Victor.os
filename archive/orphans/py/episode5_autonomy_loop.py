        ...
        for agent_id in AGENT_IDS[:1]:  # Just test the first agent
            ...
            if jarvis_available and prompt:
                ...
                try:
                    ...
                    logger.info(
                        f"JARVIS processing test: {'Success' if success else 'Failed'}"
                    )
                except Exception as e:
            ...
            try:
                ...
                if jarvis_available:
                    try:
                        ...
                        if jarvis_success:
                            ...
                            logger.warning(
                                f"JARVIS failed to process prompt for {agent_id}, falling back to classic pipeline"
                            )
                    except Exception as e:
                ...
                if response:
                    ...
                    if jarvis_success and jarvis_response:
                        jarvis_content = jarvis_response.get(
                ...
                logger.error(
                    f"[{agent_id}] Error during episode5 loop: {e}", exc_info=True
                )
...
def run_episode5_loop_with_fallback(max_retries: int = 3, dry_run: bool = False):
    ...
    while retry_count < max_retries:
        try:
            ...
            retry_count += 1
            logger.error(
                f"Episode5 loop failed (retry {retry_count}/{max_retries}): {str(e)}"
            )
            ...
            if jarvis:
                try:
                    ...
                    logger.error(
                        "Failed to recover JARVIS, will retry with new instance"
                    )
                    jarvis = None
                    interaction_manager = None
            ...
            if retry_count >= max_retries:
                logger.warning(
                    f"Maximum retries ({max_retries}) reached, running without JARVIS"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Test connections without performing actions",
    )
