...
def robust_move_item(source_path, dest_path, is_directory, dry_run=True):
    ...
    print(
        f"Processing move: '{source_path}' -> '{dest_path}' (Directory: {is_directory})"
    )
    ...
    if not os.path.exists(dest_parent_dir):
        if dry_run:
            ...
            if not file_io.ensure_directory(Path(dest_parent_dir)):
                print(
                    f"ERROR: Failed to create destination parent directory {dest_parent_dir} using file_io. Aborting for this item."
                )
        ...
        try:
            if is_directory:
                if os.path.exists(dest_path):
                    print(
                        f"Warning: Destination directory '{dest_path}' already exists. shutil.copytree might fail or merge."
                    )
    ...
    if not run_git_command(["add", dest_path], dry_run=dry_run):
        print(
            f"Failed to git add '{dest_path}'. Aborting further git ops for this item."
        )
    ...
    if not run_git_command(git_rm_args, dry_run=dry_run):
        print(
            f"Failed to git rm '{source_path}'. The file was copied and added, but old path remains."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Robustly move files/directories and update Git tracking."
    )
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Actually execute file operations and Git commands. Defaults to dry-run.",
    )
    parser.add_argument(
        "--mapping_file",
        default=PATH_MAPPING_FILE,
        help=f"Path to the JSON mapping file. Defaults to {PATH_MAPPING_FILE}",
    )
    ...
    for old_path, new_path in path_mapping.items():
        ...
        normalized_new_path = new_path.rstrip(
            "/"
        )  # Assuming new_path for dir also implies dir
        ...
        if robust_move_item(
            normalized_old_path, normalized_new_path, is_dir, dry_run=not args.execute
        ):
