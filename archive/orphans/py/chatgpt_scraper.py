...
from dreamos.utils.ai_output_logger import (
    log_ai_output,  # Corrected import assuming location within dreamos.utils
)
...
class HybridResponseHandler:
    ...
    def parse_hybrid_response(self, raw_response: str) -> Tuple[str, dict]:
        logger.info(
            "Parsing hybrid response for narrative text and MEMORY_UPDATE JSON."
        )
...
class ResponseHandler:
    ...
    def _init_driver(self) -> uc.Chrome:
        ...
        if os.path.exists(cached_driver_path):
            ...
            logger.warning(
                "No cached ChromeDriver found. Attempting to download latest via ChromeDriverManager..."
            )
            try:
                driver_path = ChromeDriverManager().install()
                logger.info(
                    f"ChromeDriver downloaded/installed by ChromeDriverManager to: {driver_path}"
                )
                ...
                logger.error(
                    f"Error during ChromeDriver download/install: {e}", exc_info=True
                )
                raise

        logger.info(
            f"Initializing uc.Chrome with driver_executable_path: {driver_path}"
        )
        try:
    ...
    def save_cookies(self, domain: Optional[str] = None) -> None:
        try:
            ...
            logger.info(
                f"Saved {len(cookies_to_save)} cookies to: {COOKIE_FILE}"
                + (f" (filtered for {domain})" if domain else "")
            )
        except Exception as e:
    ...
    def load_cookies(self, domain: Optional[str] = None) -> bool:
        ...
        try:
            ...
            try:  # Get current domain to ensure cookies are valid for it
                ...
                logger.warning(
                    "Could not determine current domain for cookie validation."
                )
                # Proceed cautiously

            logger.info(
                f"Applying cookies (Target domain filter: {domain}, Current browser domain: {current_domain})"
            )
            for cookie in cookies:
                ...
                if (
                    current_domain
                    and not cookie_domain.endswith(current_domain)
                    and not current_domain.endswith(cookie_domain)
                ):
                    logger.warning(
                        f"Skipping cookie for domain '{cookie_domain}' - may not match current context '{current_domain}'"
                    )
                    skipped_count += 1
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not add cookie: {cookie.get('name')} for domain {cookie_domain}. Error: {e_add}"
                    )
                    skipped_count += 1

            logger.info(
                f"Finished applying cookies. Added: {loaded_count}, Skipped/Failed: {skipped_count}."
            )
            if loaded_count > 0:
    ...
    def is_logged_in(self) -> bool:
        ...
        try:
            ...
            if "chatgpt.com" not in current_url_for_check:
                logger.info(
                    f"Not on a chatgpt.com URL ('{current_url_for_check}'). Navigating to https://chatgpt.com/ for login check."
                )
            ...
            logger.warning(
                f"Could not get current URL or navigate for login check: {e}. Attempting fallback navigation to https://chatgpt.com/"
            )
            try:
                self.driver.get("https://chatgpt.com/")
                time.sleep(3)
                ...
                logger.error(
                    f"Failed navigating to https://chatgpt.com/ for login check: {e_nav}"
                )
                return False
        ...
        try:
            ...
            WebDriverWait(self.driver, 20).until(
                self._chat_box_present
            )  # Increased timeout to 20s
            logger.info(
                f"LOGIN_CHECK: Chat input detected on {self.driver.current_url} - session appears active."
            )
            return True
        except TimeoutException:
            logger.warning(
                f"LOGIN_CHECK: Chat input NOT detected on {self.driver.current_url} after 20s - session likely inactive or page issue."
            )
            try:
                logger.warning(
                    f"LOGIN_CHECK: (Timeout) Final URL was {self.driver.current_url}"
                )
            except:
                pass
            return False
        ...
        for css_selector in CHAT_INPUT_SELECTORS:  # Use the updated list
            try:
                ...
                if elements:  # Check if the list of found elements is not empty
                    logger.debug(
                        f"_chat_box_present: Found element(s) with selector: {css_selector}"
                    )
                    return True
            except (
                Exception
            ) as e:  # Catch potential errors during find_elements, though it usually returns empty list
                logger.debug(
                    f"_chat_box_present: Selector {css_selector} check failed with error: {e}"
                )
    ...
    def ensure_chat_page(self, chat_url: str):
        ...
        if not self.driver.window_handles:
            ...
            raise NoSuchWindowException(
                "Browser window is closed (detected in ensure_chat_page)."
            )
        # EDIT END
        ...
        try:
            ...
            if not isinstance(current_url, str) or not current_url.startswith(chat_url):
                logger.info(
                    f"Current URL '{current_url}' does not start with target '{chat_url}'. Navigating..."
                )
                self.driver.get(chat_url)

                # ADDED: Intermediate logging and longer sleep
                logger.info(
                    "Waiting for 5 seconds after self.driver.get() for initial load..."
                )
                time.sleep(5)
                try:
                    post_get_url = self.driver.current_url
                    logger.info(
                        f"DEBUG_NAV: URL immediately after get() and 5s sleep: {post_get_url}"
                    )
                    ...
                    logger.error(
                        f"DEBUG_NAV: Error getting URL/page source after get(): {e_debug}"
                    )
                ...
                logger.info(
                    f"Waiting up to 60s for URL to start with BASE path {base_chat_url}..."
                )
                WebDriverWait(self.driver, 60).until(
                    lambda d: d.current_url.startswith(base_chat_url)
                )
                ...
                logger.info(
                    f"Successfully navigated to URL starting with base path: {self.driver.current_url}"
                )
                # EDIT END
            else:
                logger.info(
                    f"Browser already on target URL or a sub-path: {current_url}"
                )
        except Exception as e:
            logger.error(
                f"Failed to ensure browser is on {chat_url} or subpath: {e}",
                exc_info=True,
            )
    ...
    def ensure_login_session(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Could not get current URL: {e}. Proceeding with navigation."
            )

        if not isinstance(current_url, str) or not current_url.startswith(target_url):
            logger.info(
                f"Current URL '{current_url}' is not target. Navigating to {target_url}..."
            )
            try:
                ...
                logger.error(
                    f"Failed to navigate to {target_url}: {e_nav}. Aborting login attempt."
                )
                return False
        else:
        ...
        try:
            if self.load_cookies(
                domain="chatgpt.com"
            ):  # load_cookies returns True if cookies were loaded and page refreshed
                logger.info(
                    "Cookies loaded for chatgpt.com domain. Session might be active."
                )
                # load_cookies itself should refresh; add a small wait for page to settle
                time.sleep(
                    3
                )  # Wait for page to settle after potential refresh from load_cookies
            else:
                logger.info(
                    "Cookie load failed or no relevant cookies found for chatgpt.com."
                )
            ...
            logger.error(
                f"Exception during cookie loading: {e_cookie_load}. Proceeding without cookie-based session."
            )
            if not self.driver.current_url.startswith(
                target_url
            ):  # Ensure navigation if error
                try:
                    self.driver.get(target_url)
                    time.sleep(3)
                except Exception as e_nav_retry:
                    logger.error(
                        f"Failed to navigate to {target_url} after cookie error: {e_nav_retry}. Aborting."
                    )
                    return False
        ...
        logger.warning(
            f"Session not active. MANUAL LOGIN NEEDED. Please log in on {self.driver.current_url} (should be {target_url} or its login page)."
        )
        ...
        try:
            ...
            logger.error(
                "EOFError: Cannot prompt for manual login in non-interactive environment."
            )
            return False
        ...
        if self.is_logged_in():  # is_logged_in will re-verify thoroughly
            logger.info(
                "Login confirmed by is_logged_in() on chatgpt.com after manual step."
            )
            logger.info("Attempting to save cookies (for chatgpt.com domain)...")
            self.save_cookies(
                domain="chatgpt.com"
    ...
    def send_prompt(self, prompt: str) -> bool:
        ...
        try:
            self._last_msg_count = len(self._get_message_elements())
        except Exception as e:
            logger.error(f"Error getting initial msg count: {e}")
            self._last_msg_count = 0
        ...
        for attempt in range(max_attempts):
            logger.info(
                f"Send prompt attempt {attempt + 1}/{max_attempts} on {self.driver.current_url}"
            )
            input_box = None
            ...
            if not input_box:
                logger.error(
                    f"Attempt {attempt+1}: Could not find interactable input box with any known selector."
                )
                if attempt < max_attempts - 1:
                    time.sleep(2)
                    continue
                else:
                    return False  # All attempts failed to find input box
            ...
            try:
                ...
                if len(sanitized_prompt) < len(prompt):
                    logger.warning(
                        f"Prompt was sanitized due to non-BMP characters. Original length: {len(prompt)}, Sanitized length: {len(sanitized_prompt)}"
                    )

                # EDIT START: Use JavaScript for non-textarea elements to set text content
                if input_box.tag_name.lower() != "textarea":
                    logger.debug(
                        f"Input element is '{input_box.tag_name}', using JavaScript to set innerText."
                    )
                    ...
                    logger.debug(
                        "Input element is textarea, using send_keys for prompt text."
                    )
                ...
                if input_box.tag_name.lower() == "textarea":
                    logger.debug(
                        "Input element is textarea, sending Keys.RETURN to submit."
                    )
                    input_box.send_keys(Keys.RETURN)
                else:
                    logger.debug(
                        f"Input element is {input_box.tag_name}, attempting to click Send button to submit."
                    )
                    ...
                    for btn_css in SEND_BUTTON_SELECTORS:
                        try:
                            ...
                            send_button.click()
                            logger.debug(
                                f"Clicked Send button using selector: {btn_css}"
                            )
                            ...
                            logger.debug(
                                f"Send button selector failed or not interactable: {btn_css}"
                            )
                            continue  # Try next selector

                    if not send_button_clicked:
                        logger.error(
                            "Could not find or click the Send button after sending prompt text."
                        )
                ...
                logger.warning(
                    f"Attempt {attempt + 1} failed: StaleElementReferenceException during send_keys/click. Retrying..."
                )
                if attempt < max_attempts - 1:
                    time.sleep(2)
                    continue
                else:
                    logger.error("StaleElementReferenceException on final attempt.")
                    return False
            except Exception as e_send:
                logger.error(
                    f"Attempt {attempt + 1} failed during send_keys/click: {type(e_send).__name__}",
                    exc_info=True,
                )
                if attempt < max_attempts - 1:
                    time.sleep(2)
    ...
    def fetch_response(self) -> str:
        logger.debug(
            "ResponseHandler.fetch_response() called. Assuming browser is on the correct page."
        )
        ...
        try:
            ...
            logger.debug(
                f"fetch_response: current_message_count={current_message_count}, self._last_msg_count={self._last_msg_count}"
            )
            ...
            if current_message_count > self._last_msg_count:
                ...
                logger.debug(
                    f"Fetched NEW response candidate (msg count {self._last_msg_count} -> {current_message_count}): '{response_text[:100]}...'"
                )
                return response_text
            elif (
                current_message_count > 0
                and current_message_count == self._last_msg_count
            ):
                ...
                logger.debug(
                    f"No new messages structurally (count {current_message_count}). Returning current last message text for stability check: '{last_message_text[:100]}...'"
                )
                ...
                logger.warning(
                    f"fetch_response: Message count decreased from {self._last_msg_count} to {current_message_count}. Returning empty."
                )
                return ""
        except Exception as e:
    ...
    def wait_for_stable_response(self) -> str:
        ...
        if len(initial_messages) > self._last_msg_count:
            ...
            logger.debug(
                f"wait_for_stable_response: Initial last_response_text from newest message (if any beyond prompt): '{last_response_text[:100]}...'"
            )
        elif (
            initial_messages
        ):  # E.g. _last_msg_count includes the prompt, so no "new" yet.
            last_response_text = initial_messages[-1].text.strip()
            logger.debug(
                f"wait_for_stable_response: Initial last_response_text (likely prompt): '{last_response_text[:100]}...'"
            )
        ...
        while time.time() - start_time < self.timeout:
            ...
            current_response_text = (
                self.fetch_response()
            )  # This now gets newest message if count > _last_msg_count

            if current_response_text != last_response_text:
                logger.info(
                    f"Response text changed. Old: '{last_response_text[:60]}...' New: '{current_response_text[:60]}...'. Resetting stability timer."
                )
                ...
                if (
                    current_message_elements_for_count
                    and current_message_elements_for_count[-1].text.strip()
                    == current_response_text
                ):
                    if (
                        len(current_message_elements_for_count) > self._last_msg_count
                    ):  # only update if it is indeed a new message element structurally
                        logger.debug(
                            f"Updating _last_msg_count from {self._last_msg_count} to {len(current_message_elements_for_count)} as new response text is being processed."
                        )
                        self._last_msg_count = len(current_message_elements_for_count)
                    else:
                        logger.debug(
                            f"Response text changed but message count ({len(current_message_elements_for_count)}) not greater than _last_msg_count ({self._last_msg_count}). Likely streaming into existing last message."
                        )
                else:
                    logger.debug(
                        "Could not confirm message elements for count update after response text change. _last_msg_count remains unchanged."
                    )

            elif (
                stable_start_time
                and (time.time() - stable_start_time) >= self.stable_period
            ):
                logger.info(
                    f"Response stabilized after {time.time() - stable_start_time:.2f}s: '{last_response_text[:100]}...'"
                )
                ...
                if (
                    final_message_elements
                    and final_message_elements[-1].text.strip() == last_response_text
                ):
                    self._last_msg_count = len(final_message_elements)
                    logger.debug(
                        f"Final _last_msg_count set to {self._last_msg_count} for stable response."
                    )
                ...
                log_metadata_success = {
                    ...
                    "result": "success",
                    "stability_check_duration_s": (
                        time.time() - stable_start_time if stable_start_time else -1
                    ),
                }
                log_ai_output(
        ...
        logger.warning(
            f"Response stabilization timeout after {self.timeout}s. Returning last observed response: '{last_response_text[:100]}...'"
        )
        # Final update to _last_msg_count even on timeout.
        final_message_elements_timeout = self._get_message_elements()
        if (
            final_message_elements_timeout
            and final_message_elements_timeout[-1].text.strip() == last_response_text
        ):
            self._last_msg_count = len(final_message_elements_timeout)
            logger.debug(
                f"Final _last_msg_count set to {self._last_msg_count} on timeout."
            )
    ...
    # ---------------------------
    def handle_hybrid_response(
        self, raw_response: str, prompt_manager: Any, chat_title: str = "Unknown Chat"
    ) -> None:
        """
        ...
        narrative_text, memory_update_json = hybrid_handler.parse_hybrid_response(
            raw_response
        )
    ...
    def execute_prompt_cycle(self, prompt: str, rate_limit: int = 2) -> str:
        ...
        logger.info(
            f"Login confirmed, proceeding to send prompt on current page: {self.driver.current_url}"
        )
    ...
    # ---------------------------
    def execute_prompts_on_all_chats(
        self, prompts: list, chat_list: list, rate_limit: int = 2
    ) -> dict:
        # REVISED: Replace manual login with ensure_login_session call
        if not self.ensure_login_session():
            logger.error(
                "Automated login failed. Aborting multi-chat prompt execution."
            )
            return (
                {}
            )  # Return empty dict as per existing error handling / success path with no results

        # Ensure login session should leave us on the correct page (chat.openai.com/chat)
        logger.info(
            f"Login confirmed, proceeding with multi-chat execution from page: {self.driver.current_url}"
        )
        ...
        for chat_info in chat_list:
            ...
            if not chat_url.startswith("https://chat.openai.com/"):
                logger.warning(
                    f"Chat URL {chat_url} does not start with https://chat.openai.com/. Skipping or potential error."
                )
            ...
            for idx, prompt_text in enumerate(prompts, start=1):
                ...
                if not self.send_prompt(prompt_text):
                    logger.error(
                        f"Failed to send prompt #{idx} to chat '{chat_title}'."
                    )
    ...
    def _get_message_elements(self) -> List[Any]:
        ...
        try:
            ...
            if message_elements:
                logger.debug(
                    f"_get_message_elements: Found {len(message_elements)} messages with primary selector ('{self._message_elements_selector_primary}')."
                )
                return message_elements

            # Try fallback selector if primary fails
            logger.debug(
                f"_get_message_elements: Primary selector ('{self._message_elements_selector_primary}') found no messages. Trying fallback ('{self._message_elements_selector_fallback}')."
            )
            message_elements = self.driver.find_elements(
            ...
            if message_elements:
                logger.debug(
                    f"_get_message_elements: Found {len(message_elements)} messages with fallback selector ('{self._message_elements_selector_fallback}')."
                )
            else:
                logger.debug(
                    "_get_message_elements: No messages found with fallback selector either."
                )

        except Exception as e:
            logger.error(
                f"_get_message_elements: Error finding message elements: {e}",
                exc_info=True,
            )
        return message_elements
        ...
        try:
            ...
            for element in message_elements:
                try:
                    ...
                    logger.warning(
                        "StaleElementReferenceException while getting text from a message element, skipping it."
                    )
                    continue
            ...
            logger.info(
                f"Successfully scraped conversation content (total length: {len(full_content)})."
            )
            return full_content
    ...
    def get_conversation_links(self, timeout: int = 20) -> List[dict]:
        ...
        try:
            ...
            logger.info(
                f"Waiting for the scrollable history pane: {scrollable_element_selector}"
            )
            scrollable_element = WebDriverWait(self.driver, timeout).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, scrollable_element_selector)
                )
            )
            ...
            logger.debug(
                f"History list container found ('{history_list_selector}'). Searching for links..."
            )

            # Find all conversation link elements
            link_elements = self.driver.find_elements(By.CSS_SELECTOR, link_selector)
            logger.info(
                f"Found {len(link_elements)} potential conversation link elements."
            )
            ...
            for element in link_elements:
                try:
                    ...
                    if href and title:
                        ...
                        logger.warning(
                            f"Skipping history element: Missing href ('{href}') or title ('{title}')"
                        )
                except StaleElementReferenceException:
                    logger.warning(
                        "StaleElementReferenceException while processing a history link, skipping it."
                    )
                    continue
                except Exception as e_link:
                    logger.error(
                        f"Error processing individual history link: {e_link}",
                        exc_info=False,
                    )  # Keep log clean

            logger.info(
                f"Successfully extracted {len(conversations)} conversation links."
            )

        except TimeoutException:
            logger.error(
                f"Timeout waiting for history list container ('{history_list_selector}') after {timeout}s."
            )
        except Exception as e:
    ...
    def get_available_models(self) -> List[str]:
        ...
        try:
            ...
            for selector in model_button_selectors:
                try:
                    ...
                    if (
                        "gpt" in button_text
                        or "4" in button_text
                        or "3.5" in button_text
                        or "o" in button_text
                        or "model" in button_text
                    ):
                        model_button = candidate
                        logger.debug(
                            f"Found potential model selector button with selector: {selector}"
                        )
                        break
                except TimeoutException:
                    logger.debug(
                        f"Model selector button not found/clickable with CSS: {selector}"
                    )
                    continue

            if not model_button:
                logger.error(
                    "Could not find or click the model selector button using known patterns."
                )
                try:
                    self.driver.find_element(
                        By.TAG_NAME, "body"
                    ).click()  # Close potential menus
                except:
                    pass
                return []
            ...
            try:
                ...
                logger.warning(
                    f"Minor error clicking model button to close dropdown: {close_e}. Trying body click."
                )
                try:
                    self.driver.find_element(By.TAG_NAME, "body").click()
                except:
                    logger.warning("Body click also failed to close dropdown.")

            return models
        except Exception as e:
            logger.error(
                f"General error during get_available_models: {e}", exc_info=True
            )
            try:
                self.driver.find_element(By.TAG_NAME, "body").click()  # Cleanup click
            except:
                pass
            return []
    ...
    def select_model(self, model_name: str) -> bool:
        ...
        try:
            ...
            for selector in model_button_selectors:
                try:
                    ...
                    if (
                        ...
                        model_button = candidate
                        logger.debug(
                            f"Found model selector button for selection using: {selector}"
                        )
                        break
                except TimeoutException:
                    continue
            ...
            if not model_button:
                logger.error(
                    "Could not find model selector button to open for selection."
                )
                return False
            ...
            try:
                model_element = WebDriverWait(self.driver, 5).until(
                ...
                logger.error(
                    f"Model '{model_name}' not found/clickable with XPath: {model_xpath}"
                )
            ...
            logger.error(
                f"Error during select_model for '{model_name}': {e}", exc_info=True
            )
            try:
                self.driver.find_element(By.TAG_NAME, "body").click()  # Cleanup click
            except:
            ...
            for banner in banners:
                try:
                    if banner.is_displayed():
                        logger.warning(
                            f"Rate limit or capacity banner detected: '{banner.text[:100]}...'"
                        )
                        rate_limited = True
                    ...
                    continue
            if not rate_limited:
                logger.debug("No rate limit banners detected.")
            return rate_limited
        except Exception as e:
    ...
    def prompt_with_fallback(
        self, prompt: str, preferred_models: List[str] = None, queue_fn=None
    ):
        """
        ...
        if queue_fn is None:
            ...
            def default_queue_fn(p):
                logger.error(f"QUEUE_FN (Default): Dropping prompt: {p[:100]}...")
        ...
        if not available_models:
            logger.error(
                "prompt_with_fallback: Could not discover any available models."
            )
            queue_fn(prompt)
            return ""
        ...
        for pref_lower in preferred_models_lower:
            if pref_lower in available_models_lower:
                ...
                del available_models_lower[
                    pref_lower
                ]  # Remove from available to avoid duplication
        ...
        if not priority_list_actual_case:
            logger.error(
                "prompt_with_fallback: No usable models found after filtering."
            )
            queue_fn(prompt)
            return ""
        ...
        for model_name in priority_list_actual_case:
            ...
            logger.info(
                f"Prompt sent with '{model_name}'. Waiting for stable response..."
            )
            response = self.wait_for_stable_response()

            if response:
                logger.info(
                    f"SUCCESS: Received non-empty response from '{model_name}'."
                )
            ...
            logger.warning(
                f"Model '{model_name}' returned empty response. Trying next..."
            )
...
if __name__ == "__main__":
    ...
    single_prompt = "You are my devlog assistant. Summarize the recent development work with a focus on challenges overcome and what's next."
    ...
    chat_list = [
        {
            "title": "Chat #1",
            "link": "https://chatgpt.com/c/67d7521e-acf0-8009-aed6-2748b3b49249",
        },
        {
            "title": "Chat #2",
            "link": "https://chatgpt.com/c/67d774ad-8bfc-8009-a488-6b5392f1326f",
        },
    ]
    ...
    all_chat_results = handler.execute_prompts_on_all_chats(
        prompts=prompts_to_send, chat_list=chat_list, rate_limit=2
    )
