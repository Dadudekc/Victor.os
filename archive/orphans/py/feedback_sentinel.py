            ...
            for line in f:
                if line.strip():
                    try:
                        ...
                        print(
                            f"ERROR: Skipping corrupted line in log: {line.strip()}",
                            file=sys.stderr,
                        )
        return log_data
...
def log_anomaly(filename, file_hash, reason):
    ...
    if file_hash != "<hash_calculation_failed>" and file_hash in existing_hashes:
        print(
            f"Duplicate anomaly detected (Hash: {file_hash[:8]}...). Skipping log.",
            file=sys.stderr,
        )
    ...
    log_entry = {
        "timestamp": timestamp,
        "quarantined_file": os.path.basename(filename),
        "file_hash": file_hash,
        "reason": reason,
    }
        ...
        if not os.path.exists(ALERT_FLAG_FILE):
            ...
            try:
                with open(ALERT_FLAG_FILE, "w") as f:
                    f.write(
                        f"ALERT: {recent_anomalies} anomalies detected within {ALERT_WINDOW_SECONDS}s window. Triggered at {now.isoformat()}Z by Hexmire."
                    )
            ...
            print(
                f"Alert condition met ({recent_anomalies} anomalies), but alert file already exists.",
                file=sys.stderr,
            )
...
def process_file(filepath):
    ...
    try:
        ...
        if missing_fields:
            ...
            print(
                f"Malformed payload detected: {os.path.basename(filepath)}. {reason}",
                file=sys.stderr,
            )
        ...
        print(
            f"Error processing file {os.path.basename(filepath)}: {reason}",
            file=sys.stderr,
        )
...
def quarantine_file(filepath, reason):
    ...
    try:
        ...
        print(
            f"Quarantined: {os.path.basename(filepath)} to {QUARANTINE_DIR}",
            file=sys.stderr,
        )
        ...
        print(
            f"ERROR: Failed to quarantine {os.path.basename(filepath)}: {e}",
            file=sys.stderr,
        )
...
def purge_old_logs():
    ...
    for entry in log_data:
        try:
            ...
            print(
                f"Skipping potentially malformed entry during purge: {entry}",
                file=sys.stderr,
            )
    ...
    if len(purged_log) < original_count:
        save_log(purged_log)
        print(
            f"Purged {original_count - len(purged_log)} old log entries.",
            file=sys.stderr,
        )
    else:
...
def sentinel_loop():
    ...
    while True:
        ...
        try:
            ...
            for filepath in found_files:
                ...
                if not is_valid:
                    ...
                    print(
                        f"File {os.path.basename(filepath)} validated.", file=sys.stderr
                    )
        ...
        print(
            f"Scan complete. Sleeping for {SCAN_INTERVAL_SECONDS}s...", file=sys.stderr
        )
...
def log_state_update(anomaly_entry):
    ...
    try:
        ...
        print(
            f"ERROR: Failed to write to state sync log {STATE_SYNC_LOG_FILE}: {e}",
            file=sys.stderr,
        )
    ...
    try:
        ...
        if os.path.exists(SWARM_STATE_FILE):
            try:
                ...
                print(
                    f"ERROR: Swarm state file {SWARM_STATE_FILE} corrupted. Initializing new state.",
                    file=sys.stderr,
                )
                state_data = {}
            except Exception as e:
                print(
                    f"ERROR: Failed to read swarm state {SWARM_STATE_FILE}: {e}. Initializing new state.",
                    file=sys.stderr,
                )
                state_data = {}
        else:
        ...
        if "feedback_anomalies" not in state_data:
            ...
            print(
                f"WARNING: feedback_anomalies in {SWARM_STATE_FILE} is not a list. Overwriting.",
                file=sys.stderr,
            )
            state_data["feedback_anomalies"] = []
        ...
        print(
            f"Successfully updated swarm state file: {SWARM_STATE_FILE}",
            file=sys.stderr,
        )
