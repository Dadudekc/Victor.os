...
class Strategy:
    ...
    def fetch_historical_data(self) -> pd.DataFrame:
        ...
        if bars.empty:
            self.logger.warning(
                f"⚠️ No data returned for {self.symbol} {self.timeframe}."
            )
            return pd.DataFrame()
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.copy()
        df["SMA_short"] = (
            df["close"].rolling(window=self.maShortLength, min_periods=1).mean()
        )
        df["SMA_long"] = (
            df["close"].rolling(window=self.maLongLength, min_periods=1).mean()
        )
        delta = df["close"].diff()
    ...
    def optimize_signals(
        self, df: pd.DataFrame, model_name: str = "randomforest"
    ) -> pd.DataFrame:
        """
