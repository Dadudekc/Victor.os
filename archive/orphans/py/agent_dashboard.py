        ...
        try:
            ...
            if isinstance(messages, list):
                ...
                messages = [
                    msg
                    for msg in messages
                    if msg.get("id") != task_id or not (task_to_move := msg)
                ]
            ...
            if isinstance(messages, list):
                ...
                if task:
                    ...
                    if isinstance(thea_messages, list):
                        ...
                        asyncio.create_task(
                            handle_thea_escalation(agent_id, task_id, escalation)
                        )
        except Exception as e:
                ...
                if inbox_path.exists():
                    ...
                    if isinstance(messages, list):
                        ...
                        if thea_response.get("status"):
                            update_task_status(
                                agent_id, task_id, thea_response["status"]
                            )
...
class ProjectTab(QWidget):
    def __init__(self):
        ...
        self.modules_tree.setHeaderLabels(
            ["Module", "Files", "Status", "Owner", "Coverage"]
        )
    ...
    def _get_module_coverage(self, module_name: str) -> dict:
        ...
        return self.test_coverage.get(
            module_name, {"coverage": 0.0, "lines": 0, "covered": 0}
        )
    ...
    def _populate_summary(self):
        ...
        if ana:
            ...
            self.orphaned_files_label.setText(
                f"Orphaned Files: {ana.get('orphaned_files', 0)}"
            )
            self.missing_docs_label.setText(
                f"Missing Docs: {ana.get('missing_docs', 0)}"
            )
    ...
    def _populate_modules(self):
        ...
        for module_name, module_data in modules.items():
            ...
            if show_ownership:
                agent_color = THEME["light"]["agent_colors"].get(
                    owner, THEME["light"]["default"]
                )
            ...
            for submodule, submodule_data in module_data.get("submodules", {}).items():
                sub_owner, sub_ownership = self._get_module_owner(
                    f"{module_name}.{submodule}"
                )
                sub_owner_text = (
                    f"{sub_owner} ({sub_ownership:.0f}%)" if show_ownership else ""
                )

                sub_coverage = self._get_module_coverage(f"{module_name}.{submodule}")
                sub_coverage_text = (
                    f"{sub_coverage['coverage']:.0f}%" if show_coverage else ""
                )
                ...
                if show_ownership:
                    submodule_item.setForeground(
                        3,
                        QColor(
                            THEME["light"]["agent_colors"].get(
                                sub_owner, THEME["light"]["default"]
                            )
                        ),
                    )
                ...
                if show_coverage:
                    sub_coverage_color = (
                        THEME["light"]["success"]
                        if sub_coverage["coverage"] >= 80
                        else (
                            THEME["light"]["warning"]
                            if sub_coverage["coverage"] >= 50
                            else THEME["light"]["error"]
                        )
                    )
    ...
    def update_metrics(self):
        ...
        unowned_modules = sum(
            1 for m in modules if self._get_module_owner(m)[0] == "Unknown"
        )
        unowned_percent = (
            (unowned_modules / total_modules * 100) if total_modules > 0 else 0
        )
        ...
        active_agents = len(
            set(
                owner
                for owner, _ in (self._get_module_owner(m) for m in modules)
                if owner != "Unknown"
            )
        )
    ...
    def export_to_markdown(self):
        ...
        try:
            ...
            QTimer.singleShot(
                2000, lambda: self.export_button.setText("📤 Export to Markdown")
            )
    ...
    def refresh_data(self):
        ...
        QTimer.singleShot(
            2000, lambda: self.refresh_button.setText("🔄 Refresh Analysis")
        )
...
class AgentDashboard(QMainWindow):
        ...
        if self.inbox_base.exists():
            for agent_dir in self.inbox_base.iterdir():
                if agent_dir.is_dir() and agent_dir.name != "THEA":
                    agent_tab = AgentTab(
                        agent_dir.name, self.inbox_base, self.thea_handler
                    )
    ...
    def refresh_data(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Refresh Error", f"Error refreshing dashboard data: {str(e)}"
            )
