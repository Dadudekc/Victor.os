...
def simulate_agent_bootstrap(config: AgentConfig) -> bool:
    ...
    try:
        AgentUIInteractor(logger, config)
...
def main() -> int:
    ...
    console.print(
        "[bold yellow]⚠️ Dry-run mode: No clicks or clipboard operations[/bold yellow]"
    )
    ...
    logger.info(
        f"✅ Simulation complete: {success_count}/{len(AGENTS)} agents simulated"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Dry-Run Simulator for Dream.OS Agent Bootstrap Runner"
    )
