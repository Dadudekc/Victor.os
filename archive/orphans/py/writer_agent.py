...
class ContentWriterAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str, **kwargs):
        ...
        self.model = (
            agent_config.dreamscape.agents.writer.model
            if hasattr(agent_config.dreamscape.agents, "writer")
            and hasattr(agent_config.dreamscape.agents.writer, "model")
            else "gpt-4-turbo"
        )
        self.max_tokens = (
            agent_config.dreamscape.agents.writer.max_tokens
            if hasattr(agent_config.dreamscape.agents, "writer")
            and hasattr(agent_config.dreamscape.agents.writer, "max_tokens")
            else 4096
        )
