...
class TestTaskTransformation(unittest.TestCase):
    def test_basic_future_task_transform(self):
        ...
        self.assertEqual(
            transformed["history"][0]["action"], "MIGRATED_FROM_future_tasks.json"
        )
        self.assertEqual(
            transformed["params"]["_migration_info"]["original_name"],
            "Future Task Example",
        )
    ...
    def test_basic_working_task_transform(self):
        ...
        self.assertEqual(
            transformed["started_at"], "2024-01-02T12:00:00Z"
        )  # From CLAIMED history
        self.assertIsNone(transformed["completed_at"])
        self.assertEqual(len(transformed["history"]), 2)  # Original + migration entry
        self.assertEqual(
            transformed["history"][1]["action"], "MIGRATED_FROM_working_tasks.json"
        )
        self.assertEqual(
            transformed["params"]["_migration_info"]["original_assigned_agent"],
            "Gemini",
        )
