...
ALERT_FILE_PATH = Path(
    "D:\\Dream.os\runtime\alerts\rustbyte_ping_thea.txt"
)  # Use absolute path as directed
...
)  # Alert if no update for 10 cycles
STALL_ALERT_FLAG_PATH = project_root / "runtime" / "alerts" / "module6_stall.flag"
# Stall Escalation Config
STALL_ESCALATION_THRESHOLD_CYCLES = (
    15  # Escalate if stall flag persists this many cycles
)
STALL_ESCALATION_FLAG_PATH = (
    project_root / "runtime" / "alerts" / "escalation_knurlshade_critical.flag"
)
...
# --- Swarm Sync Import ---
try:
    from dreamos.core.swarm_sync import AGENT_IDS, read_swarm_state

    logger.info("Successfully imported swarm_sync functions.")
except ImportError as e:
    logger.critical(
        f"Failed to import required swarm_sync module: {e}. Monitor cannot run."
    )
    sys.exit(1)
...
def log_integrity_issue(issue_type: str, details: Dict[str, Any]):
    ...
    try:
        ...
        logger.error(
            f"Failed to write to integrity log {INTEGRITY_LOG_FILE}: {e}", exc_info=True
        )
...
def trigger_alert(reason: str):
    ...
    if not ALERT_FILE_PATH.exists():
        logger.critical(
            f"ALERT CONDITION MET: {reason}. Triggering primary alert file: {ALERT_FILE_PATH}"
        )
        try:
            ...
            logger.error(
                f"Failed to create primary alert file {ALERT_FILE_PATH}: {e}",
                exc_info=True,
            )
    else:
        logger.warning(
            f"Primary alert condition met ({reason}), but alert file already exists: {ALERT_FILE_PATH}"
        )
...
def manage_stall_flag(is_stalled: bool):
    ...
    if is_stalled:
        if not STALL_ALERT_FLAG_PATH.exists():
            logger.warning(
                f"Stall detected for {STALL_AGENT_ID}. Creating flag file: {STALL_ALERT_FLAG_PATH}"
            )
            try:
                ...
                logger.error(
                    f"Failed to create stall flag file {STALL_ALERT_FLAG_PATH}: {e}. Log only."
                )
    else:
        if STALL_ALERT_FLAG_PATH.exists():
            logger.info(
                f"Stall condition cleared for {STALL_AGENT_ID}. Removing flag file: {STALL_ALERT_FLAG_PATH}"
            )
            try:
                STALL_ALERT_FLAG_PATH.unlink()
            except OSError as e:
                logger.error(
                    f"Failed to delete stall flag file {STALL_ALERT_FLAG_PATH}: {e}. Manual cleanup may be required."
                )
...
def trigger_escalation_flag(duration_cycles: int):
    ...
    if not STALL_ESCALATION_FLAG_PATH.exists():
        ...
        logger.critical(
            f"ESCALATION: {reason} Triggering escalation flag: {STALL_ESCALATION_FLAG_PATH}"
        )
        log_integrity_issue(
            "StallEscalation",
            {"agent_id": STALL_AGENT_ID, "duration_cycles": duration_cycles},
        )
        try:
            ...
            logger.error(
                f"Failed to create escalation flag file {STALL_ESCALATION_FLAG_PATH}: {e}. Log only."
            )
    else:
        logger.warning(
            f"Escalation condition met, but escalation flag already exists: {STALL_ESCALATION_FLAG_PATH}"
        )
...
def run_monitor():
    ...
    """
    logger.info(
        f"Starting Swarm Monitor ({MONITOR_ID}). Interval: {MONITOR_INTERVAL_SECONDS}s"
    )
    consecutive_json_errors = 0
    knurlshade_stalled = (
        STALL_ALERT_FLAG_PATH.exists()
    )  # Initial state based on flag presence
    ...
    while True:
        ...
        if not STATE_FILE_PATH.exists():
            logger.warning(
                f"Swarm state file not found: {STATE_FILE_PATH}. Cannot monitor."
            )
            ...
            try:
                ...
                if not isinstance(current_state, dict):
                    logger.error(
                        f"Swarm state file {STATE_FILE_PATH} does not contain a valid JSON object."
                    )
                    log_integrity_issue(
                        "FormatError",
                        {"file": str(STATE_FILE_PATH), "error": "Not a JSON object"},
                    )
                ...
                logger.error(
                    f"Swarm state file {STATE_FILE_PATH} contains invalid JSON: {e}"
                )
                log_integrity_issue(
                    "JSONError", {"file": str(STATE_FILE_PATH), "error": str(e)}
                )
                state_read_error = True
            except OSError as e:
                logger.error(
                    f"Error reading swarm state file {STATE_FILE_PATH}: {e}. Treating as read error."
                )
                state_read_error = True
            except Exception as e:
                logger.error(
                    f"Unexpected error reading swarm state: {e}. Treating as read error."
                )
                state_read_error = True
        ...
        if current_state is not None:
            ...
            if missing_agents:
                ...
                log_integrity_issue(
                    "MissingAgents",
                    {
                        "file": str(STATE_FILE_PATH),
                        "missing": sorted(list(missing_agents)),
                    },
                )
            ...
            if extra_agents:
                ...
                log_integrity_issue(
                    "ExtraAgents",
                    {"file": str(STATE_FILE_PATH), "extra": sorted(list(extra_agents))},
                )

            for agent_id in expected_agents:
                if agent_id not in current_state:
                ...
                if (
                    ...
                    and agent_status.strip().lower() == "error"
                ):
                    ...
                    log_integrity_issue(
                        "AgentError",
                        {
                            ...
                            "agent_id": agent_id,
                            "status": agent_status,
                            "health_notes": agent_data.get("health_notes"),
                        },
                    )
                    if (
                        alert_reason is None
                    ):  # Prioritize error status alert over JSON errors
                        alert_reason = msg
                ...
                if agent_id == STALL_AGENT_ID:
                    ...
                    last_update_dt = (
                        parse_iso_utc(last_update_str) if last_update_str else None
                    )
                    ...
                    if last_update_dt:
                        ...
                        if (
                            ...
                            if not knurlshade_stalled:
                                ...
                                logger.warning(
                                    f"Stall detected for {STALL_AGENT_ID} ({STALL_MODULE_EXPECTED}): Last update {time_since_last_update.total_seconds():.0f}s ago."
                                )
                            ...
                            if knurlshade_stalled:
                                logger.info(
                                    f"Stall condition cleared for {STALL_AGENT_ID}. Update received."
                                )
                                log_integrity_issue(
                                    "StallResumed",
                                    {
                        ...
                        logger.warning(
                            f"Could not parse last_updated_utc for {STALL_AGENT_ID}. Cannot perform stall check."
                        )
                        log_integrity_issue(
                            "SchemaError",
                            {
                                "agent_id": STALL_AGENT_ID,
                                "error": "Missing or unparseable last_updated_utc",
                            },
                        )

            if not stall_check_performed and STALL_AGENT_ID not in missing_agents:
                logger.warning(
                    f"Stall check could not be performed for {STALL_AGENT_ID} (agent present but data likely invalid). Check integrity logs."
                )

        # 3. Update Stall Counter & Check Escalation
        if (
            knurlshade_stalled
        ):  # Check internal state flag reflecting current cycle's check
            stall_flag_consecutive_cycles += 1
            logger.debug(
                f"Stall flag for {STALL_AGENT_ID} present. Consecutive cycles: {stall_flag_consecutive_cycles}"
            )
            if (
                stall_flag_consecutive_cycles >= STALL_ESCALATION_THRESHOLD_CYCLES
            ...
            if stall_flag_consecutive_cycles > 0:
                logger.info(
                    f"Stall flag for {STALL_AGENT_ID} removed. Resetting consecutive cycle count from {stall_flag_consecutive_cycles}."
                )
        ...
        logger.debug(
            f"Monitoring cycle complete. Sleeping for {MONITOR_INTERVAL_SECONDS} seconds."
        )
...
if __name__ == "__main__":
    ...
    INTEGRITY_LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
    ALERT_FILE_PATH.parent.mkdir(parents=True, exist_ok=True)
    STALL_ALERT_FLAG_PATH.parent.mkdir(
        parents=True, exist_ok=True
    )  # Ensure alerts dir exists
    STALL_ESCALATION_FLAG_PATH.parent.mkdir(
        parents=True, exist_ok=True
    )  # Ensure escalation dir exists
    ...
    try:
        ...
        logger.critical(
            f"Swarm Monitor ({MONITOR_ID}) encountered a critical error: {e}",
            exc_info=True,
        )
        sys.exit(1)
