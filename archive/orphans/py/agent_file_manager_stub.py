...
class AgentFileManager:
    ...
    def _resolve_path(self, relative_path: Union[str, Path]) -> Path:
        ...
        if (
            self.base_path.resolve() not in resolved_path.parents
            and resolved_path != self.base_path.resolve()
        ):
            # Or use os.path.commonprefix?
            raise PermissionError(
                f"Attempted path traversal outside base directory: {relative_path}"
            )
    ...
    def read_file(self, relative_path: Union[str, Path]) -> Optional[str]:
        ...
        try:
            ...
            )  # Example Logging / Error Handling
            # TODO: Refine error handling
            return None

    def write_file(
        self, relative_path: Union[str, Path], content: str, overwrite: bool = False
    ) -> bool:
        ...
        try:
            ...
            if full_path.exists() and not overwrite:
                print(
                    f"File exists and overwrite is False: {full_path}"
                )  # Example Logging
                return False
            ...
            print(
                f"Error writing file {relative_path}: {e}"
            )  # Example Logging / Error Handling
            # TODO: Refine error handling
            return False
    ...
    def read_json(self, relative_path: Union[str, Path]) -> Optional[Union[dict, list]]:
        ...
        if content is not None:
            try:
                ...
                print(
                    f"Error decoding JSON from {relative_path}: {e}"
                )  # Example Logging
                return None
        return None
    ...
    def file_exists(self, relative_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            print(
                f"Error checking existence for {relative_path}: {e}"
            )  # Example Logging
            return False
