...
formats = [
    ...
    (
        ...
        lambda now: now.replace(tzinfo=None),
    ),
    (
        "naive_utc",
        "Naive datetime assumed UTC",
        lambda now: now.astimezone(utc_tz).replace(tzinfo=None),
    ),
    (
        "iso_utc_Z",
        "Correct ISO 8601 UTC",
        lambda now: now.astimezone(utc_tz)
    ...
    (
        ...
        lambda now: now.isoformat(timespec="milliseconds"),
    ),
    (
        "rfc_3339",
        "RFC 3339 format",
    ...
    (
        "malformed_iso_no_T_no_Z",
        "ISOish without T or Z/offset",
        lambda now: now.astimezone(utc_tz)
        .isoformat(timespec="milliseconds")
        .replace("T", " ")
        .replace("+00:00", "")[:-1],
    ...
    (
        "human_readable",
        "Ambiguous human format",
        lambda now: now.strftime("%m/%d/%Y %I:%M:%S %p"),
    ),
]
...
for i in range(10):
    ...
    current_time = base_time + datetime.timedelta(
        seconds=random.randint(-60, 60), milliseconds=random.randint(-500, 500)
    )
