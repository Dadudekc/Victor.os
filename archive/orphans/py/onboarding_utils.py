...
async def affirm_onboarding_contract(
    ...
    try:
        ...
        protocol_hash = await asyncio.to_thread(
            file_io.calculate_file_sha256, onboarding_protocols_path
        )
        ...
        logger.error(
            f"Unexpected error calculating hash for {onboarding_protocols_path}: {e}",
            exc_info=True,
        )
        return False
