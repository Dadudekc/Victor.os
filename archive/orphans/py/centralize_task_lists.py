    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            if isinstance(content, list):
                ...
                logger.error(
                    f"Expected list in {file_path}, got {type(content)}. Cannot process."
                )
                return []
        ...
        try:
            ...
            if isinstance(task_board, list):
                ...
                for task in task_board:
                    ...
                    task_id = task.get(
                        "task_id", task.get("id", f"EP-{episode_id}-{len(tasks) + 1}")
                    )

                    standardized_task = {
                        "task_id": task_id,
                        "name": task.get(
                            "name", task.get("intent", f"Task from {episode_title}")
                        ),
                        "description": task.get("description", ""),
                        "status": task.get("status", "PENDING"),
                        "priority": task.get("priority", "MEDIUM"),
                        "assigned_agent": task.get(
                            "owner", task.get("assigned_to", None)
                        ),
                    ...
                    for key, value in task.items():
                        if key not in standardized_task and key not in [
                            "id",
                            "owner",
                            "intent",
                        ]:
                ...
                for task_id, task_data in task_board.items():
                    ...
                    standardized_task = {
                        ...
                        "name": task_data.get(
                            "name",
                            task_data.get("intent", f"Task from {episode_title}"),
                        ),
                        "description": task_data.get("description", ""),
                        "status": task_data.get("status", "PENDING"),
                        "priority": task_data.get("priority", "MEDIUM"),
                        "assigned_agent": task_data.get(
                            "owner", task_data.get("assigned_to", None)
                        ),
                        "_source_episode": str(episode_file),
                        "_episode_id": episode_id,
                        "_episode_title": episode_title,
            ...
            if isinstance(milestones, list):
                for milestone in milestones:
                    ...
                    milestone_id = milestone.get(
                        "id", f"MS-{episode_id}-{len(tasks) + 1}"
                    )
            ...
            logger.error(
                f"Error processing episode file {episode_file}: {e}", exc_info=True
            )
...
def normalize_task(task: Dict, source_file: str) -> Dict:
    ...
    if "task_id" not in task:
        if "id" in task:
            ...
            logger.warning(
                f"Generated task_id {task['task_id']} for task without ID from {source_file}"
            )
...
def merge_tasks(
        ...
        for task in tasks:
            try:
                ...
                if task_id in task_ids_seen:
                    logger.warning(
                        f"Duplicate task_id {task_id} found in {source_name}. Using first occurrence only."
                    )
                    error_count += 1
                    continue
                ...
                logger.error(
                    f"Error processing task from {source_name}: {e}", exc_info=True
                )
                error_count += 1
    ...
    if include_episodes:
        ...
        for task in episode_tasks:
            try:
                ...
                if task_id in task_ids_seen:
                    logger.warning(
                        f"Duplicate task_id {task_id} found in episode. Using first occurrence only."
                    )
                    error_count += 1
                    continue
    ...
    if not dry_run:
        ...
        try:
            ...
            logger.error(
                f"Failed to write merged tasks to {output_file}: {e}", exc_info=True
            )
            error_count += 1
    else:
        ...
        try:
            ...
            replacements = [
                (
                    ...
                    str(central_task_path),
                ),
                (
                    "runtime/agent_comms/project_boards/completed_tasks.json",
                    str(central_task_path),
                ),
                (
                ...
                (
                    "runtime/agent_comms/project_boards/task_ready_queue.json",
                    str(central_task_path),
                ),
            ...
            logger.error(
                f"Failed to update documentation file {doc_file}: {e}", exc_info=True
            )
            success = False
...
"""
    ...
    readme_content = readme_content.format(
        date=datetime.datetime.now().strftime("%Y-%m-%d"), backup_dir=DEFAULT_BACKUP_DIR
    )
    ...
    try:
        ...
        logger.error(
            f"Failed to create README.md in {central_task_dir}: {e}", exc_info=True
        )
        return False
...
def main():
    parser = argparse.ArgumentParser(
        description="Centralize task lists into a single location"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    # Merge tasks
    success_count, error_count = merge_tasks(
        source_files, output_file, not args.skip_episodes, args.dry_run
    )
    ...
    logger.info(
        f"Task centralization {'would have ' if args.dry_run else ''}completed with {success_count} successes and {error_count} errors"
    )
