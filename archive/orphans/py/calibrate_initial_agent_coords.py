...
def main():
    ...
    print(
        "Please ensure the agent's GUI window is visible and in its initial state (no messages sent yet)."
    )
    print(
        "IMPORTANT: PyAutoGUI's FailSafe can be triggered by moving the mouse to a screen corner."
    )
    ...
    for agent_id in AGENT_IDS:
        ...
        input(
            f"1. Ensure {agent_id}'s GUI is ready and its input box is clear. Press Enter to continue..."
        )

        print(
            "2. You have 5 seconds to move your mouse cursor to the CENTER of {agent_id}'s input box."
        )
        ...
        try:
            ...
            print(
                f"   Captured coordinates for {agent_id} input_box_initial: ({pos.x}, {pos.y})"
            )
            ...
            print(
                "   Skipping this agent. You may need to re-run the script or manually edit the JSON."
            )
...
if __name__ == "__main__":
    try:
        ...
        print(
            "Error: PyAutoGUI library is not installed. Please install it (pip install pyautogui) and try again."
        )
    except Exception as e:
