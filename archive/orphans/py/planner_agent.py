...
class PlannerAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str, **kwargs):
        ...
        )  # BaseAgent handles its own config via get_config

        agent_config = get_config()  # Get global config
        # Assuming path like: agent_config.dreamscape.agents.planner.model
        self.model = (
            agent_config.dreamscape.agents.planner.model
            if hasattr(agent_config.dreamscape.agents, "planner")
            and hasattr(agent_config.dreamscape.agents.planner, "model")
            else "gpt-4-turbo"
        )
        self.max_tokens = (
            agent_config.dreamscape.agents.planner.max_tokens
            if hasattr(agent_config.dreamscape.agents, "planner")
            and hasattr(agent_config.dreamscape.agents.planner, "max_tokens")
            else 4096
        )
        ...
        if not hasattr(self, "openai_client") or not self.openai_client:
            self.openai_client = (
                OpenAIClient()
            )  # OpenAIClient uses get_config internally now
