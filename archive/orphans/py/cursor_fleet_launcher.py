...
try:
    ...
    if (
        ...
        print(
            "Warning: Cursor path from config is not valid or not set. Trying fallbacks."
        )
except Exception as e:
    print(
        f"Warning: Could not load AppConfig to get Cursor path: {e}. Trying fallbacks."
    )
...
if not CURSOR_PATH:
    raise FileNotFoundError(
        "❌ Cursor.exe not found. Please check configuration or standard installation paths."
    )
...
def get_cursor_windows(target_count=8, timeout=30):
    ...
    while time.time() - start_time < timeout:
        ...
        current_windows = {
            w._hWnd: w
            for w in gw.getWindowsWithTitle("cursor")
...
def main():
    ...
    try:
        windows = get_cursor_windows(
            target_count=len(processes), timeout=45
