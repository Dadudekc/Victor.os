...
if PYAUTOGUI_AVAILABLE:
    ...
    try:
        ...
        pygetwindow = None
        logger.info(
            "pygetwindow not found, window activation capabilities will be limited."
        )
else:
    pyautogui = None
    pygetwindow = None
...
class ChatGPTWebAgent:
    ...
    async def inject_response(self, message: str) -> None:
        ...
        CHATGPT_WINDOW_TITLE_SUBSTRING = getattr(
            self.config, "agents.chatgpt_web.window_title_substring", "ChatGPT"
        )
        SEND_BUTTON_IMAGE_PATH = getattr(
            self.config,
            "agents.chatgpt_web.send_button_image_path",
            "assets/gui_elements/chatgpt_send_button.png",
        )
        # Fallback image path if the primary one is not found or configured
        FALLBACK_SEND_BUTTON_IMAGE_PATH = (
            "assets/gui_elements/fallback_chatgpt_send_button.png"  # Example
        )
        USE_PYAUTOGUI_FOCUS_CHECK = getattr(
            self.config, "agents.chatgpt_web.use_pyautogui_focus_check", True
        )
        USE_PYAUTOGUI_SEND_FALLBACK = getattr(
            self.config, "agents.chatgpt_web.use_pyautogui_send_fallback", True
        )
        SELENIUM_SEND_BUTTON_XPATH = getattr(
            self.config,
            "agents.chatgpt_web.selenium_send_button_xpath",
            "//button[contains(@class, 'send-button') or @data-testid='send-button']",
        )  # Example common XPaths
        ...
        if PYAUTOGUI_AVAILABLE and USE_PYAUTOGUI_FOCUS_CHECK and pygetwindow:
            logger.debug(
                f"[{self.agent_id}] Checking window focus for title substring: '{CHATGPT_WINDOW_TITLE_SUBSTRING}'"
            )
            if not is_window_focused(CHATGPT_WINDOW_TITLE_SUBSTRING):
                ...
                try:
                    target_windows = pygetwindow.getWindowsWithTitle(
                    ...
                    if target_windows:
                        ...
                        logger.info(
                            f"[{self.agent_id}] Attempted to activate window: {target_window.title}"
                        )
                        if not is_window_focused(CHATGPT_WINDOW_TITLE_SUBSTRING):
                            logger.warning(
                                f"[{self.agent_id}] Failed to confirm focus on target window after activation attempt."
                            )
                    else:
                        logger.warning(
                            f"[{self.agent_id}] No window found with title substring '{CHATGPT_WINDOW_TITLE_SUBSTRING}' to activate."
                        )
                except Exception as e_focus:
                    logger.error(
                        f"[{self.agent_id}] Error during window activation attempt: {e_focus}"
                    )
            else:
                logger.debug(
                    f"[{self.agent_id}] Target window '{CHATGPT_WINDOW_TITLE_SUBSTRING}' is focused."
                )
            ...
            try:
                ...
                try:
                    send_button = self.driver.find_element(
                        By.XPATH, SELENIUM_SEND_BUTTON_XPATH
                    )
                    if send_button.is_displayed() and send_button.is_enabled():
                        send_button.click()
                        selenium_sent_successfully = True
                        logger.info(
                            f"[{self.agent_id}] Successfully sent message via Selenium button click."
                        )
                    else:
                        logger.warning(
                            f"[{self.agent_id}] Selenium send button found but not interactable. Will try newline."
                        )
                except NoSuchElementException:
                    logger.warning(
                        f"[{self.agent_id}] Selenium send button not found by XPATH '{SELENIUM_SEND_BUTTON_XPATH}'. Will try newline."
                    )

                if not selenium_sent_successfully:
                    logger.info(
                        f"[{self.agent_id}] Attempting to send by sending newline character to textarea."
                    )
                    input_box.send_keys("\n")
                    selenium_sent_successfully = (
                        True  # Assume newline works if no immediate error
                    )
                    logger.info(
                        f"[{self.agent_id}] Successfully sent message via Selenium newline."
                    )
                ...
                logger.error(
                    f"[{self.agent_id}] Selenium error during injection/send: {e_selenium}"
                )
                return False  # Selenium failed
            except Exception as e_general_selenium:
                logger.error(
                    f"[{self.agent_id}] Unexpected Selenium error: {e_general_selenium}",
                    exc_info=True,
                )
                return False
        ...
        try:
            selenium_send_success = await asyncio.to_thread(
            ...
            if selenium_send_success:
                logger.info(
                    f"[{self.agent_id}] 🛰️ Message injection via Selenium presumed successful."
                )
            else:
                logger.warning(
                    f"[{self.agent_id}] Selenium send failed or was inconclusive."
                )
                if PYAUTOGUI_AVAILABLE and USE_PYAUTOGUI_SEND_FALLBACK:
                    logger.info(
                        f"[{self.agent_id}] Attempting PyAutoGUI fallback for sending message..."
                    )
                    try:
                        ...
                        try:
                            send_button_location = pyautogui.locateCenterOnScreen(
                                SEND_BUTTON_IMAGE_PATH, confidence=0.8
                            )
                        except (
                            Exception
                        ) as e_locate_primary:  # pyautogui.ImageNotFoundException might not be raised if confidence fails
                            logger.warning(
                                f"[{self.agent_id}] Primary send button image '{SEND_BUTTON_IMAGE_PATH}' not found ({e_locate_primary}). Trying fallback image."
                            )
                            try:
                                send_button_location = pyautogui.locateCenterOnScreen(
                                    FALLBACK_SEND_BUTTON_IMAGE_PATH, confidence=0.8
                                )
                            except Exception as e_locate_fallback:
                                logger.warning(
                                    f"[{self.agent_id}] Fallback send button image '{FALLBACK_SEND_BUTTON_IMAGE_PATH}' not found ({e_locate_fallback}). Will try Enter key."
                                )

                        if send_button_location:
                            pyautogui.click(send_button_location)
                            logger.info(
                                f"[{self.agent_id}] 🛰️ Message sent via PyAutoGUI image click."
                            )
                        else:
                            logger.info(
                                f"[{self.agent_id}] PyAutoGUI send button image not found. Attempting to send with Enter key."
                            )
                            pyautogui.press("enter")
                            logger.info(
                                f"[{self.agent_id}] 🛰️ Message sent via PyAutoGUI Enter key press."
                            )
                        await asyncio.sleep(1)  # Pause after PyAutoGUI action
                    except Exception as e_pyauto:
                        logger.error(
                            f"[{self.agent_id}] PyAutoGUI fallback send failed: {e_pyauto}",
                            exc_info=True,
                        )
                        # If both Selenium and PyAutoGUI fail, this is a hard failure for injection.
                        raise ElementNotInteractableException(
                            "Both Selenium and PyAutoGUI failed to send message."
                        )
                elif not PYAUTOGUI_AVAILABLE and USE_PYAUTOGUI_SEND_FALLBACK:
                    logger.warning(
                        f"[{self.agent_id}] PyAutoGUI fallback requested but PyAutoGUI is not available."
                    )
                    raise ElementNotInteractableException(
                        "Selenium send failed and PyAutoGUI is not available for fallback."
                    )
                else:  # Selenium failed and PyAutoGUI fallback not used/requested
                    raise ElementNotInteractableException(
                        "Selenium send failed and PyAutoGUI fallback not configured."
                    )

        except Exception as e:
            logger.error(
                f"[{self.agent_id}] Overall failure in inject_response: {e}",
                exc_info=True,
            )
    ...
    async def _scrape_latest_response(self, attempts=3, delay_seconds=5):
        ...
        for attempt in range(attempts):
            try:
                logger.info(
                    f"[{self.agent_id}] Attempt {attempt + 1} to scrape response."
                )
                latest_reply = await asyncio.to_thread(sync_scrape)
                if latest_reply and (
                    not self.last_seen or latest_reply != self.last_seen
                ):
                    self.last_seen = latest_reply
                    logger.info(
                        f"[{self.agent_id}] Successfully scraped new response: {latest_reply[:100]}..."
                    )
                    return latest_reply
                else:
                    logger.info(
                        f"[{self.agent_id}] No new response or response unchanged. Last seen: {self.last_seen[:100]}..."
                    )
            except Exception as e:
                logger.error(
                    f"[{self.agent_id}] Error during scrape attempt {attempt + 1}: {e}",
                    exc_info=True,
                )

            if attempt < attempts - 1:
                logger.info(
                    f"[{self.agent_id}] Waiting {delay_seconds}s before next scrape attempt."
                )
                await asyncio.sleep(delay_seconds)

        logger.warning(
            f"[{self.agent_id}] Failed to scrape a new response after {attempts} attempts."
        )
        return None
    ...
    async def process_prompt_via_ui(self, prompt_text: str) -> str | None:
        ...
        if self.simulate:
            logger.info(
                f"[{self.agent_id}] Simulating UI processing for prompt: {prompt_text[:100]}..."
            )
        ...
        if not self.driver:
            logger.info(
                f"[{self.agent_id}] Browser not initialized. Initializing now for UI processing."
            )
            if not self._initialize_browser():
                logger.error(
                    f"[{self.agent_id}] Failed to initialize browser. Cannot process prompt."
                )
                return None

        try:
            logger.info(
                f"[{self.agent_id}] Injecting prompt into UI: {prompt_text[:100]}..."
            )
            ...
            if response:
                logger.info(
                    f"[{self.agent_id}] Successfully processed prompt via UI. Response: {response[:100]}..."
                )
            else:
                logger.warning(
                    f"[{self.agent_id}] Did not get a response from UI after processing prompt."
                )
            return response
        except Exception as e:
            logger.error(
                f"[{self.agent_id}] Error during UI processing of prompt '{prompt_text[:50]}...': {e}",
                exc_info=True,
            )
            return None
    ...
    async def process_external_prompt(self, prompt_text: str) -> str | None:
        ...
        """
        logger.info(
            f"[{self.agent_id}] Received external prompt: {prompt_text[:100]}..."
        )

        if self.simulate:
            logger.info(
                f"[{self.agent_id}] Simulating external prompt processing for: {prompt_text[:50]}..."
            )
        ...
        if not self.driver:
            logger.info(
                f"[{self.agent_id}] Browser not initialized for external prompt. Initializing..."
            )
            if not await asyncio.to_thread(
                self._initialize_browser
            ):  # _initialize_browser is sync
                logger.error(
                    f"[{self.agent_id}] Critical: Browser initialization failed. Cannot process external prompt."
                )
                return None
        ...
        try:
            ...
            if self.conversation_url not in current_url:
                logger.warning(
                    f"[{self.agent_id}] Not on conversation URL ({current_url}). Navigating..."
                )
            ...
            external_processing_delay = getattr(
                self.config,
                "agents.chatgpt_web.external_prompt_delay",
                self.interval / 2,
            )
            logger.info(
                f"[{self.agent_id}] Waiting {external_processing_delay}s for reply to external prompt..."
            )
            ...
            if not page_source:
                logger.error(
                    f"[{self.agent_id}] Failed to get page source for scraping external reply."
                )
                return None

            latest_reply, _ = extract_latest_reply(
                page_source, self.last_seen
            )  # extract_latest_reply is sync

            if latest_reply:
                self.last_seen = (
                    latest_reply  # IMPORTANT: Update last_seen with the new reply
                )
                logger.info(
                    f"[{self.agent_id}] Successfully scraped reply for external prompt: {latest_reply[:100]}..."
                )
                return latest_reply
            else:
                logger.warning(
                    f"[{self.agent_id}] No new reply found after injecting external prompt."
                )
            ...
            logger.error(
                f"[{self.agent_id}] Error processing external prompt: {e}",
                exc_info=True,
            )
    ...
    async def close(self):
        ...
        try:
            ...
            logger.error(
                f"[{self.agent_id}] Error during browser close: {e}", exc_info=True
            )
