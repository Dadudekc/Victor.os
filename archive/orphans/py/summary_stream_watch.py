...
def log_status(status_log_path, message):
    ...
    try:
        ...
        print(
            f"[ERROR] Failed to write to status log {status_log_path}: {e}"
        )  # Print error, don't crash watcher
                ...
                for i, line in enumerate(f):
                    ...
                    if current_line > start_line:
                        try:
                            ...
                            print(
                                f"WARN: Skipping invalid JSON on line {current_line} in {file_path}"
                            )
...
def find_correlation(gpt_entry, scraper_log_full_path, status_log_full_path):
    ...
    try:
        ...
        log_status(
            status_log_full_path,
            f"WARN: Could not read/parse scraper log {scraper_log_full_path} for correlation: {e}",
        )
...
if __name__ == "__main__":
    ...
    log_status(
        status_log_path_abs,
        "Ironvale Summary Stream Watcher starting (Enhanced with Trust Scoring).",
    )
    log_integrity_event(
        integrity_report_path_abs, {"event_type": "watcher_start_trust_scoring"}
    )
    ...
    for path in [gpt_log_path_abs, scraper_log_path_abs]:
        if not os.path.exists(path):
            try:
                ...
                log_status(
                    status_log_path_abs, f"ERROR: Failed to ensure log file {path}: {e}"
                )
                log_integrity_event(
                    integrity_report_path_abs,
                    {"event_type": "startup_error", "error": str(e), "file": path},
                )
    ...
    log_status(
        status_log_path_abs,
        f"Starting GPT log watch from line {processed_gpt_line_count + 1}",
    )

    for cycle in range(MAX_RUNTIME_CYCLES):
        log_status(status_log_path_abs, f"Cycle {cycle + 1}/{MAX_RUNTIME_CYCLES}")

        new_gpt_entries, new_line_count = read_new_jsonl_lines(
            gpt_log_path_abs, processed_gpt_line_count
        )

        if new_gpt_entries:
            log_status(
                status_log_path_abs,
                f"Detected {len(new_gpt_entries)} new entries in {gpt_log_path_abs}.",
            )
            cycles_since_last_new = 0
            if os.path.exists(watchdog_flag_path_abs):
                ...
                try:
                    ...
                    log_status(
                        status_log_path_abs,
                        f"WARN: Failed to remove watchdog flag: {e}",
                    )
            ...
            for entry in new_gpt_entries:
                ...
                if not response_text:
                    log_status(
                        status_log_path_abs, "Skipping entry with empty response."
                    )
                    continue
                ...
                scraper_id, expected_hash = find_correlation(
                    entry, scraper_log_path_abs, status_log_path_abs
                )
                hashes_verified += 1
                ...
                if scraper_id != "correlation_not_found":
                    if expected_hash and computed_hash == expected_hash:
                        ...
                        trust_counts["1.0"] += 1
                        hash_mismatches = (
                            0  # Reset mismatch count on successful verification?
                        )
                        ...
                        log_status(
                            status_log_path_abs,
                            f"ALERT: Hash mismatch detected! GPT Log Time: {entry.get('timestamp')}, Scraper ID: {scraper_id}",
                        )
                        log_integrity_event(
                            integrity_report_path_abs,
                            {
                        ...
                        if not os.path.exists(hash_mismatch_flag_path_abs):
                            try:
                                with open(hash_mismatch_flag_path_abs, "w") as f:
                                    f.write(
                                        f"First mismatch detected at {get_current_timestamp()}\n"
                                    )
                                log_status(
                                    status_log_path_abs,
                                    f"Created hash mismatch flag: {hash_mismatch_flag_path_abs}",
                                )
                            except Exception as e:
                                log_status(
                                    status_log_path_abs,
                                    f"ERROR: Failed to create hash mismatch flag: {e}",
                                )
                        ...
                        log_status(
                            status_log_path_abs,
                            f"WARN: Found correlation ({scraper_id}) but no hash in scraper log for comparison. Score: 0.5",
                        )
                        log_integrity_event(
                            integrity_report_path_abs,
                            {
                    ...
                    log_status(
                        status_log_path_abs,
                        f"WARN: No correlation found for GPT entry {entry.get('timestamp')}. Score: 0.0",
                    )
                    log_integrity_event(
                        integrity_report_path_abs,
                        {
                ...
                try:
                    ...
                    log_status(
                        status_log_path_abs,
                        f"ERROR: Failed to write to {summary_log_path_abs}: {e}",
                    )

                if entries_since_last_status >= STATUS_UPDATE_INTERVAL:
                    log_status(
                        status_log_path_abs,
                        f"Processed {entries_since_last_status} new entries. Last summary at {last_summary_timestamp}",
                    )
                    log_integrity_event(
                        integrity_report_path_abs,
                        {
            ...
            if cycles_since_last_new >= MAX_CYCLES_WITHOUT_NEW:
                if not os.path.exists(watchdog_flag_path_abs):
                    log_status(
                        status_log_path_abs,
                        f"ALERT: Stream stalled for {cycles_since_last_new} cycles. Creating watchdog flag.",
                    )
                    log_integrity_event(
                        integrity_report_path_abs,
                        {
                            "event_type": "stream_stalled",
                            "cycles_stalled": cycles_since_last_new,
                        },
                    )
                    try:
                        ...
                        log_status(
                            status_log_path_abs,
                            f"ERROR: Failed to create watchdog flag {watchdog_flag_path_abs}: {e}",
                        )
                else:
                    log_status(
                        status_log_path_abs,
                        f"Stream stalled for {cycles_since_last_new} cycles. Watchdog flag already present.",
                    )
    ...
    log_status(
        status_log_path_abs,
        f"Completed {MAX_RUNTIME_CYCLES} monitoring cycles. Exiting.",
    )
    ...
    log_status(
        status_log_path_abs, f"Writing trust histogram to {trust_histogram_path_abs}"
    )
    try:
        ...
        log_integrity_event(
            integrity_report_path_abs,
            {"event_type": "histogram_write_error", "error": str(e)},
        )
