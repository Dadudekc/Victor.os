...
try:
    ...
    try:
        ...
        from dreamos.agents.shadow_task_nexus import (
            ShadowTaskNexus as TaskNexus,  # type: ignore
        )
        from dreamos.agents.shadow_task_nexus import (
            Task,
        )
...
def main():
    ...
    if args.tags:
        try:
            parsed_tags = {
                tag.split(":")[0].upper(): tag.split(":", 1)[1]
            ...
            logger.error(
                "Invalid format for --tags argument. Use KEY:value pairs separated by space.",
                exc_info=True,
            )
            sys.exit(1)
