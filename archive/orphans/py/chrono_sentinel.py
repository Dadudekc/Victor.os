...
FUTURE_SKEW_THRESHOLD_SECONDS = (
    10  # Timestamps this far in the future indicate clock skew
)
...
def log_to_ledger(anomaly_details: Dict[str, Any]):
    ...
    try:
        ...
        logger.error(
            f"Failed to write to UTC Fidelity Ledger {UTC_LEDGER_FILE}: {e}",
            exc_info=True,
        )
...
def get_local_tz_cached() -> tzinfo:
    ...
    if _local_tz is None:
        try:
            ...
            logger.error(
                f"Could not determine local timezone: {e}. Defaulting to UTC.",
                exc_info=True,
            )
            _local_tz = LOCAL_TIMEZONE_ASSUMPTION
    return _local_tz
...
def parse_integrity_log_entry(line: str) -> Optional[datetime]:
    ...
    if match:
        ...
        try:
            ...
            logger.debug(
                f"Could not parse timestamp in integrity log line: {line.strip()}"
            )
            return None
    return None
...
def parse_stress_log_entry(line: str) -> Optional[datetime]:
    ...
    if match:
        ...
        try:
            ...
            ts_str_tz = ts_str.replace(
                "Z", "+00:00"
            )  # Ensure Z is handled by fromisoformat
            ...
            if event_timestamp_naive_or_aware.tzinfo is None:
                ...
                utc_timestamp = event_timestamp_naive_or_aware.replace(
                    tzinfo=timezone.utc
                )
            else:
            ...
            logger.debug(
                f"Could not parse timestamp in stress log line: {line.strip()}"
            )
            return None
    return None
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            for i, line in enumerate(lines):
                ...
                if event_time_utc:
                    ...
                    if event_time_utc > reference_time_utc + timedelta(
                        seconds=FUTURE_SKEW_THRESHOLD_SECONDS
                    ):
                        deviation = (
                            event_time_utc - reference_time_utc
                        ).total_seconds()
                        anomaly = {
                        ...
                        logger.warning(
                            f"Future skew detected in {file_path}: {deviation:.2f}s"
                        )
                        log_to_ledger(anomaly)

                    # Check for staleness
                    if event_time_utc < reference_time_utc - timedelta(
                        minutes=STALENESS_THRESHOLD_MINUTES
                    ):
                        deviation = (
                            reference_time_utc - event_time_utc
                        ).total_seconds()
                        anomaly = {
                            "detection_timestamp_utc": reference_time_utc.isoformat(),
                            "sentinel_id": SENTINEL_ID,
                        ...
                        logger.warning(
                            f"Stale timestamp detected in {file_path}: {deviation/60:.2f} minutes old"
                        )
                        log_to_ledger(anomaly)
...
def run_sentinel():
    logger.info(
        f"Starting Chrono Drift Sentinel ({SENTINEL_ID})... Monitoring interval: {MONITOR_INTERVAL_SECONDS}s"
    )
    ...
    try:
        ...
        logger.error(
            f"Could not create or access ledger file {UTC_LEDGER_FILE}: {e}. Sentinel cannot log findings."
        )
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(
            f"Chrono Drift Sentinel encountered a critical error: {e}", exc_info=True
        )
        sys.exit(1)
