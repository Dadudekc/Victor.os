...
parser.add_argument(
    "--prompt", type=str, help="Custom prompt text to use instead of default"
)
parser.add_argument(
    "--prompt-file", type=str, help="Path to file containing custom prompt text"
)
parser.add_argument(
    "--prompt-dir",
    type=str,
...
parser.add_argument(
    "--list-prompts", action="store_true", help="List available prompt files and exit"
)
...
def _validate_coords(path: Path, agent_key: str, expect_dict: bool = True):
    ...
    try:
        ...
        if expect_dict and not isinstance(value, dict):
            log.error(
                f"Coord file {path} has wrong format for key '{agent_key}'. Expected dict, got {type(value).__name__}."
            )
            sys.exit(1)
        ...
        if not expect_dict and not (isinstance(value, list) and len(value) == 2):
            log.error(
                f"Coord file {path} has wrong format for key '{agent_key}'. Expected [x,y] list, got {value}."
            )
            sys.exit(1)
...
async def agent_loop(bus: AgentBus):
    ...
    retriever = ResponseRetriever(
        agent_id=agent_id_for_retriever, coords_file=str(COPY_COORDS)
    )
    ...
    log.info(
        f"{AGENT_ID} bootstrap loop started. Using retriever agent_id: {agent_id_for_retriever}"
    )
    ...
    while True:
        ...
        for msg in _read_inbox():
            ...
            if inject_text:
                ...
                if (
                    "retrieve_response" in prompt.lower() or True
                ):  # Always try to retrieve
                    response = None
                    ...
                    if response:
                        ...
                        with DEVLOG_PATH.open("a", encoding="utf-8") as fh:
                            fh.write(
                                f"[{datetime.now(timezone.utc).isoformat()}] RESPONSE: {response}\n"
                            )
                    else:
...
async def main():
    ...
    if ARGS.prompt:
        ...
        log.info("Using custom prompt from command line argument")
