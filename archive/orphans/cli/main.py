...
def cli(ctx, cli_config_path):
    ...
    try:
        ...
        if cli_config_path:
            logger.info(
                f"CLI --config specified: {cli_config_path}. Attempting override."
            )
...
def run(ctx):
    ...
    try:
        ...
        if not db_path_str:
            ...
            db_path = (
                config_from_ctx.paths.project_root / "runtime" / "db" / "dreamos.db"
            )
            logger.warning(
                f"AppConfig.paths.database_path not set, using default: {db_path}"
            )
        else:
        ...
        controller = SwarmController(
            config=config_from_ctx, adapter=sqlite_adapter, agent_bus=agent_bus
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        )  # Add standalone_mode=False for click>=8 compatibility
