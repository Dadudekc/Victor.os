...
class MockConfigInterface:
    ...
    def get(self, key: str, default: Optional[Any] = None) -> Any:
        # Return dummy values
        if key == "cursor_bridge.injector.delay":
            return 0.1
        if key == "cursor_bridge.reader.timeout":
            return 5.0
...
class MockPromptInjector:
    ...
    def inject_prompt(self, prompt: str) -> bool:
        ...
        logger.info(
            f"[MockInjector] Attempting to inject prompt (length {len(prompt)})..."
        )
        if not prompt:
...
class MockLoopController:
    ...
    def run_cycle(self, prompt: str) -> Optional[str]:
        ...
        for attempt in range(self.max_retries):
            logger.info(
                f"[MockController] Injection attempt {attempt + 1}/{self.max_retries}"
            )
            ...
            if inject_success:
                ...
                logger.warning(
                    "[MockController] Injection failed. Retrying after delay..."
                )
                time.sleep(0.5)  # Simulate retry delay

        if not inject_success:
            logger.error(
                "[MockController] Injection failed after max retries. Aborting cycle."
            )
            return None
        ...
        for attempt in range(self.max_retries):
            logger.info(
                f"[MockController] Read attempt {attempt + 1}/{self.max_retries}"
            )
        ...
        logger.info("[MockController] Cycle completed successfully.")
        return response
