...
class MultibotManager:
    ...
    def __init__(
        ...
        status_callback: Optional[
            Callable[[Path, Optional[Any]], Coroutine[Any, Any, None]]
        ] = None,  # Callback can be async
    ):
        self.task_queue = asyncio.Queue()  # Use asyncio.Queue
        self.results_list: List[
            Any
        ] = []  # Store results here (ensure thread/task safety if accessed outside manager)
        ...
        self._stop_event = (
            asyncio.Event()
        )  # For graceful shutdown if needed, though sentinel is primary
        ...
        while True:
            ...
            try:
                ...
                if file_path is None:  # Sentinel value to stop
                    logger.info(
                        f"Async worker {worker_id} received sentinel, stopping."
                    )
                    break
                ...
                if hasattr(self.scanner, "process_file_async"):
                    ...
                    self.scanner, "_process_file"
                ) and asyncio.iscoroutinefunction(self.scanner._process_file):
                    result = await self.scanner._process_file(file_path)
                else:
                    logger.error(
                        f"Worker {worker_id}: Scanner has no suitable async process_file method for {file_path}"
                    )
                ...
                logger.error(
                    f"Error in async worker {worker_id} processing task {file_path}: {e}",
                    exc_info=True,
                )
            finally:
