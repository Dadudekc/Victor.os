...
class TaskAutoRewriter:
    ...
    def __init__(self):
        ...
        if hasattr(self.config, "integrations") and hasattr(
            self.config.integrations, "openai"
        ):
            ...
            if not self.openai_client.is_functional():
                logger.error(
                    "TaskAutoRewriter: OpenAI client is not functional. Rewriting capabilities may be limited."
                )
        else:
            logger.warning(
                "TaskAutoRewriter: OpenAI configuration not found. Rewriting capabilities disabled."
            )
        ...
        if (
            hasattr(self.config, "tools")
            and hasattr(self.config.tools, "task_editor")
            and hasattr(self.config.tools.task_editor, "rewrite_model_name")
        ):
    ...
    def analyze_and_propose_rewrite(
        ...
        if any(keyword in task_name.lower() for keyword in broad_task_keywords) or any(
            ...
            if not task_data.get(
                ...
                logger.info(
                    f"Task {task_id} appears too broad. Proposing it be marked as an Epic AND SPLIT."
                )
                ...
                original_description = params.get(
                    "description", "No description provided for the epic task."
                )
                parent_action = task_data.get(
                    "action", "generic_sub_task_action"
                )  # Inherit or default action
                ...
                sub_task_base_properties = {
                    ...
                    "timestamp_injected_utc": datetime.datetime.now(
                    ...
                    "assigned_to_type": task_data.get(
                        "assigned_to_type"
                    ),  # Inherit assignment preference
                }
