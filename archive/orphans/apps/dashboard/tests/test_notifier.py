...
def config_path(tmp_path):
    ...
    config = {
        ...
        "episode": {"number": 5, "name": "Jarvis Integration", "status": "active"},
    }
...
async def test_discord_alert(notifier, mocker):
    ...
    await notifier.alert_thea_escalation(
        agent_id="test_agent", task_id="test_task", reason="Test escalation"
    )
    ...
    await notifier.alert_drift_detected(confidence=75.0, details="Test drift")
...
async def test_slack_alert(notifier, mocker):
    ...
    await notifier.alert_resource_warning(resource="CPU", usage=95.0)
...
async def test_alert_error_handling(notifier, mocker):
    ...
    await notifier.alert_loop_halt(agent_id="test_agent", reason="Test halt")
...
async def test_missing_webhook(notifier, mocker):
    ...
    # Test alert sending
    await notifier.send_alert(level="info", title="Test Alert", message="Test message")
