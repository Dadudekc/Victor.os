...
from apps.dashboard.modules.resume_autonomy import (
...
@pytest.mark.skipif(
    platform.system() == "Windows", reason="GUI tests not supported on Windows"
)
...
def test_resume_trigger_on_drift(agent_tab, status_file, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        agent_tab,
        "check_drift",
        lambda: ResumeAutonomyAgentTab.check_drift.__func__(agent_tab),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (
            bridge_file
            if args and args[0] == original_bridge_file
...
def test_status_file_update(agent_tab, status_file):
    ...
    # Update status
    agent_tab.update_status(current_task="testing", compliance_score=95)
