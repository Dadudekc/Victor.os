...
def mock_env():
    with patch.dict(
        "os.environ",
        {"DISCORD_BOT_TOKEN": "mock_token", "DISCORD_CHANNEL_ID": "123456789"},
    ):
        yield
...
### TEST: Initialization ###
@patch(
    "basicbot.discord_notifier.config.get_env",
    side_effect=lambda key, cast_type=None: (
...
@patch(
    "basicbot.discord_notifier.config.get_env",
    side_effect=lambda key, cast_type=None: None,
)
def test_notifier_missing_credentials(mock_config):
    with pytest.raises(
        ValueError, match="Missing DISCORD_BOT_TOKEN or DISCORD_CHANNEL_ID"
    ):
        DiscordNotifier()
