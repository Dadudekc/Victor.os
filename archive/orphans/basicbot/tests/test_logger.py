...
def test_logging_levels(tmp_path: Path):
    ...
    logger = setup_logging(
        "test_logger", log_dir=log_dir, console_log_level=logging.WARNING
    )

    # Filter for the console handler by its custom name
    console_handlers = [
        h for h in logger.handlers if getattr(h, "name", "") == "console_handler"
    ]
    ...
    ), f"❌ Expected WARNING (30), got {console_handler.level}"
...
def test_feedback_loop_enabled(tmp_path):
    ...
    log_dir = tmp_path / "logs"
    setup_logging("test_logger", log_dir=log_dir, feedback_loop_enabled=True)
    ...
    ), "❌ AI feedback loop message not found in log file."
