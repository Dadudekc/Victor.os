...
def find_orphaned_modules(graph_path: Path) -> list[str]:
    ...
    for imports_list in import_graph.values():
        for imp in imports_list:
            raw_imports.add(
                imp
            )  # These are top-level names like 'os', 'dreamos', 'pathlib'
    ...
    module_stems = {
        m.replace("src/", "").replace("/", ".").replace(".py", "")
        ...
        for imp_name in imports_list:
            ...
            if possible_module_path_stem in module_stems:
                ...
                matched_defined = [
                    m
                    for m in defined_modules
                    if m.replace("src/", "").replace("/", ".").replace(".py", "")
                    == possible_module_path_stem
                ]
                if matched_defined:
    ...
    logging.info(
        f"Unique imports resolved to project files: {len(imported_in_graph_values)}"
    )
