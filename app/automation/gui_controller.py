...
# Basic Mouse Actions
def click(x: int | None = None, y: int | None = None, button: str = 'left', clicks: int = 1, interval: float = 0.0):
...
# Screen Interaction
def locate_on_screen(image_path: str, confidence: float = 0.9, grayscale: bool = False) -> tuple | None:
    """Locates the given image on the screen.
    
    Returns:
        A tuple (left, top, width, height) of the bounding box, or None if not found.
    """
    print(f"Simulating locating image '{image_path}' on screen (confidence={confidence})")
    # try:
