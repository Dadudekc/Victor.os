...
def audit_ledger():
        ...
        with open(UTC_LEDGER_FILE, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                ...
                try:
                    ...
                    logger.warning(f"Skipping malformed JSON line {i+1} in {UTC_LEDGER_FILE}")
                    continue
                ...
                if not all([detection_ts_str, log_file, anomaly_type]):
                    logger.warning(f"Skipping incomplete ledger entry on line {i+1}: {line}")
                    continue
                    
                detection_dt = parse_iso_utc(detection_ts_str)
                if not detection_dt:
                    logger.warning(f"Skipping entry with unparseable timestamp on line {i+1}")
                    continue
                ...
                if window_start_utc < detection_dt <= now_utc:
                    ...
                    sentinel_ids[log_file] = sentinel_id # Store last seen sentinel for this source

                    # Update latest anomaly timestamp for this source
                    current_latest = latest_anomaly_ts.get(log_file, datetime.min.replace(tzinfo=timezone.utc))
                    if detection_dt > current_latest:
                        latest_anomaly_ts[log_file] = detection_dt
                        
    except Exception as e:
        logger.error(f"Error reading or processing ledger file {UTC_LEDGER_FILE}: {e}", exc_info=True)
        return

    logger.info(f"Processed ledger. Found {total_anomalies_in_window} anomalies in the last {ANALYSIS_WINDOW_HOURS} hours.")

    # --- Generate Report --- 
    report_data: Dict[str, Any] = {
        "report_generated_utc": now_utc.isoformat().replace('+00:00', 'Z'),
        "analysis_window_start_utc": window_start_utc.isoformat().replace('+00:00', 'Z'),
    ...
    for log_file, type_counts in stats.items():
        ...
        report_data["anomaly_summary_by_source"][log_file] = {
            ...
            "latest_anomaly_utc": latest_ts.isoformat().replace('+00:00', 'Z') if latest_ts else None,
        ...
        if threshold_exceeded:
            ...
            latest_ts_str = latest_ts.isoformat().replace('+00:00', 'Z') if latest_ts else "N/A"
            sentinel_str = sentinel_ids.get(log_file, "UnknownSentinel")
            # Simulate logging to THEA with a high-priority log message
            logger.critical(f"ALERT [THEA]: UTC drift anomaly threshold exceeded for source '{log_file}' (Count: {total_source_anomalies}/{ANALYSIS_WINDOW_HOURS}h). Sentinel: {sentinel_str}. Latest Anomaly: {latest_ts_str}")
    ...
    if not alert_triggered:
         logger.info(f"No sources exceeded the anomaly threshold ({ANOMALY_THRESHOLD_PER_SOURCE}/{ANALYSIS_WINDOW_HOURS}h).")
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(f"Temporal Ledger Auditor encountered a critical error: {e}", exc_info=True)
        sys.exit(1) 