...
CODE_EXTENSIONS = {'.py', '.js', '.jsx', '.ts', '.tsx', '.html', '.css', '.scss', '.vue'}
EXCLUDE_DIRS = {'__pycache__', 'node_modules', '.git', '.vscode', '.idea'} # Exclude common large/binary dirs
...
def get_asset_files(domains_path: Path) -> list[str]:
    ...
    try:
        ...
        if isinstance(domains_data, dict) and 'domains' in domains_data and 'asset' in domains_data['domains']:
            # Return full paths as stored in the domains file
            return domains_data['domains']['asset']
        else:
            logging.warning(f"Could not find 'asset' domain or valid structure in {domains_path}")
            return []
...
def main():
    ...
    scanned_count = 0
    log_interval = max(1, len(code_files_to_scan) // 10) # Log progress roughly 10 times
    ...
    for code_file in code_files_to_scan:
        ...
        if scanned_count % log_interval == 0:
            logging.info(f"Scanned {scanned_count}/{len(code_files_to_scan)} code files...")
    ...
    final_usage = {asset_path: usage_counts.get(asset_path, 0) for asset_path in asset_files}
