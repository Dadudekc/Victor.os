...
def main():
    ...
    if manifest_data is not None:
        ...
        summary_lines.append("- *Manifest file (`manifest.json`) not found or could not be read.*")
    ...
    total_modules_in_graph = len(import_graph_data) if isinstance(import_graph_data, dict) else 0
    summary_lines.append("## 2. Module Imports & Orphaned Files")
    if import_graph_data is not None:
        summary_lines.append(f"- Total Python modules analyzed: {total_modules_in_graph}")
    else:
         summary_lines.append("- *Import graph file (`import-graph.json`) not found or could not be read.*")
    ...
    if orphaned_data is not None and isinstance(orphaned_data, dict):
        ...
        if count > 0:
            summary_lines.append("  - **Note:** Analysis based on AST parsing; requires manual verification.")
            summary_lines.append("  - **Files (first 10):**")
            for i, f in enumerate(files[:10]):
                summary_lines.append(f"    - `{f}`")
            if count > 10:
                summary_lines.append("    - ... (see `orphaned-files.json` for full list)")
    else:
        summary_lines.append("- *Orphaned files report (`orphaned-files.json`) not found or could not be read.*")
    summary_lines.append("")

    # 3. Domain Classification
    domains_data = read_json_file(DOMAINS_FILE, "domain classification")
    summary_lines.append("## 3. Domain Classification")
    if domains_data is not None and isinstance(domains_data, dict) and 'domains' in domains_data:
        domain_counts = {domain: len(files) for domain, files in domains_data['domains'].items()}
        ...
        summary_lines.append("- *Domain classification file (`domains.json`) not found or could not be read.*")
    summary_lines.append("")

    # 4. Asset Usage
    unreferenced_assets_data = read_json_file(UNREFERENCED_ASSETS_FILE, "unreferenced assets report")
    summary_lines.append("## 4. Asset Usage")
    if unreferenced_assets_data is not None and isinstance(unreferenced_assets_data, dict):
        count = unreferenced_assets_data.get('unreferenced_assets_count', 0)
        files = unreferenced_assets_data.get('unreferenced_assets', [])
        summary_lines.append(f"- Potentially unreferenced assets found (count=0 in usage scan): {count}")
        if count > 0:
            summary_lines.append("  - **Note:** Analysis based on simple filename presence in code; requires manual verification.")
            summary_lines.append("  - **Files:**")
            for f in files:
                summary_lines.append(f"    - `{f}`")
        else:
             summary_lines.append("  - All identified assets had at least one potential reference found in the code scan.")
    else:
        summary_lines.append("- *Unreferenced assets report (`unreferenced-assets.json`) not found or could not be read.*")
    summary_lines.append("")
