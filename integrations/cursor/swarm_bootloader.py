...
try:
    from integrations.cursor.window_controller import CursorWindowController, WindowWrapper
except ImportError as e:
     # Add fallback or raise critical error if controller is essential
     logging.critical(f"Failed to import CursorWindowController: {e}. SwarmBootloader cannot function.")
     # Define dummy classes if needed for type hinting downstream, though raising is better
     class WindowWrapper: pass 
     class CursorWindowController: def detect_all_instances(self, *args, **kwargs): return []
     # Re-raise the error to prevent silent failure
     raise ImportError(f"Could not import CursorWindowController for SwarmBootloader: {e}") from e
...
class TheaSwarmBootloader:
    ...
    DEFAULT_CURSOR_PATH_WIN = "C:/Users/Default/AppData/Local/Programs/cursor/Cursor.exe" # Example path
    ...
    def _resolve_cursor_path(self, provided_path: Optional[str]) -> str:
        ...
        if provided_path:
            if Path(provided_path).exists() or shutil.which(provided_path): # Check if path exists or is command
                 logger.debug(f"Using provided Cursor path: {provided_path}")
                 return provided_path
            else:
                 logger.warning(f"Provided Cursor path '{provided_path}' not found. Falling back to defaults.")
        ...
        if os_type == 'Windows':
            ...
            if Path(default_path).exists():
                 ...
                 logger.warning(f"Default Windows Cursor path not found: {default_path}")
                 # Fallback to assuming 'cursor' is in PATH
                 if shutil.which('cursor'):
                     logger.debug("Using 'cursor' command found in PATH (Windows).")
                     return 'cursor'
                 else:
                     raise FileNotFoundError("Cannot find Cursor executable. Please provide path or ensure default exists/in PATH.")
        elif os_type == 'Darwin':
             # On macOS, we typically launch via 'open -a'
             app_path = self.DEFAULT_CURSOR_PATH_MAC
             if Path(app_path).exists():
                 logger.debug(f"Using default macOS Cursor path for 'open -a': {app_path}")
                 return app_path # Store path, but launch uses 'open -a'
             else:
                 raise FileNotFoundError("Cannot find Cursor.app at default location. Please provide path.")
        else: # Linux
            if shutil.which(self.DEFAULT_CURSOR_CMD_LINUX):
                 logger.debug(f"Using default Linux command '{self.DEFAULT_CURSOR_CMD_LINUX}' found in PATH.")
                 return self.DEFAULT_CURSOR_CMD_LINUX
            else:
                raise FileNotFoundError(f"Cannot find '{self.DEFAULT_CURSOR_CMD_LINUX}' command. Please provide path or ensure it's in PATH.")
    ...
    def launch_instances(self, count: int = 1) -> List[subprocess.Popen]:
        ...
        for i in range(count):
            try:
                ...
                logger.error(f"Failed to launch instance {i+1}: Executable not found at {self.cursor_path}")
                ...
                # Optionally stop launching more
                break 
        logger.info(f"Launched {len(launched_processes)} process(es). Check system for windows.")
        return launched_processes

    def wait_for_detection(self, expected_count: int, timeout: int = 30, poll_interval: float = 1.0) -> bool:
        """Waits until the specified number of Cursor windows are detected."""
        logger.info(f"Waiting up to {timeout}s for {expected_count} Cursor window(s) to be detected...")
        start_time = time.time()
        while time.time() - start_time < timeout:
            detected_windows = self.controller.detect_all_instances() # Use the controller
            if len(detected_windows) >= expected_count:
                logger.info(f"Successfully detected {len(detected_windows)} Cursor window(s)." )
                return True
            logger.debug(f"Waiting... detected {len(detected_windows)}/{expected_count} windows.")
            time.sleep(poll_interval)
        
        logger.warning(f"Timeout: Only detected {len(self.controller.windows)}/{expected_count} Cursor window(s) after {timeout}s.")
        return False
    ...
    def move_windows_to_desktop(self, desktop_index: int = 1) -> int:
        ...
        Args:
            desktop_index (int): The target virtual desktop number (usually 1-based).
                                   Note: pyvda uses 0-based internally for current desktop, 
                                   but seems to use 1-based for target number.

        Returns:
            int: The number of windows successfully moved.
        """
        if platform.system() != 'Windows':
            logger.warning("Virtual desktop management via pyvda is only supported on Windows.")
            return 0
            
        if not _pyvda_available:
            logger.warning("pyvda library not installed. Cannot move windows to virtual desktop.")
            return 0
        ...
        logger.info(f"Attempting to move {len(self.controller.windows)} windows to Virtual Desktop {target_desktop_num}...")
        ...
        try:
             ...
             if target_desktop_num > desktop_count:
                  logger.warning(f"Target desktop {target_desktop_num} does not exist (only {desktop_count} desktops found). Moving to last desktop.")
                  target_desktop_num = desktop_count # Adjust to last valid desktop
        except Exception as e:
             logger.error(f"Failed to get desktop count via pyvda: {e}. Cannot verify target desktop.")
        ...
        for window in self.controller.windows:
            try:
                ...
                logger.debug(f"Moved window {window.id} (HWND: {hwnd}) to Desktop {target_desktop_num}.")
                moved_count += 1
            except Exception as e:
                logger.error(f"Failed to move window {window.id} (HWND: {window.handle}) to Desktop {target_desktop_num}: {e}", exc_info=False) # Keep log cleaner
        
        logger.info(f"Successfully moved {moved_count}/{len(self.controller.windows)} windows to Virtual Desktop {target_desktop_num}.")
        return moved_count

    def setup_swarm(self, count: int = 8, 
                    wait_timeout: int = 30, 
                    move_to_desktop: Optional[int] = None) -> List[WindowWrapper]:
        ...
        if not self.wait_for_detection(expected_count=count, timeout=wait_timeout):
             logger.warning("Detection timeout/failure. Proceeding with detected windows, if any.")
        ...
        logger.info(f"--- Cursor Swarm Setup Complete ({len(detected_windows)} detected) ---")
...
if __name__ == '__main__':
    ...
    try:
        ...
        if final_windows:
            logger.info(f"Bootloader finished. {len(final_windows)} Cursor instances are ready.")
            ...
            if len(final_windows) > 0:
                 ...
                 logger.info(f"Attempting to activate first detected window: {first_window_id}")
                 success = bootloader.controller.activate_window(final_windows[0])
                 logger.info(f"Activation successful: {success}")
        else:
            logger.warning("Bootloader finished, but no Cursor instances were successfully detected.")

    except FileNotFoundError as e:
         logger.critical(f"CRITICAL ERROR: {e}. Could not find Cursor. Please check path configuration.")
    except ImportError as e:
         logger.critical(f"CRITICAL ERROR: {e}. Missing required dependency (e.g., pywin32, pyobjc-framework-Cocoa, python-xlib, pyvda)?")
    except Exception as e:
        logger.critical(f"An unexpected error occurred during bootloader execution: {e}", exc_info=True)
