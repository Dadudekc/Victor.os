...
from .compliance import PrincipleCompliance, SafeguardCompliance, ValueCompliance
...
class EthosValidator:
    ...
    def _calculate_metrics(
        ...
        drift_score = (
            non_compliant_actions / total_actions if total_actions > 0 else 0.0
        )
    ...
    def _count_compliant_actions(self, action_logs: List[Dict[str, Any]]) -> int:
        ...
        for log in action_logs:
            ...
            if (
                value_result.is_compliant
                and principle_result.is_compliant
                and safeguard_result.is_compliant
            ):
                compliant_count += 1
    ...
    def _count_warning_actions(self, action_logs: List[Dict[str, Any]]) -> int:
        ...
        for log in action_logs:
            ...
            if (
                ...
                and safeguard_result.is_compliant
                and (
                    len(value_result.warnings) > 0
                    or len(principle_result.warnings) > 0
                    or len(safeguard_result.warnings) > 0
                )
            ):
                warning_count += 1
    ...
    def _generate_recommendations(self, metrics: ValidationMetrics) -> List[str]:
        ...
        if metrics.drift_score > 0.1:  # More than 10% drift
            recommendations.append(
                "High drift detected - review recent actions for ethos compliance"
            )

        # Check warning rate
        if (
            metrics.warning_actions > metrics.total_actions * 0.2
        ):  # More than 20% warnings
            recommendations.append(
                "High warning rate - investigate common warning patterns"
            )
        ...
        if "emotional_contexts" in emotional_trends:
            frustration_count = sum(
                1
                for ctx in emotional_trends["emotional_contexts"]
                if ctx.get("user_frustrated", False)
            )
            if (
                frustration_count > len(emotional_trends["emotional_contexts"]) * 0.3
            ):  # More than 30% frustration
                recommendations.append(
                    "High user frustration rate - review interaction patterns"
                )
        ...
        if "action_distribution" in action_stats:
            high_risk_actions = sum(
                count
                for action, count in action_stats["action_distribution"].items()
                if action.startswith("high_risk_")
            )
            if (
                high_risk_actions > action_stats["total_actions"] * 0.1
            ):  # More than 10% high-risk actions
                recommendations.append(
                    "High rate of high-risk actions - review risk assessment"
                )
...
def main():
    ...
    logger.info("\nEmotional Trends:")
    for intent, count in metrics.emotional_trends.get(
        "intent_distribution", {}
    ).items():
    ...
    logger.info("\nAction Statistics:")
    for action, count in metrics.action_statistics.get(
        "action_distribution", {}
    ).items():
