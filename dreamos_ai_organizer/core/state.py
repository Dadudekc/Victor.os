...
class StateDB:
    ...
    def fetch_agents(self):
        ...
        cur = self.conn.execute(
            "SELECT id, name, task, points, queue_depth, last_command_ts FROM agents ORDER BY name"
        )
    ...
    def update_task_lane(self, task_id: str, new_lane: str):
        ...
        try:
            with self.conn:
                self.conn.execute(
                    "UPDATE tasks SET lane = ? WHERE id = ?", (new_lane, task_id)
                )
        except sqlite3.Error as e:
    ...
    def add_devlog_entry(
        self, agent_name: str, event_type: str, status: str, details: dict
    ):
    ...
    def fetch_devlog_entries(self, limit: int = 100):
        ...
        try:
            ...
            cur = self.conn.execute(
                "SELECT * FROM devlog ORDER BY timestamp DESC LIMIT ?", (limit,)
            )
