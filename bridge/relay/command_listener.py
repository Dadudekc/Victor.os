...
COMMAND_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../incoming_commands'))
SCHEMA_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '../schemas/gpt_command_schema.json'))
...
class CommandFileHandler(FileSystemEventHandler):
    ...
    def process_file(self, file_path):
        try:
            ...
            logger.info(f"Payload {payload.get('request_id', '')} validated against schema.")

            # Process the command
            result = process_gpt_command(payload)
            logger.info(f"Processing result for {payload.get('request_id', '')}: {result.get('status')}")

            # Move to processed directory (or error directory)
            target_dir = PROCESSED_DIR if result.get('status') == 'success' else ERROR_DIR
            os.makedirs(target_dir, exist_ok=True)
            os.rename(file_path, os.path.join(target_dir, os.path.basename(file_path)))
            logger.info(f"Moved {os.path.basename(file_path)} to {os.path.basename(target_dir)} directory.")

        except json.JSONDecodeError:
            logger.error(f"Error decoding JSON from {file_path}. Moving to error directory.")
            os.makedirs(ERROR_DIR, exist_ok=True)
            os.rename(file_path, os.path.join(ERROR_DIR, os.path.basename(file_path)))
        except jsonschema.exceptions.ValidationError as ve:
            logger.error(f"Schema validation failed for {file_path}: {ve.message}. Moving to error directory.")
            os.makedirs(ERROR_DIR, exist_ok=True)
            os.rename(file_path, os.path.join(ERROR_DIR, os.path.basename(file_path)))
        except FileNotFoundError:
            logger.warning(f"File {file_path} not found, likely processed already or race condition.")
            ...
            try:
                ...
                os.rename(file_path, os.path.join(ERROR_DIR, os.path.basename(file_path)))
                 ...
                 logger.error(f"Failed to move corrupted file {file_path} to error dir: {move_err}")
