...
# --- Configuration ---
LOG_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../logs')) # Assume logs are in sandbox/logs
...
TIMESTAMP_TOLERANCE = timedelta(seconds=5) # How close timestamps need to be for scraper match
CYCLE_TIME = timedelta(seconds=1) # Define 1 cycle time
SUMMARY_LATENCY_TOLERANCE = 2 * CYCLE_TIME # Max time allowed from scraper log to summary log
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    ...
                    logger.error(f"Skipping invalid JSON line in {file_path}: {e} - Line: {line.strip()}")
...
def verify_traces():
    ...
    if not source_records:
        logger.warning(f"Source log {SOURCE_LOG} is empty or not found. Cannot perform verification.")
        return
    ...
    for src in source_records:
        ...
        if not src_ts or not src_prompt_sub:
            logger.warning(f"Source record {src_id} missing timestamp or usable prompt/query. Skipping.")
            continue

        found_scraper = None
        for scr in scraper_records:
            scr_ts = parse_timestamp(scr.get('timestamp'))
            scr_prompt = scr.get('prompt') or scr.get('query', '') # Match field in scraper log
        ...
        if not found_scraper:
            logger.warning(f"Trace broken: No matching scraper log found for source {src_id} ({src_prompt_sub}) around {src_ts}")
            failure_record = {"timestamp": datetime.now().isoformat(), "failure_point": "source_to_scraper", "source_id": src_id, "source_timestamp": src.get('timestamp'), "reason": "No matching scraper entry found within time/prompt tolerance."}
        ...
        scr_ts = parse_timestamp(found_scraper.get('timestamp')) # Re-parse needed timestamp
        
        found_summary = None
        for smr in summary_records:
            smr_ts = parse_timestamp(smr.get('timestamp'))
            smr_ref_id = smr.get('request_id') or smr.get('scraper_ref') # Field linking to scraper/source
            ...
            id_match = (smr_ref_id == src_id) or (smr_ref_id == scr_id) # Allow matching either ID
            latency_match = (smr_ts - scr_ts) <= SUMMARY_LATENCY_TOLERANCE if scr_ts and smr_ts else False
        ...
        if not found_summary:
            logger.warning(f"Trace broken: No summary log found within {SUMMARY_LATENCY_TOLERANCE} for scraper {scr_id} (Source: {src_id}) from {scr_ts}")
            failure_record = {"timestamp": datetime.now().isoformat(), "failure_point": "scraper_to_summary", "source_id": src_id, "scraper_id": scr_id, "scraper_timestamp": found_scraper.get('timestamp'), "reason": f"No matching summary entry found within latency tolerance ({SUMMARY_LATENCY_TOLERANCE})."}
            write_jsonl(FAILURE_LOG, failure_record)
            failure_detected = True
            failed_count += 1
            continue # Move to next source record
        ...
        success_record = {
            ...
            "summary_id": found_summary.get('request_id', 'Unknown Summary ID'), # Or other ID field
            "summary_timestamp": found_summary.get('timestamp'),
            "scraper_to_summary_latency_seconds": latency.total_seconds() if latency else None
        }
        write_jsonl(SUCCESS_LOG, success_record)
        verified_count += 1

    logger.info(f"Verification complete. Verified traces: {verified_count}, Failed traces: {failed_count}")

    # Autonomy Chain Step 5: Trigger Alert Flag
    if failure_detected:
        logger.warning("Failures detected during trace verification. Creating alert flag.")
...
if __name__ == "__main__":
    ...
    for log_path in [SOURCE_LOG, SCRAPER_LOG, SUMMARY_LOG]:
        if not os.path.exists(log_path):
            ...
            if log_path == SOURCE_LOG:
                mock_data.append({"request_id": "src-mock-1", "timestamp": ts_now, "parameters": {"query": "This is the first mock query for testing purposes."}})
            elif log_path == SCRAPER_LOG:
                 mock_data.append({"request_id": "scr-mock-1", "timestamp": (datetime.fromisoformat(ts_now) + timedelta(seconds=1)).isoformat(), "query": "This is the first mock query for testing purposes."})
            elif log_path == SUMMARY_LOG:
                 mock_data.append({"request_id": "src-mock-1", "timestamp": (datetime.fromisoformat(ts_now) + timedelta(seconds=2)).isoformat(), "summary": "Mock summary"})
    ...
    if mock_files_created:
         logger.info("Mock log files created. Please replace with actual logs for real verification.")
