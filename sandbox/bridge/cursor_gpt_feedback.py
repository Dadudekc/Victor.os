...
def format_feedback(input_payload):
    ...
    if status == "success":
        ...
        if result_type == "edit_file" or result_type == "run_terminal":
            ...
            output_payload["data"]["message"] = input_payload.get("message") # Optional message
        ...
        output_payload["data"]["error_message"] = input_payload.get("message", "Unknown error details.")
    else:
        # Handle unexpected status values
        output_payload["result_type"] = "error"
        output_payload["status"] = "error"
        output_payload["data"]["error_message"] = f"Invalid status '{status}' received in input."
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(json.dumps({
                ...
                "status": "error",
                "result_type": "error",
                "data": {"error_message": f"Error reading file {sys.argv[1]}: {e}"}
            }))
            sys.exit(1)
    else:
        try:
