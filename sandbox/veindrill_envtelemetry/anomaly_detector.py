...
class TimestampNormalizer:
    ...
    def _try_log_format(ts_string: str) -> Optional[datetime]:
        possible_formats = ['%Y-%m-%d %H:%M:%S.%f', '%Y-%m-%d %H:%M:%S,%f', '%Y-%m-%d %H:%M:%S']
...
class TelemetryParser:
    ...
    @staticmethod
    def parse_line(line_num: int, line: str) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
        ...
        if not line_content or line_content.startswith('##') or line_content.startswith('---') or "Timestamp," in line_content:
             return None, None # Skip silently

        # // EDIT START: Procedural parsing based on splitting by comma
        parts = [p.strip() for p in line_content.split(',')] # Split and strip whitespace
...
class AnomalyDetector:
    ...
    def check_record(self, record: Dict[str, Any]):
        ...
        if self.last_valid_record and self.last_valid_record["normalized_utc_dt"]:
            ...
            temp_diff = abs(record["temperature_c"] - self.last_valid_record["temperature_c"])
            humidity_diff = abs(record["humidity_percent"] - self.last_valid_record["humidity_percent"])
...
class TelemetryAnalyzer:
    ...
    def __init__(self, input_path: str, output_path: str, summary_output_path: str, temp_thresh: float, humid_thresh: float):
    ...
    def run_analysis(self) -> bool:
        ...
        try:
            ...
            all_timestamp_issues = self.all_parse_errors + analysis_results["timestamp_issues"]
            ...
            for anomaly in env_anomalies:
                if anomaly["type"] == "Temperature Shift":
                    self.max_temp_deviation = max(self.max_temp_deviation, anomaly["change"])
            ...
            self._generate_summary_report(all_timestamp_issues, env_anomalies, lines_processed_count)
    ...
    def _generate_report(self, timestamp_issues: List[Dict], env_anomalies: List[Dict]):
        ...
        summary = {
            ...
            "potential_drift_zones (lines with parse failures or non-monotonic)": sorted(list(set(
                [item['line'] for item in timestamp_issues if 'line' in item]
            )))
        }
    ...
    def _generate_summary_report(self, timestamp_issues: List[Dict], env_anomalies: List[Dict], processed_line_count: int):
        ...
        status = "Complete" if total_timestamp_errors == 0 and total_env_anomalies == 0 else "IssuesFound"
