...
def read_json_artifact(file_path, module_name):
    ...
    full_path = os.path.join(os.path.dirname(__file__), file_path) if not os.path.isabs(file_path) else file_path
    ...
    try:
        if os.path.exists(full_path):
            ...
            return {"status": "MISSING", "error": f"Artifact file not found: {full_path}"}
    except json.JSONDecodeError as e:
        return {"status": "ERROR", "error": f"Failed to parse JSON from {full_path}: {e}"}
...
def generate_summary_report(inputs):
    ...
    if any_errors:
        ...
        any_progress = any(d.get("status", "").upper() in ["COMPLETED", "WORKING", "IN_PROGRESS"] for d in inputs.values())
        if any_progress:
    ...
    report["summary_message"] = f"Bridge construction status: {report['overall_status']}. {len(inputs)} modules reporting."
    if any_errors:
...
if __name__ == "__main__":
    ...
    for key, path in INPUT_PATHS.items():
        ...
        absolute_input_path = os.path.join(workspace_root, "sandbox", path.lstrip("../"))
