...
SANDBOX_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, '..')) # Go up one level from tests
...
def calculate_drift(original_ts_str, normalized_ts_str, expected_utc_dt):
    ...
    try:
        ...
        print(f"Error calculating drift for '{normalized_ts_str}' vs '{expected_utc_dt}': {e}")
        return None, "Error"

def run_test(log_file_path):
    """Runs the drift detection test on the given log file."""
    results = []
    print(f"--- [DEBUG] Starting Drift Detection Test on {log_file_path} ---") # DEBUG PRINT
    try:
        print(f"--- [DEBUG] Attempting to open file: {log_file_path} ---") # DEBUG PRINT
        with open(log_file_path, 'r') as f:
            print(f"--- [DEBUG] File opened successfully. Reading lines... ---") # DEBUG PRINT
            for i, line in enumerate(f):
                ...
                # Calculate drift
                drift_seconds, drift_str = calculate_drift(original_ts_str, normalized_ts_str, expected_utc_dt)

                results.append({
                    "line": i + 1,
                    "original": original_ts_str,
                    "normalized_utc": normalized_ts_str if normalized_ts_str else "Parse Failed",
                    "expected_utc": expected_utc_dt.isoformat(timespec='milliseconds') + 'Z' if expected_utc_dt else "N/A",
    ...
    for r in results:
        print(f"{r['line']:<5} | {r['original']:<{max_orig}} | {r['normalized_utc']:<{max_norm}} | {r['expected_utc']:<{max_exp}} | {r['drift_display']:<{max_drift}} | {r['message']}")
...
if __name__ == "__main__":
    ...
    print(f"--- [DEBUG] Calculated log file path: {log_file_abs_path} ---") # DEBUG PRINT
