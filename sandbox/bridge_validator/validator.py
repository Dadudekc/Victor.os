...
DEFAULT_RESULTS_FILE = "sandbox/bridge_validator/validation_results_partial.json" # Updated output file
UNAVAILABLE_MODULES_FILE = "sandbox/bridge_validator/bridge_validator_unavailable_modules.json"
...
def trigger_bridge_task(task_payload):
    ...
    if MODULE_6_ENABLED and check_file_path("Module 6", MODULE_6_TRIGGER_SCRIPT): # Check if enabled AND exists
        ...
        try:
            ...
            return True
             ...
             print(f"ERROR: Module 6 Harness script not found at {MODULE_6_TRIGGER_SCRIPT}")
             return False
         ...
         if not MODULE_6_ENABLED:
              print(f"INFO: Module 6 not enabled, using Module 1.")
         else: # Implies file check failed
              print(f"WARNING: Module 6 enabled but script not found at {MODULE_6_TRIGGER_SCRIPT}. Falling back to Module 1.")
         ...
         try:
            ...
            print(f"INFO: Module 1 Relay API Response: {response.status_code} - {response.text}")
...
def check_bridge_status(task_id):
    ...
    try:
        ...
        response = requests.get(MODULE_2_FEEDBACK_API, params={'task_id': task_id}, timeout=10)
        response.raise_for_status()
        status_data = response.json()
        current_status = status_data.get("status", "UNKNOWN")
        print(f"INFO: Module 2 Status Response: {current_status} - {status_data.get('detail', '')}")
        # Return values expected: 'PENDING', 'RUNNING', 'COMPLETED', 'ERROR'
        return current_status
    except requests.exceptions.RequestException as e:
        print(f"WARNING: Module 2 Feedback API ({MODULE_2_FEEDBACK_API}) failed: {e}. Falling back to Module 5.")
    except Exception as e:
         print(f"WARNING: Unexpected error calling Module 2 ({MODULE_2_FEEDBACK_API}): {e}. Falling back to Module 5.")

    # Fallback to Module 5
    try:
        # Assumed API: GET /state?task_id=<task_id>
        response = requests.get(MODULE_5_STATE_API, params={'task_id': task_id}, timeout=10)
        response.raise_for_status()
...
def get_bridge_result(task_id):
    ...
    try:
         ...
         response = requests.get(MODULE_2_RESULT_API, params={'task_id': task_id}, timeout=10)
         response.raise_for_status()
...
def check_output_file():
    ...
    try:
        with open(SAMPLE_INPUT_FILE, 'r') as infile, open(SAMPLE_OUTPUT_FILE, 'r') as outfile:
...
def check_logs(task_id, module3_available):
    ...
    try:
        ...
        with open(MODULE_3_LOG_FILE, 'r') as f:
            log_content = f.read() # Read whole log for simplicity, might need optimization

        # --- Assumed Log Format Checks (NEEDS REFINEMENT) ---
        # Check for entries indicating start, progress, and end for the task_id
        start_found = f"task_id: {task_id}" in log_content and "status: PENDING" in log_content # Example
        progress_found = f"task_id: {task_id}" in log_content and "status: RUNNING" in log_content # Example
        end_found = f"task_id: {task_id}" in log_content and ("status: COMPLETED" in log_content or "status: ERROR" in log_content) # Example

        if start_found and progress_found and end_found:
            print("INFO: Found relevant entries in Module 3 logs.")
            return True
        else:
            print(f"WARNING: Did not find expected log entries for task {task_id}. Start: {start_found}, Progress: {progress_found}, End: {end_found}")
            return False
    except Exception as e:
    ...
    while retries <= PREFLIGHT_MAX_RETRIES:
        ...
        if essential_ready:
            ...
            return True # Ready
             ...
             print(f"WARNING: [Preflight Check Only] Essential modules not ready (Attempt {retries+1}/{PREFLIGHT_MAX_RETRIES+1}). Waiting {PREFLIGHT_RETRY_DELAY_SECS}s...")
             retries += 1
             if retries <= PREFLIGHT_MAX_RETRIES:
                 time.sleep(PREFLIGHT_RETRY_DELAY_SECS)

    print("ERROR: [Preflight Check Only] Essential modules did NOT become available after retries.")
    return False # Not ready

def run_validation():
    """Performs preflight checks and then executes the validation test."""
    print("=== Starting Bridge Integration Validation (Integration Mode w/ Preflight) ===")
    ...
    while retries <= PREFLIGHT_MAX_RETRIES:
        module_status = {
            ...
            'module6': check_file_path("Module 6 (Harness)", MODULE_6_TRIGGER_SCRIPT) if MODULE_6_ENABLED else 'DISABLED',
            'module7': check_api_endpoint("Module 7 (Summarizer)", MODULE_7_HEALTH_API),
        }

        # Check if essential modules (1, 2, 5) are ready
        essential_ready = module_status['module1'] and module_status['module2'] and module_status['module5']

        if essential_ready:
            print("INFO: [Preflight] Essential modules (1, 2, 5) are ready.")
            break # Exit retry loop
        else:
             print(f"WARNING: [Preflight] Essential modules not ready (Attempt {retries+1}/{PREFLIGHT_MAX_RETRIES+1}). Waiting {PREFLIGHT_RETRY_DELAY_SECS}s...")
             retries += 1
             if retries <= PREFLIGHT_MAX_RETRIES:
                 time.sleep(PREFLIGHT_RETRY_DELAY_SECS)

    print("--- Preflight Checks Complete ---")
    unavailable_modules = [name for name, status in module_status.items() if status is False] # Record only unavailable, not disabled
    ...
    if unavailable_modules:
         ...
         try:
             ...
             with open(UNAVAILABLE_MODULES_FILE, 'w') as f:
                 json.dump({"timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "unavailable": unavailable_modules}, f, indent=2)
    ...
    if not essential_ready:
         print("ERROR: Essential modules (1, 2, 5) did not become available after retries. Aborting validation.")
    ...
    try:
        ...
        results["steps"]["cleanup"] = {"status": "SUCCESS", "message": "Old output file removed (if existed)."}
    except Exception as e:
         results["steps"]["cleanup"] = {"status": "ERROR", "message": f"Failed to remove old output file: {e}"}
    ...
    if final_status == "COMPLETED" and module_status['module2']: # Only try if Module 2 was available
         task_result_data = get_bridge_result(validation_run_id)
         results["steps"]["result_retrieval"] = {"status": "SUCCESS" if task_result_data else "FAIL", "data": task_result_data}
         if not task_result_data:
              # If task completed but we can't get result, is that an error?
              print("WARNING: Task reported COMPLETED but failed to retrieve results via Module 2.")
              # Keep final_status as COMPLETED for now, but note the retrieval issue
    elif final_status != "COMPLETED":
         print(f"ERROR: Bridge task did not complete successfully (Final Status: {final_status}). Aborting further checks.")
         return results # Stop if task didn't complete

    # 3. Verify Output File (Crucial Check)
    output_verified = check_output_file()
    results["steps"]["output_verification"] = {"status": "SUCCESS" if output_verified else "FAIL"}

    # 4. Verify Logs (Using Module 3 interface, if available)
    logs_verified = check_logs(validation_run_id, module_status['module3'])
    results["steps"]["log_verification"] = {"status": "SUCCESS" if logs_verified else "FAIL", "log_file": MODULE_3_LOG_FILE, "checked": module_status['module3']}

    # 5. Call Summarizer (Module 7, if available)
    summary = call_summarizer(validation_run_id, task_result_data, module_status['module7'])
    results["steps"]["summarization"] = {"status": "SUCCESS" if summary else ("SKIPPED" if not module_status['module7'] else "FAIL"), "summary_generated": summary, "checked": module_status['module7']}
    ...
    if final_status == "COMPLETED" and output_verified:
        ...
        print(f"=== Validation FAILED (Integration Mode) - Status: {final_status}, Output OK: {output_verified}, Logs OK: {logs_verified} ===")
        results["status"] = "FAIL"

    return results

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run End-to-End Bridge Validation (Integration Mode w/ Prechecks).")
    parser.add_argument("--output", default=DEFAULT_RESULTS_FILE, help="JSON file to store validation results.")
    parser.add_argument("--check-only", action="store_true", help="Run only preflight checks for essential modules and exit.")
    # Add args to override module interfaces
    parser.add_argument("--module1-health-api", default=MODULE_1_HEALTH_API, help="Module 1 Health API endpoint.")
    parser.add_argument("--module2-health-api", default=MODULE_2_HEALTH_API, help="Module 2 Health API endpoint.")
    parser.add_argument("--module3-log-file", default=MODULE_3_LOG_FILE, help="Module 3 Log file path.")
    parser.add_argument("--module5-health-api", default=MODULE_5_HEALTH_API, help="Module 5 Health API endpoint.")
    parser.add_argument("--module6-script-path", default=MODULE_6_TRIGGER_SCRIPT, help="Module 6 Trigger script path.")
    parser.add_argument("--module7-health-api", default=MODULE_7_HEALTH_API, help="Module 7 Health API endpoint.")
    parser.add_argument("--enable-module6", action="store_true", help="Enable use of Module 6 Harness.")
    ...
    if args.check_only:
        ...
        try:
            ...
            print(f"Warning: Could not remove old unavailable modules log {UNAVAILABLE_MODULES_FILE}: {e}")
        ...
        if validation_results.get("status") not in ["PASS", "BLOCKED"]:
            exit(1)
        elif validation_results.get("status") == "BLOCKED": # Exit code 2 if blocked by preflight
             exit(2) 