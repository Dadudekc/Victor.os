...
def test_read_file(path: Path):
    ...
    try:
        ...
        elif isinstance(raw_content_or_response, dict) and 'content' in raw_content_or_response:
            content = raw_content_or_response['content']
        elif raw_content_or_response is None: # Handles if read_file returns None on error
             pass # Handled by "if not content" below


        duration = time.perf_counter() - start
        if not content and not (isinstance(raw_content_or_response, dict) and raw_content_or_response.get('error')):
            print(f"⚠️ EMPTY or None content returned [took {duration:.2f}s]")
        elif isinstance(raw_content_or_response, dict) and raw_content_or_response.get('error'):
            print(f"❌ Failed (returned error structure): {raw_content_or_response.get('error')} [took {duration:.2f}s]")
        else:
            # Truncate content for display if too long
            display_content = content[:200] + "..." if content and len(content) > 200 else content
            print(f"✅ Success [took {duration:.2f}s] – {len(content) if content else 0} chars. Preview: {display_content}")
...
def main():
    ...
    print(f"Please ensure this script is run from the project's root directory, or adjust TARGET_FILES paths.")
