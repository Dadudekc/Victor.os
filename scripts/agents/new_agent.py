...
AGENT_ID = "Agent-9"
MAILBOX_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../runtime/agent_comms/agent_mailboxes'))
...
def read_mailbox():
    ...
    try:
        ...
        mailbox_data = {"inbox": [], "outbox": [], "loop_state": {"status": "initializing"}}
        ...
        return {"inbox": [], "outbox": [], "loop_state": {"status": "error_mailbox_corrupt"}} # Indicate error state
...
def update_loop_state(current_state, status="running", last_processed_count=0):
    ...
    current_state["last_update_utc"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
...
def check_idle_state(loop_state):
    ...
    if idle_intervals >= MAX_IDLE_INTERVALS:
        print(f"WARNING: Agent {AGENT_ID} idle for {idle_intervals} intervals. Potential issue.")
...
def new_agent_loop():
    ...
    while True:
        ...
        if processed_message_ids or new_outbox_items:
            ...
            mailbox["inbox"] = [msg for msg in inbox_messages if msg.get('id') not in processed_message_ids]
            # Add new messages to outbox
            mailbox["outbox"] = mailbox.get("outbox", []) + new_outbox_items
            # Update loop state (active)
            mailbox["loop_state"] = update_loop_state(loop_state, status="active", last_processed_count=len(processed_message_ids))
            write_mailbox(mailbox)
            print(f"Processed {len(processed_message_ids)} messages. {len(new_outbox_items)} items added to outbox.")
        else:
        ...
        if current_time - last_heartbeat_time >= HEARTBEAT_INTERVAL:
            print(f"Heartbeat: {AGENT_ID} loop running. Status: {loop_state.get('status', 'unknown')}")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(MAILBOX_DIR):
        print(f"FATAL ERROR: Mailbox directory not found at expected location: {MAILBOX_DIR}")
        print("Ensure the script is run from the correct working directory or adjust MAILBOX_DIR.")
        exit(1)
    new_agent_loop() 