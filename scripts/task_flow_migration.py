        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    logger.error(f"JSON decode error in {file_path} at line {line_num}: {e}. Skipping line: '{line.strip()}'")
...
def transform_task(task_data: dict, source_file_type: str) -> dict | None:
    ...
    new_task = {
        "task_id": task_id,
        "action": task_data.get('task_type', task_data.get('name', 'migrated_task')), # Best effort for action
        "params": {
        ...
        "priority": task_data.get('priority'), # Assuming priority types are compatible or will be parsed
        "depends_on": task_data.get('dependencies', []),
        "retry_count": 0,
        "repair_attempts": 0,
        "failure_count": 0, # Legacy?
        "injected_at": task_data.get('created_at', datetime.datetime.now(datetime.timezone.utc).isoformat()),
    ...
    new_task['params']['_migration_info'] = {
         ...
         "original_assigned_agent": task_data.get('assigned_agent') or task_data.get('assigned_to'),
    ...
    new_task['params']['_migration_info'] = {k: v for k, v in new_task['params']['_migration_info'].items() if v is not None}

    # Validate required fields from schema
    required_fields = ["task_id", "action", "status", "priority", "injected_at", "injected_by"]
    is_valid = True
    for req_field in required_fields:
        if req_field not in new_task or new_task[req_field] is None:
            ...
            if req_field == 'action' and not new_task.get('action'):
                 ...
                 logger.warning(f"Task {task_id} missing 'action', defaulted to '{new_task['action']}'.")
            elif req_field == 'injected_by' and not new_task.get('injected_by'):
                 new_task['injected_by'] = 'TaskMigrationScript'
                 logger.warning(f"Task {task_id} missing 'injected_by', defaulted to '{new_task['injected_by']}'.")
            # Add other potential default logic here if needed
            
            # Re-check after attempting defaults
            if req_field not in new_task or new_task[req_field] is None:
                logger.error(f"Task {task_id} missing required field '{req_field}' after transformation and defaulting attempts. Cannot migrate.")
                is_valid = False
...
def merge_into_task_board(task_board_path: Path, new_tasks: list[dict], dry_run: bool = False) -> tuple[int, int]:
    """
    ...
    if task_board_path.exists():
        existing_tasks = read_jsonl_file(task_board_path) # Assuming task_board is also jsonl for consistency
    ...
    for task in new_tasks:
        ...
        if task_id in existing_task_ids:
            logger.info(f"Task ID {task_id} already exists in {task_board_path}. Skipping duplicate.")
            tasks_skipped_duplicates += 1
        else:
            final_task_list.append(task)
            existing_task_ids.add(task_id) # Add to set to catch duplicates within the new_tasks list too
            tasks_added += 1
        ...
        try:
            ...
            with open(task_board_path, 'w', encoding='utf-8') as f:
                for entry in final_task_list:
                    json.dump(entry, f)
                    f.write('\\n')
            logger.info(f"Successfully wrote {len(final_task_list)} tasks to {task_board_path}")
        except Exception as e:
            logger.error(f"Failed to write updated task board to {task_board_path}: {e}", exc_info=True)
            # Potentially re-raise or handle as a critical failure
    else:
        logger.info(f"[DRY RUN] Would write {len(final_task_list)} tasks to {task_board_path} ({tasks_added} new, {tasks_skipped_duplicates} duplicates skipped from source).")
        ...
        if old_file.exists():
            ...
            if not dry_run:
                try:
                    ...
                    logger.info(f"Successfully deprecated {old_file} to {deprecated_name}")
                except Exception as e:
                    logger.error(f"Failed to deprecate {old_file}: {e}", exc_info=True)
            else:
                logger.info(f"[DRY RUN] Would deprecate {old_file} to {deprecated_name}")
        else:
            logger.warning(f"Old file {old_file} not found for deprecation. Skipping.")

def main():
    parser = argparse.ArgumentParser(description="Task Flow Migration Script: Migrates tasks from old JSONL files to a new task_board.jsonl.")
    parser.add_argument("--working-tasks", type=Path, default=Path("working_tasks.json"), help="Path to working_tasks.jsonl")
    parser.add_argument("--future-tasks", type=Path, default=Path("future_tasks.json"), help="Path to future_tasks.jsonl")
    parser.add_argument("--task-ready-queue", type=Path, default=Path("task_ready_queue.json"), help="Path to task_ready_queue.jsonl (optional)")
    parser.add_argument("--task-board", type=Path, required=True, help="Path to the target task_board.jsonl")
    parser.add_argument("--backup-dir", type=Path, default=Path(DEFAULT_BACKUP_DIR), help="Directory to store backups.")
    parser.add_argument("--log-file", type=Path, default=None, help="Path to a log file. If None, logs to console only.")
    parser.add_argument("--log-level", type=str, default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], help="Logging level.")
    parser.add_argument("--dry-run", action="store_true", help="Simulate migration without making changes.")
    parser.add_argument("--deprecate-old", action="store_true", help="After successful migration (not in dry-run), deprecate old task files.")
    
    args = parser.parse_args()

    # Setup logging
    log_file_path = args.log_file if args.log_file else Path(f"task_migration_log_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    setup_logging(args.log_level, log_file_path)

    logger.info("--- Starting Task Flow Migration ---")
    if args.dry_run:
        logger.info("*** DRY RUN MODE ENABLED: No actual file changes will be made to task files. ***")

    # 1. Backup files
    source_files_to_backup = [args.working_tasks, args.future_tasks]
    if args.task_ready_queue and args.task_ready_queue.exists(): # Only backup if specified and exists
    ...
    for task in working_tasks_data:
        ...
        if transformed: all_old_tasks_transformed.append(transformed)
    ...
    logger.info(f"Total tasks read and transformed (pre-deduplication): {len(all_old_tasks_transformed)}")
    ...
    tasks_added, tasks_skipped = merge_into_task_board(args.task_board, all_old_tasks_transformed, args.dry_run)
    
    logger.info(f"Migration summary: Tasks added to board = {tasks_added}, Tasks skipped (duplicates from source files) = {tasks_skipped}")

    # 4. Deprecate old files (if requested and not dry run)
    if args.deprecate_old and not args.dry_run:
        if tasks_added > 0 or (tasks_added == 0 and len(all_old_tasks_transformed) > 0 and tasks_skipped == len(all_old_tasks_transformed)):
            ...
            deprecate_old_files(files_to_deprecate_paths, args.dry_run) # dry_run here is for safety, already checked
        else:
            logger.warning("Deprecation skipped: No new tasks were successfully added to the task board from the source files, or source files were empty.")
    elif args.deprecate_old and args.dry_run:
        logger.info("[DRY RUN] Deprecation step would run if not in dry mode and tasks were processed.")
