#!/usr/bin/env python3
"""
Cleanup script for duplicate directories in Dream.OS.
This script safely removes duplicate directories while preserving the most recent/relevant data.
"""
        ...
        try:
            ...
            for root, dirs, _ in os.walk(cleanup_backup_dir):
                ...
                if backup_count > 1:
                    ...
                    logging.info(f"Found nested backup (depth {backup_count}): {root_path}")
            ...
            for _, backup_path in nested_backups:
                if not backup_path.exists():  # Check again as parent deletion might have removed it
                    continue
