...
import uuid # For generating IDs if needed, though source should provide
...
COMMAND_FILE_PATH = project_root / "runtime" / "gpt_to_cursor_queue" / "command_input.json"
...
try:
    ...
    if hasattr(cursor_bridge, 'inject_prompt_into_cursor') and callable(cursor_bridge.inject_prompt_into_cursor):
        inject_prompt_func = cursor_bridge.inject_prompt_into_cursor
        logger.info("Successfully imported inject_prompt_into_cursor from cursor_bridge.")
    else:
        raise ImportError("Function not found or not callable")
except ImportError as e:
    logger.warning(f"Could not import inject_prompt_into_cursor from dreamos.tools.cursor_bridge: {e}. Using simulation.")
    # Define a mock function for simulation purposes
    def simulate_inject_prompt_into_cursor(prompt: str, **kwargs):
        logger.info(f"SIMULATED call to inject_prompt_into_cursor:")
...
def process_command(command_data: Dict[str, Any]) -> bool:
    ...
    if not all([command_id, payload_type, payload]):
        logger.error(f"Invalid command format received: Missing required fields. Data: {command_data}")
    ...
    try:
        ...
        if payload_type == "code_injection":
            ...
            if code:
                ...
                success = inject_prompt_func(prompt=code, is_code=True, language=language)
            else:
                logger.error(f"Missing 'code' in payload for code_injection (ID: {command_id})")

        elif payload_type == "chat_message":
            message = payload.get("message")
            if message:
                logger.info(f"Injecting chat message...")
                success = inject_prompt_func(prompt=message, is_code=False)
            else:
                logger.error(f"Missing 'message' in payload for chat_message (ID: {command_id})")
        ...
        if success:
            ...
            logger.error(f"Failed to inject command ID: {command_id} via cursor_bridge.")
            return False
            
    except Exception as e:
        logger.error(f"Unexpected error processing command ID {command_id}: {e}", exc_info=True)
        return False
...
def run_relay():
    ...
    """
    logger.info(f"Starting GPT->Cursor Relay ({RELAY_ID}). Monitoring: {COMMAND_FILE_PATH}")
        ...
        try:
            if COMMAND_FILE_PATH.exists():
                ...
                if current_mtime > last_command_file_mtime:
                    logger.info(f"Detected change in {COMMAND_FILE_PATH}. Checking for new command.")
                    ...
                    try:
                        ...
                        if not current_command_id:
                             logger.error("Command file is missing 'command_id'. Skipping.")
                        elif current_command_id != last_processed_command_id:
                            logger.info(f"New command ID detected: {current_command_id}")
                            if process_command(command_content):
                                last_processed_command_id = current_command_id
                                state["last_processed_command_id"] = last_processed_command_id
                                state["last_processed_timestamp_utc"] = datetime.now(timezone.utc).isoformat()
                                save_relay_state(state)
                            else:
                                # Optional: Implement retry logic or move to error queue
                                logger.error(f"Processing failed for command ID: {current_command_id}. Will retry on next file change.")
                        else:
                            logger.info(f"Command ID {current_command_id} already processed. No action needed.")
                            
                    except json.JSONDecodeError:
                        logger.error(f"Command file {COMMAND_FILE_PATH} contains invalid JSON. Skipping.")
                    except Exception as e:
                        logger.error(f"Error reading or processing command file {COMMAND_FILE_PATH}: {e}", exc_info=True)
                else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(f"GPT->Cursor Relay ({RELAY_ID}) encountered a critical error: {e}", exc_info=True)
        sys.exit(1) 