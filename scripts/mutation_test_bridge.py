...
try:
    from scripts.thea_to_cursor_agent import main_loop as agent_main_loop # Example target
...
@register_fault
def fault_raise_exception(original_func: Optional[Callable] = None, exc_type_str: str = "Exception", msg: str = "Simulated mutation error") -> Callable:
    """Mutation: Raise a specified exception by string name."""
    def mutated_func(*args, **kwargs):
        func_name = original_func.__name__ if original_func else "unknown_target"
        exc_type = getattr(sys.modules[__name__], exc_type_str, Exception) # Find exception type by name
        logger.warning(f"[MUTATION:{func_name}] Raising {exc_type.__name__}('{msg}')")
        raise exc_type(msg)
    return mutated_func

@register_fault
def fault_delay(original_func: Optional[Callable] = None, delay_seconds: float = 2.0) -> Callable:
    ...
    def mutated_func(*args, **kwargs):
        ...
        logger.warning(f"[MUTATION:{func_name}] Delay finished (original call skipped in this version)")
        return None # Or a mock value
    return mutated_func

@register_fault
def fault_return_corrupted_data(original_func: Optional[Callable] = None, corruption: Any = "<corrupted>") -> Callable:
     """Mutation: Return unexpected data type (e.g., bytes)."""
     def mutated_func(*args, **kwargs):
        func_name = original_func.__name__ if original_func else "unknown_target"
        logger.warning(f"[MUTATION:{func_name}] Returning corrupted data ({type(corruption)}): {str(corruption)[:50]}")
        return corruption
     return mutated_func
...
def create_mutator(fault_config: Dict[str, Any]) -> Optional[Callable]:
    ...
    try:
        ...
        return fault_builder(**params)
         ...
         logger.error(f"Error creating mutator for type '{fault_type}' with params {params}: {e}")
         return None
...
def run_mutation_test(target_path: str, mutator: Callable, config: AppConfig):
    ...
    logger.info(f"--- Running Mutation Test --- Target: [{target_path}], Mutation: [{getattr(mutator, '__name__', 'anonymous')}] ---")
    test_passed = False
    # We apply the mutator directly as the side_effect
    with patch(target_path, side_effect=mutator(None), create=True) as mock_target: # Pass None as original func initially
        logger.info("Executing test scenario with mutation applied...")
        try:
            # Placeholder for test execution logic
            print(f"[SIMULATION] Calling target logic with mutation on {target_path}...")
            ...
            logger.info("Test scenario execution finished (simulated). Outcome assumed OK.")
    ...
    result = "PASSED (Survived Mutation)" if test_passed else "FAILED (Crashed/Unexpected Behavior)"
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(f"Failed to load AppConfig: {e}. Cannot run tests.", exc_info=True)
        sys.exit(1)
    ...
    for scenario in scenarios:
        ...
        if not target or not mutations:
            logger.warning("Skipping scenario due to missing target function or mutations.")
            continue
        ...
        for mutation_config in mutations:
            ...
            if mutator:
                ...
                time.sleep(0.1) # Small delay between mutations
                 ...
                 logger.error(f"Skipping mutation due to creation error: {mutation_config}")
    ...
    if failed_count > 0:
        ...
        logger.error("Overall Result: FAILED (One or more mutations caused unexpected failure)")
    else:
