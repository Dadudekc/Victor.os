...
def get_paths(test_mode: bool = False) -> Dict[str, pathlib.Path]:
    ...
    return {
        ...
        "mailbox": pathlib.Path(f"{base}/{'agent_mailboxes' if test_mode else 'agent_comms/agent_mailboxes'}"),
...
def inject_prompt(coords: Dict[str, Dict[int, int]], agent_id: int, prompt: str, test_mode: bool = False) -> bool:
...
def save_response(paths: Dict[str, pathlib.Path], agent_id: int, response: str, test_mode: bool = False) -> bool:
...
def onboard_agent(paths: Dict[str, pathlib.Path], agent_id: int, test_mode: bool = False) -> bool:
    ...
    try:
        ...
        if test_mode:
            ...
            while time.time() - t0 < timing["timeout"]:
                ...
                safe_click(response_coords["x"], response_coords["y"], timing["click_delay"])
            ...
            if not response:
                print(f"Agent-{agent_id} ⚠️ no reply within {timing['timeout']}s timeout.")
                return False

        # Save response (both real and simulated)
        if save_response(paths, agent_id, response, test_mode):
            print(f"Agent-{agent_id} ✅ {'simulated ' if test_mode else ''}onboarding successful.")
            return True
        return False

    except Exception as e:
        print(f"Agent-{agent_id} ❌ error during {'simulated ' if test_mode else ''}onboarding: {e}")
        return False

def parse_args() -> argparse.Namespace:
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="Dream.OS Agent Auto-Onboarding")
    parser.add_argument("--test", action="store_true", help="Run in test mode without GUI interaction")
    parser.add_argument("--agents", type=int, nargs="+", help="Specific agent IDs to onboard (e.g. 1 2 3)")
