...
class DuplicateDirectoryCleaner:
    ...
    def cleanup_task_migration_backups(self) -> None:
        ...
        for backup in backup_dir.iterdir():
            if backup.is_dir() and backup.name.startswith("backup_"):
                ...
                if (
                    date not in backups_by_date
                    or backup.name > backups_by_date[date].name
                ):
    ...
    def _are_dirs_identical(self, dir1: Path, dir2: Path) -> bool:
        ...
        try:
            dir1_files = set(
                f.relative_to(dir1) for f in dir1.rglob("*") if f.is_file()
            )
            dir2_files = set(
                f.relative_to(dir2) for f in dir2.rglob("*") if f.is_file()
            )
