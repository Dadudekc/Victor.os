...
class TestTheaBridgePipeline(unittest.TestCase):
    ...
    def run_agent_cycle(self, 
        ...
        mock_sleep.side_effect = [None, break_exception] # Allow first sleep, then raise
        ...
        return mock_inject_cursor, mock_log_extract, mock_copy_reply, mock_scraper_instance

    # --- Test Cases per Mode ---

    def test_gui_mode_cycle(self):
        """Test a single cycle in GUI mode."""
        mock_inject, mock_log, mock_gui, mock_scraper_inst = self.run_agent_cycle(mode='gui')
        
        mock_gui.assert_called_once_with(config=self.dummy_config)
        mock_scraper_inst.extract_latest_reply.assert_not_called()
        mock_log.assert_called_once_with(method='gui', text=TEST_PROMPT_GUI, extraction_uuid=KNOWN_UUID)
        mock_inject.assert_called_once_with(prompt=TEST_PROMPT_GUI, config=self.dummy_config)
        self.assertNotEqual(thea_to_cursor_agent.last_processed_reply_hash, None) # State updated

    def test_gui_mode_no_new_reply(self):
        """Test GUI mode when copy_thea_reply returns None."""
        mock_inject, mock_log, mock_gui, _ = self.run_agent_cycle(mode='gui', gui_return=None)
    ...
    def test_scraper_mode_cycle(self):
        ...
        mock_inject, mock_log, mock_gui, mock_scraper_inst = self.run_agent_cycle(mode='scraper')
        
        mock_gui.assert_not_called()
        mock_scraper_inst.extract_latest_reply.assert_called_once()
        mock_log.assert_called_once_with(method='scraper', text=TEST_PROMPT_SCRAPER, extraction_uuid=KNOWN_UUID)
        mock_inject.assert_called_once_with(prompt=TEST_PROMPT_SCRAPER, config=self.dummy_config)
    ...
    def test_scraper_mode_no_new_reply(self):
        ...
        mock_inject, mock_log, mock_gui, mock_scraper_inst = self.run_agent_cycle(mode='scraper', scraper_return=None)
    ...
    def test_hybrid_mode_scraper_first(self):
        ...
        mock_inject, mock_log, mock_gui, mock_scraper_inst = self.run_agent_cycle(mode='hybrid')
    ...
    def test_hybrid_mode_gui_fallback(self):
        ...
        mock_inject, mock_log, mock_gui, mock_scraper_inst = self.run_agent_cycle(mode='hybrid', scraper_return=None)
        
        mock_scraper_inst.extract_latest_reply.assert_called_once()
        mock_gui.assert_called_once_with(config=self.dummy_config) # GUI called as fallback
        mock_log.assert_called_once_with(method='gui', text=TEST_PROMPT_GUI, extraction_uuid=KNOWN_UUID)
        mock_inject.assert_called_once_with(prompt=TEST_PROMPT_GUI, config=self.dummy_config)
        self.assertNotEqual(thea_to_cursor_agent.last_processed_reply_hash, None)
    ...
    def test_hybrid_mode_both_fail(self):
        ...
        mock_inject, mock_log, mock_gui, mock_scraper_inst = self.run_agent_cycle(mode='hybrid', scraper_return=None, gui_return=None)
    ...
    def test_duplicate_suppression(self):
        ...
        # Run first cycle
        mock_inject, mock_log, _, _ = self.run_agent_cycle(mode='gui', gui_return="First reply")
        ...
        with patch('time.sleep', side_effect=[None, StopIteration("Break 2")]) as mock_sleep_2:
            try:
                 thea_to_cursor_agent.main_loop(self.dummy_config)
            except StopIteration as e:
                 if str(e) != "Break 2": raise

        mock_log.assert_not_called() # Should not log again
        mock_inject.assert_not_called() # Should not inject again
        self.assertEqual(thea_to_cursor_agent.last_processed_reply_hash, first_hash) # Hash unchanged
