...
from unittest.mock import MagicMock, patch  # Still using mock for dependencies
...
def rotate_log_if_needed(log_path: Path, max_bytes: int, log_mb_size: int):
    ...
    try:
        if log_path.exists() and log_path.stat().st_size > max_bytes:
            ...
            logger.info(
                f"Log file {log_path} exceeds {log_mb_size}MB. Rotating to {backup_path}..."
            )
            ...
            with open(log_path, "w", encoding="utf-8") as f:
                if header:
                    ...
                    f.write("")  # Default empty for unknown logs
...
def write_results_log(results: List[Dict]):
    ...
    try:
        with open(STRESS_TEST_LOG_FILE, "a", encoding="utf-8") as f:
            ...
            f.write(
                f"Mode: {TEST_MODE.upper()}, Messages: {NUM_MESSAGES}, Interval: {INTERVAL_SECONDS}s\n\n"
            )
            f.write(
                "| Iteration | UUID | Timestamp Start | Extraction Method | Extraction Success | Injection Success | Latency (ms) | Notes |\n"
            )
            f.write("|---|---|---|---|---|---|---|---|\n")
            for r in results:
                ...
                notes = r.get("notes", "")
                f.write(
                    f"| {r['iteration']} | `{r['uuid']}` | {r['timestamp_start']} | {r['extraction_method']} | {r['extraction_success']} | {r['injection_success']} | {latency_ms} | {notes} |\n"
                )
            f.write("\n")
        ...
        # Failsafe logging for I/O
        logger.error(
            f"FAILED to write results log {STRESS_TEST_LOG_FILE}: {e}", exc_info=True
        )
...
def run_stress_test(mock_scraper_cls, mock_inject):
    ...
    logger.info(
        f"Starting stress test: {NUM_MESSAGES} messages, {INTERVAL_SECONDS}s interval, Mode: {TEST_MODE.upper()}"
    )
    results = []
    ...
    for i in range(NUM_MESSAGES):
        ...
        if TEST_MODE == "gui":
            with patch(
                "scripts.stress_test_bridge.extract_via_gui",
                side_effect=noisy_gui_extract,
            ):
            ...
            with patch(
                "scripts.stress_test_bridge.extract_via_scraper",
                side_effect=noisy_scraper_extract,
            ):
        ...
        if extracted_text:
            ...
            try:
                ...
                logger.error(
                    f"FAILED to hash extracted text for UUID {message_uuid}: {e}",
                    exc_info=True,
                )
            ...
            if reply_hash is not None and reply_hash in processed_reply_hashes:
                ...
                try:
                    ...
                    synthetic_delay = max(
                        0,
                        random.gauss(
                            SYNTHETIC_LATENCY_MS_MEAN, SYNTHETIC_LATENCY_MS_STDDEV
                        )
                        / 1000.0,
                    )
                    ...
                    if injection_success:
                        ...
                        logger.info(
                            f"Mock injection successful for UUID {message_uuid}. Latency: {latency_sec:.4f}s (incl. synthetic {synthetic_delay:.3f}s)"
                        )
                    ...
                    logger.error(
                        f"FAILED during mock injection for UUID {message_uuid}: {e}",
                        exc_info=True,
                    )
        ...
        jitter = (
            base_wait_time * INTERVAL_JITTER_PERCENT * (random.random() * 2 - 1)
        )  # +/- jitter %
        ...
        if wait_time > 0:
            logger.debug(
                f"Sleeping for {wait_time:.2f} seconds (Base: {INTERVAL_SECONDS}s, Jitter: {jitter:.2f}s)... "
            )
            time.sleep(wait_time)

    test_end_time = time.time()
    logger.info(
        f"Stress test loop finished. Total time: {test_end_time - test_start_time:.2f} seconds."
    )
    return results
...
def audit_results(test_results: List[Dict]):
    ...
    logger.warning(
        "Audit validation against thea_extraction_relay.md is skipped (logging was mocked during test). Verifying collected results only."
    )
    ...
    injection_failures = [
        ...
        if r["extraction_success"] and not r["injection_success"]
    ]
    latency_failures = [r for r in test_results if "LATENCY FAIL" in r.get("notes", "")]
    duplicate_skips = [
        r for r in test_results if "Duplicate content detected" in r.get("notes", "")
    ]
    ...
    if not injection_failures:
        ...
        logger.error(
            f"Injection validation: FAILED ({len(injection_failures)} failures in results)"
        )
    ...
    if not latency_failures:
        logger.info(
            f"Latency validation: PASSED (No results exceeded {MAX_LATENCY_SECONDS}s)"
        )
    else:
        logger.error(
            f"Latency validation: FAILED ({len(latency_failures)} results exceeded threshold)"
        )
        for failure in latency_failures:
            logger.error(
                f"  - Latency failure for UUID: {failure['uuid']} ({failure.get('latency_ms')}ms)"
            )

    logger.info(
        f"Duplicate content check: Found {len(duplicate_skips)} instances where injection was skipped due to simulated duplicate content."
    )
    ...
    final_success = (
        not injection_failures and not latency_failures and mock_cursor_check_passed
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if test_results is not None:
            ...
            logger.info(
                f"Stress test audit result: {'PASSED' if audit_passed else 'FAILED'}"
            )
        else:
            logger.error(
                "Stress test did not produce results (likely config or setup error)."
            )
    except Exception as e:
        logger.critical(
            f"Stress test script encountered unhandled exception: {e}", exc_info=True
        )
    finally:
