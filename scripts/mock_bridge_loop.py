...
def generate_mock_response(prompt_text: str, agent_id: str) -> str: # Return type is str
    """
...
def main():
    ...
    parser.add_argument("--prompt-file", required=True, type=Path, help="Path to the file containing the prompt.")
    parser.add_argument("--response-timeout", type=int, default=90, help="Timeout in seconds (mostly ignored by mock).")
    ...
    try:
        ...
        if not prompt_text:
            print(f"[MockBridgeLoop] WARNING: Prompt file is empty.")
    ...
    try:
        ...
        print(f"[MockBridgeLoop] Mock response successfully written to: {output_file_path}")
    except Exception as e:
