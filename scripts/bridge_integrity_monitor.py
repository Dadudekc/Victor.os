...
try:
    ...
    print(
        "CRITICAL: Cannot import AppConfig/load_config. Ensure src directory is in PYTHONPATH."
    )
    ...
    if is_failure:
        ...
        if check_name in consecutive_failures:
            if consecutive_failures[check_name] > 0:
                logger.info(
                    f"Check '{check_name}' succeeded after {consecutive_failures[check_name]} failures. Resetting counter."
                )
                md_logger.info(
                    f"    - **INFO:** Check '{check_name}' resolved after previous failures.\n"
                )
            consecutive_failures[check_name] = 0
...
def check_bridge_mode_config():
    ...
    try:
        ...
        if mode not in ["gui", "scraper", "hybrid"]:
            log_anomaly(
                check_name,
                f"Invalid mode '{mode}' found in {config_file}. Allowed: gui, scraper, hybrid.",
            )
        else:
...
def check_chatgpt_cookies():
    ...
    if not cookie_file.exists():
        ...
        log_anomaly(
            check_name,
            f"Cookie file not found: {cookie_file}. This may be normal before first scraper login.",
            is_failure=False,
        )  # Log as WARN
    else:
...
def watchdog_loop():
    ...
    while True:
        ...
        logger.info(
            f"Integrity checks complete. Sleeping for {CHECK_INTERVAL_SECONDS} seconds."
        )
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(
            f"Bridge Integrity Monitor encountered fatal error: {e}", exc_info=True
        )
