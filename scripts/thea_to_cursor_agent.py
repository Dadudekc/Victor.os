...
from src.dreamos.services.utils.chatgpt_scraper import ChatGPTScraper, WebDriverException
...
def check_dependencies(mode: BridgeMode):
    ...
    scraper_needed = mode in ['scraper', 'hybrid'] # Requires selenium, undetected-chromedriver

    if gui_needed:
        if not PYAUTOGUI_AVAILABLE:
            logger.critical("PyAutoGUI is required for GUI/Hybrid mode. Please run: pip install pyautogui")
            return False
        if not PYPERCLIP_AVAILABLE:
            logger.critical("Pyperclip is required for GUI/Hybrid mode. Please run: pip install pyperclip")
            return False
        if not PYGETWINDOW_AVAILABLE:
            logger.warning("PyGetWindow is not installed. Window focus checks might be less reliable in GUI/Hybrid mode.")
    ...
    if scraper_needed:
        ...
        try:
            ...
            logger.critical(f"Scraper/Hybrid mode requires selenium and undetected-chromedriver: {e}")
            return False
...
def load_bridge_mode() -> BridgeMode:
    ...
    try:
        ...
        if mode not in ("gui", "scraper", "hybrid"):
            logger.warning(f"Invalid mode '{mode}' in {BRIDGE_MODE_CONFIG_FILE}. Defaulting to 'gui'.")
            return "gui"
        logger.info(f"Loaded bridge mode: {mode}")
        return mode
    except FileNotFoundError:
        logger.warning(f"{BRIDGE_MODE_CONFIG_FILE} not found. Defaulting to 'gui' mode.")
        return "gui"
    except (json.JSONDecodeError, Exception) as e:
        logger.error(f"Error reading {BRIDGE_MODE_CONFIG_FILE}: {e}. Defaulting to 'gui' mode.", exc_info=True)
        return "gui"
...
def extract_via_gui(config: AppConfig) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"Error during GUI extraction (copy_thea_reply): {e}", exc_info=True)
        return None
...
def extract_via_scraper(scraper: ChatGPTScraper) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"Error during Scraper extraction (extract_latest_reply): {e}", exc_info=True)
        return None
    ...
    if current_mode in ('scraper', 'hybrid'):
        try:
            ...
            if not scraper_instance.load_cookies():
                logger.warning("ChatGPT cookies not found or invalid. Requires manual login in the browser window!")
                # Give time for login
                time.sleep(30)
            ...
            logger.critical(f"Failed to initialize ChatGPTScraper: {e}. Cannot run in scraper/hybrid mode.", exc_info=True)
            # Fallback? For now, exit if primary mode fails.
            if current_mode == 'scraper':
                if scraper_instance: scraper_instance.cleanup()
                return # Exit if scraper mode fails to init
            else: # Hybrid mode, fallback to GUI
                logger.warning("Scraper init failed in Hybrid mode, falling back to GUI only.")
                current_mode = 'gui'
        ...
        while True:
            ...
            if current_mode == 'gui':
                ...
                if scraper_instance:
                    ...
                    time.sleep(POLL_INTERVAL_SECONDS * 2) # Longer sleep if scraper failed
                    continue
            ...
            if extracted_text:
                ...
                if current_reply_hash != last_processed_reply_hash:
                    ...
                    logger.info(f"Extracted new reply (UUID: {extraction_uuid}) via {extraction_method.upper()} (hash: {current_reply_hash}). Length: {len(extracted_text)} chars.")
                    # --- Pass UUID to logger --- 
                    log_extraction(method=extraction_method, text=extracted_text, extraction_uuid=extraction_uuid)
                    ...
                    try:
                        ...
                        logger.info(f"Successfully injected prompt (UUID: {extraction_uuid}) into Cursor.")
                        last_processed_reply_hash = current_reply_hash # Update state only on success
                    ...
                    logger.debug(f"Extracted reply ({extraction_method.upper()}) is the same as the last processed one. Skipping.")
            else:
