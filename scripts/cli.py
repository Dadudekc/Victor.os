...
# ───────────────────────── Backend imports ─────────────────────────
try:
    from dreamos.core.config import AppConfig, ConfigError, setup_logging
...
def configure_logging(config: AppConfig, verbose: bool) -> None:
    ...
    level = (
        logging.DEBUG
        if verbose
        else getattr(config.logging.level.upper(), "INFO", logging.INFO)
    )
    logging.basicConfig(
        level=level,
...
def validate_config(
    config_path: Path = typer.Option(
        Path("config/config.yaml"), "--config", "-c", help="Path to config file."
    ),
):
    """
    ...
    if not config_path.exists():
        click.secho(
            f"Configuration file not found at {config_path}; skipping validation.",
            fg="yellow",
        )
        return
    try:
