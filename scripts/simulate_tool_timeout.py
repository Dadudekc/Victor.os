...
def simulate_potential_timeout(target_path_str: str, operation: str):
    ...
    if operation == "read_file" and target_path.name == "task_backlog.json":
        ...
        raise TimeoutError(
            f"Simulated forced timeout during '{operation}' on '{target_path_str}'"
        )
    ...
    if random.random() < timeout_prob:
        logger.error(
            f"Simulating random timeout exception for '{operation}' on '{target_path_str}' (Prob: {timeout_prob:.2f})"
        )
        raise TimeoutError(
            f"Simulated random timeout during '{operation}' on '{target_path_str}'"
        )
    else:
        logger.info(
            f"Operation '{operation}' on '{target_path_str}' completed without simulated timeout."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate tool timeouts for testing resilience."
    )
    parser.add_argument(
        "target_path",
        type=str,
        help="The target file or directory path for the simulated operation.",
    )
    parser.add_argument(
        "operation",
        type=str,
        help="Description of the simulated operation (e.g., 'read_file', 'list_dir').",
    )
    parser.add_argument(
        "--delay",
        type=float,
        default=DEFAULT_DELAY_SECONDS,
        help="Base delay in seconds.",
    )
    parser.add_argument(
        "--prob",
        type=float,
        default=DEFAULT_TIMEOUT_PROBABILITY,
        help="Probability (0.0 to 1.0) of a random timeout.",
    )
    ...
    logger.info(
        f"Starting simulation: Operation='{args.operation}', Target='{args.target_path}', Delay={args.delay}s, TimeoutProb={args.prob}"
    )
