...
def get_latest_response_file(
    agent_id: str, outbox_dir: Path, after_timestamp: datetime
) -> Optional[Path]:
        ...
        if f_path.is_file():
            ...
            if match:
                try:
                    ...
                    print(
                        f"Warning: Could not parse timestamp from {f_path.name}",
                        file=sys.stderr,
                    )
                    continue
...
def run_self_prompt(prompt_text: str):
    ...
    BRIDGE_OUTBOX_DIR.mkdir(
        parents=True, exist_ok=True
    )  # Ensure bridge_outbox dir exists
    ...
    try:
        ...
        command = [
            ...
            str(AGENT_ID),
            "--prompt-file",
            str(prompt_file_path.resolve()),
            "--response-timeout",
            str(RESPONSE_TIMEOUT),
            "--outbox",
            str(BRIDGE_OUTBOX_DIR.resolve()),  # Explicitly pass outbox
        ]
        ...
        time.sleep(1)
        latest_response_path = get_latest_response_file(
            AGENT_ID, BRIDGE_OUTBOX_DIR, execution_start_time
        )
        ...
        if latest_response_path and latest_response_path.exists():
            ...
            try:
                ...
                if llm_response is not None:
                    print(
                        f"--- Response from LLM (from {latest_response_path.name}) ---"
                    )
                    print(llm_response)
                    print(
                        "----------------------------------------------------------\n"
                    ...
                    print(
                        f"Error: 'response' key not found in JSON data in {latest_response_path.name}."
                    )
                ...
                print(
                    f"Error reading or parsing response file {latest_response_path.name}: {e_read}"
                )
        else:
            print(
                f"No new response file found for Agent {AGENT_ID} in {BRIDGE_OUTBOX_DIR} created after {execution_start_time.strftime('%Y%m%dT%H%M%SZ')}."
            )
            print(
                "This might indicate the bridge loop did not produce an output file, or an error occurred before file creation."
            )
            print(
                "Review the Stdout/Stderr above for clues from the bridge loop itself."
            )
