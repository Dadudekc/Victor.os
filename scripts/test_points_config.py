...
PROJECT_ROOT = Path(__file__).resolve().parents[1] # Assumes scripts/test_points_config.py
SRC_PATH = PROJECT_ROOT / "src"
if str(SRC_PATH) not in sys.path:
    sys.path.insert(0, str(SRC_PATH))
if str(PROJECT_ROOT) not in sys.path: # Ensure project root itself is also there for potential top-level imports if any
...
try:
    ...
    logger.info(f"Attempting to import AppConfig from dreamos.core.config")
    from dreamos.core.config import AppConfig, DEFAULT_CONFIG_PATH
    logger.info(f"Successfully imported AppConfig. Default config path: {DEFAULT_CONFIG_PATH}")
    
    logger.info("Attempting to import AgentPointsManager from dreamos.governance.agent_points_manager")
    from dreamos.governance.agent_points_manager import AgentPointsManager, DEFAULT_POINT_VALUES
    logger.info("Successfully imported AgentPointsManager.")

except ImportError as e:
    logger.error(f"Failed to import necessary modules. Please ensure PYTHONPATH is set correctly or run from project root.", exc_info=True)
    logger.error("Make sure the script is in a 'scripts' subdirectory of your project, and 'src' is at the same level as 'scripts'.")
    sys.exit(1)
...
def main():
    ...
    if not config_file_to_test.exists():
        ...
        logger.error("Please ensure 'runtime/config/config.yaml' exists and contains the 'agent_points_system' section.")
        sys.exit(1)
    ...
    try:
        ...
        if app_config.agent_points_system:
            logger.info(f"AppConfig.agent_points_system successfully loaded: {app_config.agent_points_system.model_dump()}")
        else:
        ...
        for key in test_keys:
            ...
            if key in DEFAULT_POINT_VALUES and (not app_config.agent_points_system or key not in app_config.agent_points_system.point_values):
                expected_points = f"(expected default: {DEFAULT_POINT_VALUES[key]})"
            elif app_config.agent_points_system and key in app_config.agent_points_system.point_values:
        ...
        configured_task_completion = points_manager.get_points_for_reason("task_completion")
        default_if_missing = DEFAULT_POINT_VALUES.get("task_completion")
        
        if app_config.agent_points_system and "task_completion" in app_config.agent_points_system.point_values:
            if configured_task_completion == app_config.agent_points_system.point_values["task_completion"]:
                logger.info("SUCCESS: 'task_completion' points match config.yaml.")
            else:
                logger.error("FAILURE: 'task_completion' points DO NOT match config.yaml.")
        elif configured_task_completion == default_if_missing:
            logger.info("SUCCESS: 'task_completion' points match internal default (config.yaml entry missing or section missing).")
        else:
            logger.error(f"FAILURE: Unexpected value for 'task_completion': {configured_task_completion}. Default: {default_if_missing}")

        non_existent_points = points_manager.get_points_for_reason("non_existent_key")
        if non_existent_points == 0:
            logger.info("SUCCESS: Retrieval for 'non_existent_key' correctly defaulted to 0.")
        else:
            logger.error(f"FAILURE: Retrieval for 'non_existent_key' was {non_existent_points}, expected 0.")
