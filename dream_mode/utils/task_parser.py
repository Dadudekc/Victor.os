...
def _extract_json_block(text: str) -> dict | None:
    ...
    try:
        ...
        if isinstance(data, dict) and REQUIRED_JSON_KEYS.issubset(data.keys()):
            ...
            logger.warning(f"⚠️ Found JSON block, but missing required keys: {REQUIRED_JSON_KEYS - set(data.keys())}")
            return None
...
FEEDBACK_PATTERN_REGEX = re.compile(r"(?i)^\s*(APPROVED|REVISE|REJECTED)(?::\s*(.+))?", re.MULTILINE)
...
def _extract_regex_fallback(text: str) -> dict:
    ...
    if feedback_match:
        ...
        result["details"] = feedback_match.group(2).strip() if feedback_match.group(2) else ""
