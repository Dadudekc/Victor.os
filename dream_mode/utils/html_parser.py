...
STOP_GENERATING_SELECTOR = (By.XPATH, "//button[contains(., 'Stop generating')] | //button[contains(@aria-label, 'Stop generating')] | //textarea[@disabled]") 
        ...
        for element in stop_elements:
            if element.is_displayed():
                 ...
                 if element.tag_name == 'button':
                     ...
                     return True
                 elif element.tag_name == 'textarea' and element.get_attribute('disabled'):
...
def extract_latest_reply(driver):
    ...
    try:
        ...
        containers = driver.find_elements(By.CSS_SELECTOR, "div[data-message-author-role='assistant']")
        ...
        for div in reversed(containers):
            text = div.text.strip()
            if text:
                logger.info("✅ Latest assistant reply extracted (generation finished).")
                return text
