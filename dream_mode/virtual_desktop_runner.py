...
if platform.system() == "Linux":
    try: # Added try-except for Linux dependency
        ...
        raise ImportError("Please install pyvirtualdisplay: pip install pyvirtualdisplay")
...
class VirtualDesktopController:
    ...
    def launch_cursor_headless(self, cursor_exe_path: str, args=None):
        ...
        if self.os == "Linux":
            ...
            print(f"Launched Cursor headless on Linux (PID: {self._process.pid}) under DISPLAY {self._display.display}")
            time.sleep(5)
            ...
            try:
                ...
                print(f"Warning: Could not get current desktop number via VirtualDesktop.current(): {e}. Assuming 1.")
            ...
            try:
                ...
                desktop_count_verified = True # Mark as verified
                if self._new_desktop > desktop_count:
                    print(f"Target desktop {self._new_desktop} does not exist. Attempting creation...")
                    try:
                        # Attempt to create the desktop
                        pyvda.create_virtual_desktop()
                        print(f"Attempted to create virtual desktop {self._new_desktop}.")
                        desktop_created = True
                        time.sleep(1) # Pause briefly after creation attempt
                    except AttributeError:
                         print(f"Warning: pyvda module has no attribute 'create_virtual_desktop'. Cannot create.")
                         raise RuntimeError(f"Target virtual desktop {self._new_desktop} does not exist and cannot be created.")
                    except Exception as create_err:
                        print(f"Warning: Error calling create_virtual_desktop(): {create_err}. Cannot guarantee creation.")
            except AttributeError:
                 # Specifically handle the case where .count() doesn't exist
                 print(f"Warning: Could not verify desktop count via VirtualDesktop.count() (AttributeError). Assuming target {self._new_desktop} exists.")
            except RuntimeError as e:
                print(f"Runtime Error: {e}")
                raise
            except Exception as e_count: # Catch other errors during count
                # Now 'e_count' is defined in this scope
                print(f"Warning: Could not verify desktop count: {e_count}. Assuming target {self._new_desktop} exists.")
            
            try:
                print(f"Switching to desktop: {self._new_desktop}")
                pyvda.VirtualDesktop(self._new_desktop).go()
            except Exception as e:
                 print(f"ERROR switching to desktop {self._new_desktop} via VirtualDesktop(n).go(): {e}")
                 print(f"Launching on original desktop {self._orig_desktop} due to switch failure.")
                 self._new_desktop = self._orig_desktop # Keep track of where we actually launch
            
            print(f"Launching Cursor on virtual desktop {self._new_desktop}")
            try:
                self._process = subprocess.Popen([cursor_exe_path] + (args or []))
                print(f"Launched Cursor (PID: {self._process.pid}) on virtual desktop {self._new_desktop}")
                time.sleep(5)
            except Exception as e:
                ...
                if self._new_desktop != self._orig_desktop:
                    try:
                        ...
                        pyvda.VirtualDesktop(self._orig_desktop).go()
                         ...
                         print(f"Could not switch back to original desktop {self._orig_desktop} after launch failure: {switch_err}")
                raise
    ...
    def inject_keystrokes(self, text: str, interval: float = 0.01):
        ...
        print(f"Injecting text to virtual desktop {self._new_desktop if self.os == 'Windows' else 'Xvfb'}: '{text[:50]}...'" ) # Adjusted log
        # Ensure we are on the correct desktop before injecting (Windows specific precaution)
        if self.os == "Windows":
            try: 
                 # Attempt Class-based API for current desktop
                current_desk = pyvda.VirtualDesktop.current().number
                if current_desk != self._new_desktop:
                     print(f"Warning: Current desktop ({current_desk}) is not the target ({self._new_desktop}). Switching back...")
                      # Attempt Class-based API for switching
                     pyvda.VirtualDesktop(self._new_desktop).go()
                     time.sleep(0.5)
            except Exception as e:
                print(f"Warning: Could not verify/switch current desktop before injection: {e}. Proceeding anyway.")
        ...
        try:
            ...
            if self.os == "Windows" and self._orig_desktop is not None:
                 ...
                 try:
                     ...
                     print(f"Could not switch back to original desktop {self._orig_desktop}: {switch_err}")
            raise # Re-raise the exception - Ensure this is aligned with the outer try/except
    ...
    def teardown(self):
        ...
        if self.os == "Linux":
            ...
            if self._process:
                print(f"Terminating Cursor process (PID: {self._process.pid}) (likely on virtual desktop {self._new_desktop})")
                self._process.terminate()
                try:
            ...
            if self._orig_desktop is not None and self._new_desktop != self._orig_desktop:
                print(f"Switching back to original desktop: {self._orig_desktop}")
                try:
                    # Attempt Class-based API for switching
                    pyvda.VirtualDesktop(self._orig_desktop).go()
                except Exception as e:
                    print(f"ERROR switching back to original desktop {self._orig_desktop}: {e}")
            ...
            # Attempt removal ONLY if we ended up on the new desktop AND creation might have occurred
            # This is heuristic - might need refinement
            if self._new_desktop is not None and self._new_desktop != self._orig_desktop:
                ...
                try:
                    ...
                    # Since the API is unclear and removal can fail if it's the last desktop,
                    # we will skip actual removal for now to avoid errors.
                    print(f"INFO: Automatic removal of desktop {self._new_desktop} skipped (API unclear/potential issues).")
                except Exception as remove_err:
                    print(f"Error attempting removal of desktop {self._new_desktop}: {remove_err}")
            else:
                print(f"Skipping removal of virtual desktop {self._new_desktop} (was original or not targeted).")
...
if __name__ == "__main__":
    ...
    if platform.system() == "Windows":
        ...
        CURSOR_PATH = next((path for path in possible_paths if os.path.exists(path)), None)
        if not CURSOR_PATH:
            raise FileNotFoundError("Could not automatically find Cursor.exe. Please set CURSOR_PATH manually.")
         ...
         CURSOR_PATH = next((path for path in possible_paths if os.path.exists(path)), None)
         if not CURSOR_PATH:
             raise FileNotFoundError("Could not automatically find cursor executable. Please set CURSOR_PATH manually.")
    else:
    ...
    if platform.system() == "Linux":
        try:
            from pyvirtualdisplay import Display
             ...
             print("Error: pyvirtualdisplay is required on Linux. pip install pyvirtualdisplay")
             sys.exit(1)
        if not any(os.access(os.path.join(path, 'Xvfb'), os.X_OK) for path in os.environ["PATH"].split(os.pathsep)):
            print("Error: Xvfb is required on Linux but not found in PATH. Please install it (e.g., sudo apt-get install xvfb).")
            sys.exit(1)
